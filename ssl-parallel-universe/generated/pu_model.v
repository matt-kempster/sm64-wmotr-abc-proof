...
Module Info.
  ...
  Definition source_file := "inputs/pu_model.c".
  Definition normalized := true.
End Info.

Definition _PUState : ident := $"PUState".
...
Definition _clamp_area2_coord : ident := $"clamp_area2_coord".
Definition _clamp_normal_step : ident := $"clamp_normal_step".
Definition _delta : ident := $"delta".
Definition _dx : ident := $"dx".
Definition _dz : ident := $"dz".
Definition _in_parallel_universe : ident := $"in_parallel_universe".
Definition _is_parallel_universe_coord : ident := $"is_parallel_universe_coord".
Definition _main : ident := $"main".
Definition _nextX : ident := $"nextX".
Definition _nextZ : ident := $"nextZ".
Definition _pu_abs : ident := $"pu_abs".
Definition _ssl_area2_normal_step : ident := $"ssl_area2_normal_step".
Definition _ssl_area2_step_enters_parallel_universe : ident := $"ssl_area2_step_enters_parallel_universe".
...
Definition f_pu_abs := {|
  ...
  fn_params := ((_value, tint) :: nil);
  fn_vars := nil;
  fn_temps := nil;
  fn_body :=
(Ssequence
  (Sifthenelse (Ebinop Olt (Etempvar _value tint)
                 (Econst_int (Int.repr 0) tint) tint)
    (Sreturn (Some (Eunop Oneg (Etempvar _value tint) tint)))
    Sskip)
  (Sreturn (Some (Etempvar _value tint))))
|}.

Definition f_clamp_area2_coord := {|
  fn_return := tint;
  fn_callconv := cc_default;
  fn_params := ((_value, tint) :: nil);
  fn_vars := nil;
...
(Ssequence
  (Sifthenelse (Ebinop Olt (Etempvar _value tint)
                 (Eunop Oneg (Econst_int (Int.repr 8192) tint) tint) tint)
    (Sreturn (Some (Eunop Oneg (Econst_int (Int.repr 8192) tint) tint)))
    Sskip)
  (Ssequence
    (Sifthenelse (Ebinop Ogt (Etempvar _value tint)
                   (Econst_int (Int.repr 8191) tint) tint)
      (Sreturn (Some (Econst_int (Int.repr 8191) tint)))
      Sskip)
    (Sreturn (Some (Etempvar _value tint)))))
|}.

Definition f_clamp_normal_step := {|
  fn_return := tint;
  fn_callconv := cc_default;
  fn_params := ((_delta, tint) :: nil);
  fn_vars := nil;
  fn_temps := nil;
  fn_body :=
(Ssequence
  (Sifthenelse (Ebinop Olt (Etempvar _delta tint)
                 (Eunop Oneg (Econst_int (Int.repr 4096) tint) tint) tint)
    (Sreturn (Some (Eunop Oneg (Econst_int (Int.repr 4096) tint) tint)))
    Sskip)
  (Ssequence
    (Sifthenelse (Ebinop Ogt (Etempvar _delta tint)
                   (Econst_int (Int.repr 4096) tint) tint)
      (Sreturn (Some (Econst_int (Int.repr 4096) tint)))
      Sskip)
    (Sreturn (Some (Etempvar _delta tint)))))
|}.

Definition f_is_parallel_universe_coord := {|
  fn_return := tint;
  fn_callconv := cc_default;
  fn_params := ((_value, tint) :: nil);
  fn_vars := nil;
  ...
  fn_body :=
(Ssequence
  (Scall (Some _t'1) (Evar _pu_abs (Tfunction (tint :: nil) tint cc_default))
    ((Etempvar _value tint) :: nil))
  (Sreturn (Some (Ebinop Oge (Etempvar _t'1 tint)
                   (Econst_int (Int.repr 32768) tint) tint))))
|}.

Definition f_in_parallel_universe := {|
  fn_return := tint;
  fn_callconv := cc_default;
  fn_params := ((_state, (tptr (Tstruct _PUState noattr))) :: nil);
  fn_vars := nil;
...
(Ssequence
  (Ssequence
    (Ssequence
      (Sset _t'4
        (Efield
          (Ederef (Etempvar _state (tptr (Tstruct _PUState noattr)))
            (Tstruct _PUState noattr)) _x tint))
      (Scall (Some _t'1)
        (Evar _is_parallel_universe_coord (Tfunction (tint :: nil) tint
                                            cc_default))
    ...
    (Ssequence
      (Ssequence
        (Sset _t'3
          (Efield
            (Ederef (Etempvar _state (tptr (Tstruct _PUState noattr)))
              (Tstruct _PUState noattr)) _z tint))
        (Scall (Some _t'2)
          (Evar _is_parallel_universe_coord (Tfunction (tint :: nil) tint
                                              cc_default))
...
Definition f_ssl_area2_normal_step := {|
  fn_return := tvoid;
  fn_callconv := cc_default;
  fn_params := ((_state, (tptr (Tstruct _PUState noattr))) :: (_dx, tint) ::
                (_dz, tint) :: nil);
  fn_vars := nil;
  fn_temps := ((_nextX, tint) :: (_nextZ, tint) :: (_t'4, tint) ::
...
(Ssequence
  (Ssequence
    (Sset _t'7
      (Efield
        (Ederef (Etempvar _state (tptr (Tstruct _PUState noattr)))
          (Tstruct _PUState noattr)) _area tint))
  ...
  (Ssequence
    (Ssequence
      (Scall (Some _t'1)
        (Evar _clamp_normal_step (Tfunction (tint :: nil) tint cc_default))
        ((Etempvar _dx tint) :: nil))
      (Ssequence
        (Sset _t'6
          (Efield
            (Ederef (Etempvar _state (tptr (Tstruct _PUState noattr)))
              (Tstruct _PUState noattr)) _x tint))
        (Sset _nextX
          (Ebinop Oadd (Etempvar _t'6 tint) (Etempvar _t'1 tint) tint))))
    (Ssequence
      (Ssequence
        (Scall (Some _t'2)
          (Evar _clamp_normal_step (Tfunction (tint :: nil) tint cc_default))
          ((Etempvar _dz tint) :: nil))
        (Ssequence
          (Sset _t'5
            (Efield
              (Ederef (Etempvar _state (tptr (Tstruct _PUState noattr)))
                (Tstruct _PUState noattr)) _z tint))
          (Sset _nextZ
            (Ebinop Oadd (Etempvar _t'5 tint) (Etempvar _t'2 tint) tint))))
      (Ssequence
        (Ssequence
          (Scall (Some _t'3)
            (Evar _clamp_area2_coord (Tfunction (tint :: nil) tint
                                       cc_default))
            ((Etempvar _nextX tint) :: nil))
          (Sassign
            (Efield
              (Ederef (Etempvar _state (tptr (Tstruct _PUState noattr)))
                (Tstruct _PUState noattr)) _x tint) (Etempvar _t'3 tint)))
        (Ssequence
          (Scall (Some _t'4)
            (Evar _clamp_area2_coord (Tfunction (tint :: nil) tint
                                       cc_default))
            ((Etempvar _nextZ tint) :: nil))
          (Sassign
            (Efield
              (Ederef (Etempvar _state (tptr (Tstruct _PUState noattr)))
                (Tstruct _PUState noattr)) _z tint) (Etempvar _t'4 tint)))))))
|}.

Definition f_ssl_area2_step_enters_parallel_universe := {|
  fn_return := tint;
  fn_callconv := cc_default;
  fn_params := ((_state, (tptr (Tstruct _PUState noattr))) :: (_dx, tint) ::
                (_dz, tint) :: nil);
...
(Ssequence
  (Scall None
    (Evar _ssl_area2_normal_step (Tfunction
                                   ((tptr (Tstruct _PUState noattr)) ::
                                    tint :: tint :: nil) tvoid cc_default))
    ((Etempvar _state (tptr (Tstruct _PUState noattr))) ::
     (Etempvar _dx tint) :: (Etempvar _dz tint) :: nil))
  (Ssequence
    (Scall (Some _t'1)
      (Evar _in_parallel_universe (Tfunction
                                    ((tptr (Tstruct _PUState noattr)) :: nil)
                                    tint cc_default))
      ((Etempvar _state (tptr (Tstruct _PUState noattr))) :: nil))
    (Sreturn (Some (Etempvar _t'1 tint)))))
|}.

Definition composites : list composite_definition :=
(Composite _PUState Struct
   (Member_plain _area tint :: Member_plain _x tint ::
    Member_plain _z tint :: nil)
   noattr :: nil).
...
((___compcert_va_int32,
 ...
 (___builtin_debug,
   Gfun(External (EF_external "__builtin_debug"
     ...
     (tint :: nil) tvoid
     {|cc_vararg:=(Some 1); cc_unproto:=false; cc_structret:=false|})) ::
 (_pu_abs, Gfun(Internal f_pu_abs)) ::
 (_clamp_area2_coord, Gfun(Internal f_clamp_area2_coord)) ::
 (_clamp_normal_step, Gfun(Internal f_clamp_normal_step)) ::
 (_is_parallel_universe_coord, Gfun(Internal f_is_parallel_universe_coord)) ::
 (_in_parallel_universe, Gfun(Internal f_in_parallel_universe)) ::
 (_ssl_area2_normal_step, Gfun(Internal f_ssl_area2_normal_step)) ::
 (_ssl_area2_step_enters_parallel_universe, Gfun(Internal f_ssl_area2_step_enters_parallel_universe)) ::
 nil).

Definition public_idents : list ident :=
(_ssl_area2_step_enters_parallel_universe :: _ssl_area2_normal_step ::
 _in_parallel_universe :: _is_parallel_universe_coord ::
 _clamp_normal_step :: _clamp_area2_coord :: _pu_abs :: ___builtin_debug ::
