...
Module Info.
  ...
  Definition source_file := "experiments/toy/toy.c".
...
Definition _gObjects : ident := $"gObjects".
...
Definition v_gObjects := {|
  gvar_info := (tarray (tptr (Tstruct _Object noattr)) 10);
  gvar_init := (Init_space 80 :: nil);
  gvar_readonly := false;
  gvar_volatile := false
|}.

Definition v_gUnrelated := {|
  gvar_info := tint;
...
(Sassign
  (Efield
    (Ederef (Etempvar _o (tptr (Tstruct _Object noattr)))
      (Tstruct _Object noattr)) _timer tint) (Econst_int (Int.repr 0) tint))
|}.
...
(Composite _Object Struct
   (Member_plain _timer tint :: Member_plain _other tint :: nil)
   noattr :: nil).
