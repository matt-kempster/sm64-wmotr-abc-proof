...
From SM64.Proofs Require Import CensusV2 EngineV2Consumer RestSurface
  AirborneSurface DispatchKit FloorsSurface LocalVarsSurface OutParamSurface.
...
(* the THIRD-position NON-POINTER-scalar residual shape (the np3 class:
   set_mario_anim_with_accel).  Mario's pointer first, anything second,
   a NON-POINTER value third.  Weaker than call_pres_act3's untainted
   premise -- the third arg (an animation accel) is an arbitrary scalar
   the caller computes from a float, NOT an action value, so untainted
   is neither available nor needed: the callee stores it RAW into the
   SafeB object block (`o->animInfo.animAccel = accel`, a tint<-tint
   ptr32 cast_case_pointer passthrough), where MWF's chase row only
   needs it to not forge a pointer.  No return-value claim. *)
Definition call_pres_np3 (lp : Clight.program) (bm : block)
    (NoA MWF : mem -> Prop) (fid : ident) : Prop :=
  forall fd m0 v0 v1 v2 rest t0 m1 vres0,
    eval_funcall function_entry2 (lp_ge lp) m0 fd
      (v0 :: v1 :: v2 :: rest) t0 m1 vres0 ->
    resolves_lp lp fid fd ->
    marg_ok bm (v0 :: v1 :: v2 :: rest) ->
    (forall bb oo, v2 <> Vptr bb oo) ->
    NoA m0 -> MWF m0 -> Mem.valid_block m0 bm ->
    action_sat not_tainted m0 bm ->
    Mem.valid_block m1 bm /\ action_sat not_tainted m1 bm /\
    MWF m1 /\ NoA m1.
...
(* float types -- the SOUND non-pointer cast sources: a float-to-int
   sem_cast is cast_case_s2i / cast_case_f2i (Vint-or-stuck), never the
   ptr32 cast_case_pointer passthrough (that trap is i32-SOURCE-only;
   see the reverted i2i widening). *)
Definition float_ty (ty : type) : bool :=
  match ty with Tfloat _ _ => true | _ => false end.

Lemma sem_cast_float_i32_nonptr : forall v ty1 ty2 m w,
    float_ty ty1 = true -> i32_ty ty2 = true ->
    sem_cast v ty1 ty2 m = Some w ->
    forall bb oo, w <> Vptr bb oo.
Proof.
  intros v ty1 ty2 m w H1 H2 Hc bb oo ->.
  destruct ty1 as [ | | | sz1 a1' | | | | | ]; try discriminate H1.
  destruct ty2 as [ | sz2 sg2 a2' | | | | | | | ]; try discriminate H2.
  destruct sz2; try discriminate H2.
  unfold sem_cast, classify_cast in Hc;
    destruct (intsize_eq I32 IBool) as [EE|EE]; [ discriminate EE | ];
    destruct sz1; destruct v; try discriminate Hc.
  - destruct (cast_single_int sg2 f); discriminate Hc.
...
(* EVERY unary operator is non-Vptr-producing: notbool goes through
   bool_val to a Vint; notint/neg/absfloat are classify-dispatched
   numeric combinators that get STUCK on a pointer operand. *)
Lemma sem_unop_nonptr : forall op v ty m w,
    sem_unary_operation op v ty m = Some w ->
    forall b o, w <> Vptr b o.
Proof.
  intros op v ty m w H b o ->.
  destruct op; cbn [sem_unary_operation] in H.
  - unfold sem_notbool in H.
    destruct (bool_val v ty m) as [bb|]; [ | discriminate H ].
    cbn in H. injection H as H. destruct bb; discriminate H.
  - unfold sem_notint in H.
    destruct (classify_notint ty); destruct v; try discriminate H;
      injection H as H; discriminate H.
  - unfold sem_neg in H.
    destruct (classify_neg ty); destruct v; try discriminate H;
      injection H as H; discriminate H.
  - unfold sem_absfloat in H.
    destruct (classify_neg ty); destruct v; try discriminate H;
      injection H as H; discriminate H.
Qed.
...
Definition wchase_rhs_ok (wact : list ident) (ty : type) (a2 : expr)
  ...
  || (i32_ty ty && match a2 with
                   ...
                   (* EVERY unop is non-Vptr-producing (sem_unop_nonptr):
...
Definition npsrc_pair_chk (nids wact cact : list ident) (s1 s2 : statement)
    : bool :=
  match s1, s2 with
...
(* the SafeB-to-SafeB fused PAIR `t = <chase>[...]; <chase>[...] = t`:
   an i32 cell MOVED between two censused chase targets (act_in_cannon's
   rawData shuffles `marioObj->asS32[33] = usedObj->asS32[16]`).  The
   loaded value may genuinely be a pointer (rawData is a union), but it
   is SafeB-if-a-pointer by the chase load-closure (HchaseStep -- on
   ptr32 the i32 chunk IS Mptr), so the store preserves MWF via the
   SafeB-value chase row (HMWF_chase_safe).  Fused so the provenance
   never needs a census list. *)
Definition sbsrc_chk (cact : list ident) (a : expr) : bool :=
  match a with
  | Ederef _ ty =>
      i32_ty ty
      && match chain_root_l a with
         | Some cs => mem_id cs cact
         | None => false
         end
  | _ => false
  end.

Definition sbpair_chk (nids wact cact : list ident) (s1 s2 : statement)
    : bool :=
  match s1, s2 with
  | Sset t a, Sassign a1 (Etempvar q _) =>
      Pos.eqb q t
...
(* what an nids (non-pointer) temp may be Sset from: a float-to-int
...
(* the nids-temp chase store (`o->animInfo.animAccel = accel`): the
   stored temp's value is non-Vptr by the threaded nptr provenance, the
   target block SafeB by the chase census; consumed by the existing
   VALUE-form brick chase_assign_value_pres. *)
Definition nptr_store_chk (nids cact : list ident) (a1 a2 : expr)
    : bool :=
  match chain_root_l a1, a2 with
  | Some ct, Etempvar q _ => mem_id ct cact && mem_id q nids
  | _, _ => false
  end.
...
(* ====================================================================== *)
(* The OBJECT-writer (sc) call recognizer (mirror of wc_call_chk): fid in  *)
(* sc_pids, the dst (first) arg is a chase chain `chase->...->fld[]` whose  *)
(* root temp is a CENSUSED chase temp (cact, => SafeB by chase_inv), and    *)
(* the function's first param is a pointer (the write target).  This is the *)
(* vec3f_copy / vec3s_set OBJECT call shape in set_pole_position /          *)
(* act_tornado_twirling.  sc_call_chk_pres (in Section ActWriterWalk) is    *)
(* the engine-callable unit it gates on (consuming sc_scall_pres +          *)
(* fsc_arg0_safe).                                                          *)
(* ====================================================================== *)
Definition sc_call_chk (sc_pids cact : list ident) (fid : ident)
    (fty : type) (al : list expr) : bool :=
  mem_id fid sc_pids
  && match fty, al with
     | Tfunction (Tpointer _ _ :: _) _ _, Efield a' fld (Tarray _ _ _) :: _ =>
         match chain_root_e a' with
         | Some cid => mem_id cid cact
         | None => false
         end
     | _, _ => false
     end.

Lemma sc_call_chk_decode : forall sc_pids cact fid fty al,
    sc_call_chk sc_pids cact fid fty al = true ->
    mem_id fid sc_pids = true /\
    exists ety2 pattr tyl rty cc a' fld ety n aattr rest cid,
      fty = Tfunction (Tpointer ety2 pattr :: tyl) rty cc /\
      al = Efield a' fld (Tarray ety n aattr) :: rest /\
      chain_root_e a' = Some cid /\
      mem_id cid cact = true.
Proof.
  intros sc_pids cact fid fty al H.
  unfold sc_call_chk in H.
  apply andb_true_iff in H as [Hfid H].
  split; [ exact Hfid | ].
  destruct fty as [ | | | | | | tyl rty cc | | ]; try discriminate H.
  destruct tyl as [| tp1 tyl']; try discriminate H.
  destruct tp1 as [ | i1 i2 i3 | l1 l2 | f1 f2 | ety2 pattr
                  | ar1 ar2 ar3 | pr1 pr2 pr3 | st1 st2 | un1 un2 ];
    try discriminate H.
  destruct al as [| a0 rest]; try discriminate H.
  destruct a0 as [ i0 t0 | f0 t0 | s0 t0 | l0 t0 | v0 t0 | tv0 t0
                 | d0 t0 | ad0 t0 | u0 u0' u0'' | bo0 b0 b0' t0 | c0 t0
                 | a' fld afty | sz0 t0 | al0 t0 ]; try discriminate H.
  destruct afty as [ | i1 i2 i3 | l1' l2' | r1 r2 | p1 p2
                   | ety n aattr | params res cc' | st1 st2 | un1 un2 ];
    try discriminate H.
  destruct (chain_root_e a') as [cid|] eqn:Hcr; try discriminate H.
  exists ety2, pattr, tyl', rty, cc, a', fld, ety, n, aattr, rest, cid.
  split; [ reflexivity | ]. split; [ reflexivity | ].
  split; [ exact Hcr | exact H ].
Qed.
...
Fixpoint wwalk_chk' (lids oc_pids wc_pids sc_pids nids np3_ids : list ident) (rt : bool)
      ...
      | Evar fid fty =>
          ...
          && ((mem_id fid xids
              ...
              || ((match optid with
                   ...
                   && sc_call_chk sc_pids cact fid fty al)
              || match fty, al with
                        ...
                        | Some t =>
                            ...
                            then (* an act-writer call: result feeds wact,
                              ...
                              && match tys, args with
                                 ...
                                 || np3_call_chk nids np3_ids fid tys args
                        | None => mem_id fid ids
                                  || smact_call_chk wact sids fid tys args
                                  || act3_call_chk tids fid tys args
                                  || np3_call_chk nids np3_ids fid tys args
  ...
  | Ssequence s1 s2 =>
      npsrc_pair_chk nids wact cact s1 s2
      || sbpair_chk nids wact cact s1 s2
      || (wwalk_chk' lids oc_pids wc_pids sc_pids nids np3_ids rt wact ids wids cact xids sids tids s1
          && wwalk_chk' lids oc_pids wc_pids sc_pids nids np3_ids rt wact ids wids cact xids sids tids s2)
  | Sifthenelse _ s1 s2 =>
      wwalk_chk' lids oc_pids wc_pids sc_pids nids np3_ids rt wact ids wids cact xids sids tids s1
      && wwalk_chk' lids oc_pids wc_pids sc_pids nids np3_ids rt wact ids wids cact xids sids tids s2
  | Sloop s1 s2 =>
      wwalk_chk' lids oc_pids wc_pids sc_pids nids np3_ids rt wact ids wids cact xids sids tids s1
      && wwalk_chk' lids oc_pids wc_pids sc_pids nids np3_ids rt wact ids wids cact xids sids tids s2
  | Sswitch _ sl => wwalk_chk_ls' lids oc_pids wc_pids sc_pids nids np3_ids rt wact ids wids cact xids sids tids sl
  | _ => false
  end
with wwalk_chk_ls' (lids oc_pids wc_pids sc_pids nids np3_ids : list ident) (rt : bool)
    (wact ids wids cact xids sids tids : list ident)
    (sl : labeled_statements) : bool :=
  match sl with
  | LSnil => true
  | LScons _ s sl' =>
      wwalk_chk' lids oc_pids wc_pids sc_pids nids np3_ids rt wact ids wids cact xids sids tids s
      && wwalk_chk_ls' lids oc_pids wc_pids sc_pids nids np3_ids rt wact ids wids cact xids sids tids sl'
  end.
...
Definition wwalk_chk (rt : bool) (wact ids wids cact xids sids tids : list ident)
    (s : statement) : bool :=
  wwalk_chk' nil nil nil nil nil nil rt wact ids wids cact xids sids tids s.
Definition wwalk_chk_ls (rt : bool) (wact ids wids cact xids sids tids : list ident)
    (sl : labeled_statements) : bool :=
  wwalk_chk_ls' nil nil nil nil nil nil rt wact ids wids cact xids sids tids sl.

(* ---- the switch-selection transfer (mirror of walk_chk's) ---- *)

Lemma wwalk_chk_ls_seq' : forall lids oc_pids wc_pids sc_pids nids np3_ids rt wact ids wids cact xids sids tids sl,
    wwalk_chk_ls' lids oc_pids wc_pids sc_pids nids np3_ids rt wact ids wids cact xids sids tids sl = true ->
    wwalk_chk' lids oc_pids wc_pids sc_pids nids np3_ids rt wact ids wids cact xids sids tids (seq_of_labeled_statement sl) = true.
Proof.
  intros lids oc_pids wc_pids sc_pids nids np3_ids rt wact ids wids cact xids sids tids sl;
...
Lemma wwalk_chk_ls_case' : forall lids oc_pids wc_pids sc_pids nids np3_ids rt wact ids wids cact xids sids tids n sl sl',
    wwalk_chk_ls' lids oc_pids wc_pids sc_pids nids np3_ids rt wact ids wids cact xids sids tids sl = true ->
    select_switch_case n sl = Some sl' ->
    wwalk_chk_ls' lids oc_pids wc_pids sc_pids nids np3_ids rt wact ids wids cact xids sids tids sl' = true.
Proof.
  intros lids oc_pids wc_pids sc_pids nids np3_ids rt wact ids wids cact xids sids tids n sl;
...
Lemma wwalk_chk_ls_default' : forall lids oc_pids wc_pids sc_pids nids np3_ids rt wact ids wids cact xids sids tids sl,
    wwalk_chk_ls' lids oc_pids wc_pids sc_pids nids np3_ids rt wact ids wids cact xids sids tids sl = true ->
    wwalk_chk_ls' lids oc_pids wc_pids sc_pids nids np3_ids rt wact ids wids cact xids sids tids (select_switch_default sl)
    = true.
Proof.
  intros lids oc_pids wc_pids sc_pids nids np3_ids rt wact ids wids cact xids sids tids sl;
    induction sl as [| o s sl0 IH]; intros H.
...
Lemma wwalk_chk_select' : forall lids oc_pids wc_pids sc_pids nids np3_ids rt wact ids wids cact xids sids tids n sl,
    wwalk_chk_ls' lids oc_pids wc_pids sc_pids nids np3_ids rt wact ids wids cact xids sids tids sl = true ->
    wwalk_chk' lids oc_pids wc_pids sc_pids nids np3_ids rt wact ids wids cact xids sids tids
      (seq_of_labeled_statement (select_switch n sl)) = true.
Proof.
  intros lids oc_pids wc_pids sc_pids nids np3_ids rt wact ids wids cact xids sids tids n sl H. apply wwalk_chk_ls_seq'.
  unfold select_switch.
  destruct (select_switch_case n sl) eqn:E.
  - exact (wwalk_chk_ls_case' _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ H E).
  - exact (wwalk_chk_ls_default' _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ H).
Qed.
...
Section ActWriterWalk.
  ...
  (* the non-pointer-temp invariant (the nids census): a censused nids
     temp never holds a pointer (Vundef at entry; re-established only
     from float-to-int casts). *)
  Definition nptr_inv (nids : list ident) (le : temp_env) : Prop :=
    forall t, mem_id t nids = true ->
      forall v, le ! t = Some v -> forall bb oo, v <> Vptr bb oo.
  ...
  Proof.
    ...
    { unfold wchase_rhs_ok in Hrhs.
      ...
      - apply andb_prop in HI32 as [Hi32t Ha2].
        ...
        + (* a unop rhs: sem_unary_operation never yields a pointer *)
          match goal with
          | Hev2 : eval_expr _ _ _ _ (Eunop _ _ _) _ |- _ =>
              inv Hev2;
              try (match goal with
                   | Hlv : eval_lvalue _ _ _ _ (Eunop _ _ _) _ _ _ |- _ =>
                       inv Hlv
                   end)
          end.
          match goal with
          | Hcast0 : sem_cast ?vv _ _ _ = Some _,
            Hsem : sem_unary_operation _ _ _ _ = Some ?vv |- _ =>
              exact (sem_cast_nonptr_pres _ _ _ _ _ Hcast0
                       (sem_unop_nonptr _ _ _ _ _ Hsem))
          end.
  ...
  (* ================================================================== *)
  Lemma fsc_arg0_safe :
    forall cact e le m cid a' fld ety n aattr ety2 attr args tys vargs,
      mem_id cid cact = true ->
      chain_root_e a' = Some cid ->
      chase_inv cact le ->
      eval_exprlist (lp_ge lp) e le m
        (Efield a' fld (Tarray ety n aattr) :: args)
        (Tpointer ety2 attr :: tys) vargs ->
      arg0_safe SafeB vargs.
  Proof.
    intros cact e le m cid a' fld ety n aattr ety2 attr args tys vargs
           Hcid Hcr Hch Hvl.
    inv Hvl.
    ...
    | Hd : deref_loc (Tarray _ _ _) _ _ _ _ _ |- _ =>
        ...
        try (match goal with
             | Hlb : load_bitfield (Tarray _ _ _) _ _ _ _ _ _ _ |- _ => inv Hlb
             end)
    end.
    (* the chain brick pins the dst block to cid's (SafeB) block *)
    match goal with
    | Hlv : eval_lvalue _ _ _ _ (Efield _ _ _) ?loc ?lofs ?bf |- _ =>
        assert (Hcrl : chain_root_l (Efield a' fld (Tarray ety n aattr)) = Some cid)
          by (cbn; exact Hcr);
  ...
  (* the engine-callable sc unit (mirror of wc_call_chk_pres): the dst
     chase root's cact membership + chase_inv feed fsc_arg0_safe to discharge
     the arg0_safe gate; sc_scall_pres then preserves carried. *)
  Lemma sc_call_chk_pres :
    forall sc_pids cact optid fid fty al e le0 m0 tr le1 m1 out0,
      (forall g, mem_id g sc_pids = true ->
                 call_pres_ext_sc lp bm NoA MWF SafeB g) ->
      (forall g, mem_id g sc_pids = true -> e ! g = None) ->
      chase_inv cact le0 ->
      sc_call_chk sc_pids cact fid fty al = true ->
      exec_stmt function_entry2 (lp_ge lp) e le0 m0
        (Scall optid (Evar fid fty) al) tr le1 m1 out0 ->
      ...
      carried bm NoA MWF m1 /\ out0 = Out_normal.
  Proof.
    intros sc_pids cact optid fid fty al e le0 m0 tr le1 m1 out0
           Hcp_sc Hnone Hch Hchk Hexec Hc.
    destruct (sc_call_chk_decode _ _ _ _ _ Hchk)
      as (Hfid & ety2 & pattr & tyl & rty & cc & a' & fld & ety & n & aattr
          & rest & cid & Hfty & Hal & Hcr & Hcid).
    subst fty al.
    eapply sc_scall_pres;
      [ exact (Hnone fid Hfid)
      | exact (Hcp_sc fid Hfid)
      | | exact Hexec
      | exact Hc ].
    intros vargs Hvl.
    eapply fsc_arg0_safe;
      [ exact Hcid | exact Hcr | exact Hch | exact Hvl ].
  Qed.
  ...
  (* ================================================================== *)
  (* The chase-store brick, SAFEB-VALUE form: the rhs is a temp whose    *)
  (* value is SafeB-if-a-pointer by provenance (the SafeB-to-SafeB       *)
  (* fused pair arm).  The i32 target kills By_copy; HMWF_chase_safe     *)
  (* absorbs the possibly-pointer store into the SafeB block.            *)
  (* ================================================================== *)
  Lemma chase_assign_safeb_pres :
    forall cact a1 q qty e le m0 tr le' m' out v,
      (match chain_root_l a1 with
       | Some ct => mem_id ct cact
       | None => false
       end) = true ->
      i32_ty (typeof a1) = true ->
      chase_inv cact le ->
      le ! q = Some v ->
      ...
      le' = le /\ out = Out_normal.
  Proof.
    intros cact a1 q qty e le m0 tr le' m' out v Hck Hi32 Hch Hleq Hsp
           Hexec HM HV HS.
    destruct (chain_root_l a1) as [ct|] eqn:Hcr; [ | discriminate Hck ].
    inv Hexec.
    match goal with
    ...
    pose proof (Hch _ Hck _ _ Hlet) as Hsafe.
    pose proof (HSafeNotBm _ Hsafe) as Hneq.
    (* the rhs value is the temp's, SafeB-if-ptr through the cast *)
    match goal with
    | Hev2 : eval_expr _ _ _ _ (Etempvar _ _) _ |- _ =>
        apply eval_expr_Etempvar_val in Hev2;
        rewrite Hleq in Hev2; injection Hev2 as ->
    ...
    - (* By_copy: refuted -- the target's type is i32 (By_value) *)
      match goal with
      | Hac : access_mode (typeof a1) = By_copy |- _ =>
          destruct (typeof a1) as [ | sz sg attr | | | | | | | ];
  ...
  Lemma npsrc_pair_pres :
    forall nids wact cact s1 s2 e le m0 tr1 le1 m1 tr2 le' m' out,
      npsrc_pair_chk nids wact cact s1 s2 = true ->
      e ! interaction._gGlobalTimer = None ->
      (forall b o, le ! mario_actions_airborne._m = Some (Vptr b o) ->
                   b = bm /\ o = Ptrofs.zero) ->
      act_inv wact le -> chase_inv cact le -> nptr_inv nids le ->
      ...
      out = Out_normal.
  Proof.
    intros nids wact0 cact s1 s2 e le m0 tr1 le1 m1 tr2 le' m' out
           Hck He_gt Htat Hact Hch Hnp Hx1 Hx2 HM HV HS.
    ...
    apply andb_prop in Hck as [Hck Htnn].
    ...
    - intros t0 Hmem v0 Hg.
      destruct (Pos.eq_dec t0 t) as [-> | Hne].
      + rewrite Hmem in Htnn. discriminate Htnn.
      + rewrite PTree.gso in Hg by exact Hne. exact (Hnp _ Hmem _ Hg).
  Qed.

  (* ================================================================== *)
  (* The SafeB-to-SafeB fused pair: `t = <chase>[i32]; <chase>[i32] = t`.*)
  (* The loaded value is SafeB-if-a-pointer by the chase load-closure    *)
  (* (HchaseStep; the i32 chunk IS Mptr on ptr32); the store is absorbed *)
  (* by the SafeB-value brick.                                           *)
  (* ================================================================== *)
  Lemma sbpair_pres :
    forall nids wact cact s1 s2 e le m0 tr1 le1 m1 tr2 le' m' out,
      sbpair_chk nids wact cact s1 s2 = true ->
      (forall b o, le ! mario_actions_airborne._m = Some (Vptr b o) ->
                   b = bm /\ o = Ptrofs.zero) ->
      act_inv wact le -> chase_inv cact le -> nptr_inv nids le ->
      exec_stmt function_entry2 (lp_ge lp) e le m0 s1
  ...
  Proof.
    intros nids wact0 cact s1 s2 e le m0 tr1 le1 m1 tr2 le' m' out
           Hck Htat Hact Hch Hnp Hx1 Hx2 HM HV HS.
    unfold sbpair_chk in Hck.
    destruct s1 as [ | a1x a2x | t a | | | | | | | | | | | ];
      try discriminate Hck.
    destruct s2 as [ | a1 a2r | | | | | | | | | | | | ];
      try discriminate Hck.
    destruct a2r as [ | | | | | q qty | | | | | | | | ];
      try discriminate Hck.
    apply andb_prop in Hck as [Hck Hroot].
    apply andb_prop in Hck as [Hck Hi32d].
    apply andb_prop in Hck as [Hck Hsrc].
    apply andb_prop in Hck as [Hck Htnn].
    apply andb_prop in Hck as [Hck Htnc].
    apply andb_prop in Hck as [Hck Htnw].
    ...
    apply Pos.eqb_eq in Hqt. subst q.
    inv Hx1.
    (* the loaded value is SafeB when it is a pointer *)
    match goal with
    | Hev : eval_expr _ _ _ _ a ?vv |- _ =>
        assert (Hsp : forall bb oo, vv = Vptr bb oo -> SafeB bb)
    end.
    { unfold sbsrc_chk in Hsrc.
      destruct a as [ | | | | | | ad ady | | | | | | | ];
        try discriminate Hsrc.
      apply andb_prop in Hsrc as [Hi32s Hcs].
      destruct (chain_root_l (Ederef ad ady)) as [cs|] eqn:Hcrs;
        [ | discriminate Hcs ].
      match goal with
      | Hev : eval_expr _ _ _ _ (Ederef _ _) _ |- _ => inv Hev
      end.
      ...
      | Hlv : eval_lvalue _ _ _ _ (Ederef _ _) _ _ _ |- _ =>
          destruct (chain_root_l_block _ _ _ _ _ _ _ _ _ Hcrs Hlv)
            as (o0 & Hlet)
      end.
      pose proof (Hch _ Hcs _ _ Hlet) as Hsafe_s.
      destruct ady as [ | sz sg attr | | | | | | | ];
        try discriminate Hi32s.
      destruct sz; try discriminate Hi32s.
      match goal with
      | Hdl : deref_loc _ _ _ _ _ _ |- _ => cbn [typeof] in Hdl; inv Hdl
      end.
      - (* By_value: an Mint32 (= Mptr) load from the SafeB block *)
        match goal with
        | Hac : access_mode _ = By_value _ |- _ =>
            cbn [access_mode] in Hac; injection Hac as <-
        end.
        intros bb oo ->.
        assert (HMp : Mptr = Mint32)
          by (assert (Hp64 : Archi.ptr64 = false)
               by (vm_compute; reflexivity);
              unfold Mptr; rewrite Hp64; reflexivity).
        match goal with
        | Hldv : Mem.loadv Mint32 _ _ = Some (Vptr _ _) |- _ =>
            rewrite <- HMp in Hldv;
            exact (HchaseStep _ _ _ _ _ HM Hsafe_s Hldv)
        end.
      - match goal with
        | Hac : access_mode _ = By_reference |- _ =>
            cbn [access_mode] in Hac; discriminate Hac
    ...
    match goal with
    | Hx : exec_stmt _ _ _ _ _ (Sassign _ (Etempvar _ _)) _ _ _ _ |- _ =>
        destruct (chase_assign_safeb_pres _ _ _ _ _ _ _ _ _ _ _ _
                    Hroot Hi32d Hch1 (PTree.gss _ _ _) Hsp Hx HM HV HS)
          as (HV' & HS' & HM' & Hle2 & Hout2)
    end.
    subst.
    refine (conj HV' (conj HS' (conj HM' (conj _ (conj _ (conj Hch1
  ...
  (* the THIRD-position NON-POINTER call brick (the np3 class:
     set_mario_anim_with_accel): the 2nd arg (targetAnimID) is value-
     irrelevant; the 3rd arg comes from a censused nids temp, so its
     value -- and its call-site cast image, by the GENERIC
  ...
  Proof.
    ...
    match goal with
    | Hv : eval_expr _ _ _ _ (Etempvar mario_actions_airborne._m _) _
      |- _ =>
        apply eval_expr_Etempvar_val in Hv; rename Hv into Hv1
    end.
    match goal with
    ...
    (* peel the second arg: value-irrelevant *)
    match goal with
    | Ha : eval_exprlist _ _ _ _ (_ :: _) _ _ |- _ => inv Ha
    end.
    ...
    match goal with
    | Hevf : eval_funcall _ _ _ _ (_ :: _ :: _ :: _) _ _ _ |- _ =>
        destruct (Hcpn _ _ _ _ _ _ _ _ _ Hevf Hres Hmarg Hnpv HN HM HV HS)
          as (HV' & HS' & HM' & HN')
    end.
    refine (conj HV' (conj HS' (conj HM' (conj HN' (conj eq_refl _))))).
    eexists; reflexivity.
  ...
  (* the trivial oc-arm dischargers for the nil oc_pids producers (every
     producer that does NOT pass an out-param census): mem_id _ nil = false. *)
  Lemma hcp_oc_nil : forall fid,
      mem_id fid (@nil ident) = true -> call_pres_ext_oc lp bm NoA MWF SafeB fid.
  Proof. intros fid H. discriminate H. Qed.

  Lemma hub_oc_nil : forall (e : env) g,
  ...
  (* the trivial sc-arm discharger for the nil sc_pids producers (the object
     out-param census; every producer that does NOT call an object writer). *)
  Lemma hcp_sc_nil : forall fid,
      mem_id fid (@nil ident) = true -> call_pres_ext_sc lp bm NoA MWF SafeB fid.
  Proof. intros fid H. discriminate H. Qed.

  Lemma hcp_np3_nil : forall fid,
      mem_id fid nil = true -> call_pres_np3 lp bm NoA MWF fid.
  Proof. intros fid H. discriminate H. Qed.

  Lemma nptr_inv_nil : forall le, nptr_inv nil le.
  Proof. intros le t H. discriminate H. Qed.
  ...
  Lemma wwalk_pres :
    forall (rt : bool) (wact ids wids cact xids sids tids lids oc_pids wc_pids sc_pids nids np3_ids : list ident),
      ...
      forall s e le m0 tr le' m' out,
        ...
        (forall g, mem_id g wc_pids = true -> e ! g = None) ->
        (forall g, mem_id g sc_pids = true -> e ! g = None) ->
        (forall g, mem_id g np3_ids = true -> e ! g = None) ->
        e ! interaction._gGlobalTimer = None ->
        wwalk_chk' lids oc_pids wc_pids sc_pids nids np3_ids rt wact ids wids cact xids sids tids s = true ->
        (forall b o, le ! mario_actions_airborne._m = Some (Vptr b o) ->
                     b = bm /\ o = Ptrofs.zero) ->
        act_inv wact le ->
        chase_inv cact le ->
        ...
        wret_ok rt out.
  Proof.
    intros rt wact ids wids cact xids sids tids lids oc_pids wc_pids sc_pids nids np3_ids
           Hcp Hcpa Hcpx Hcps Hcp3 Hcp_oc Hcp_wc Hcp_sc Hcp_np3
           s e le m0 tr le' m' out Hls Hlocal Hexec.
    induction Hexec; intros He Hubi Hubw Hubx Hubs Hubt Hoc_none Hwc_none Hsc_none Hnp3_none Hubgt
                            Hchk Htat Hact Hch Hnp
                            HN HM HV HS.
    - (* Sskip *)
    ...
    - (* Sassign: window / global / indexed-window / action-store bricks *)
      ...
      destruct Hchk as [Hchk | Hnpc].
      2:{ (* the nids-temp chase store: value non-ptr by provenance *)
          unfold nptr_store_chk in Hnpc.
          destruct (chain_root_l a1) as [ct|] eqn:Hcr;
            [ | discriminate Hnpc ].
          destruct a2 as [ | | | | | q qty | | | | | | | | ];
            try discriminate Hnpc.
          apply andb_prop in Hnpc as [Hctm Hq].
          assert (Hck2 : (match chain_root_l a1 with
                          | Some ct0 => mem_id ct0 cact
                          | None => false end) = true)
            by (rewrite Hcr; exact Hctm).
          match goal with
          | Hev2 : eval_expr _ _ _ _ (Etempvar q qty) _ |- _ =>
              pose proof Hev2 as Hleq;
              apply eval_expr_Etempvar_val in Hleq
          end.
          destruct (chase_assign_value_pres _ a1 q qty _ _ _ _ _ _ _ _
                      Hck2 Hch Hleq (Hnp _ Hq _ Hleq) Hex HM HV HS)
            as (HV' & HS' & HM' & _ & _).
    ...
    - (* Sset: non-_m; an act temp only from an untainted const or copy;
      ...
      cbn [wwalk_chk'] in Hchk.
      apply andb_prop in Hchk as [Hchk Hns].
    ...
    - (* Scall: censused Mario-head call (plain, into an act temp, an
      ...
      apply andb_prop in Hchk1 as [Hchk1 Hnn].
      ...
      { (* the EXTERNAL any-arg censused call: no exprlist analysis *)
        ...
        { intros t0 Hmem b o Hg. destruct optid as [t'|];
          ...
          - rewrite PTree.gso in Hg
            ...
            exact (Hch _ Hmem _ _ Hg).
          - exact (Hch _ Hmem _ _ Hg). }
        { intros t0 Hmem v0 Hg. destruct optid as [t'|];
            cbn [set_opttemp] in Hg.
          - rewrite PTree.gso in Hg
              by (intro EE; rewrite EE in Hmem; rewrite Hmem in Hnn;
                  discriminate Hnn).
            exact (Hnp _ Hmem _ Hg).
          - exact (Hnp _ Hmem _ Hg). } }
      apply orb_true_iff in Hchk.
      destruct Hchk as [Hoc | Hchk].
      { (* the OUT-PARAM (oc) call: fid in oc_pids, last arg = &caller-local;
           the honest refinement of the phantom call_pres_ext.  carried is
           preserved by oc_call_chk_pres (out-param target is a watched-
           disjoint stack block); the act/chase/TAT temp invariants survive
           because the result temp is not in _m / wact / cact. *)
        apply andb_prop in Hoc as [How Hoc].
        assert (Hex : exec_stmt function_entry2 (lp_ge lp) e le m
      ...
      { (* the WINDOW out-param (wc) call: fid in wc_pids, args = &m->pos[i]
        ...
        { intros t0 Hmem v0 Hg. destruct optid as [t'|];
          ...
          - exact (Hnp _ Hmem _ Hg). } }
      apply orb_true_iff in Hchk.
      destruct Hchk as [Hsc | Hchk].
      { (* the OBJECT out-param (sc) call: fid in sc_pids, dst arg = a
           chased object field (vec3f_copy/vec3s_set writing
           m->marioObj->...gfx).  The honest refinement of the phantom
           call_pres_ext for the object writers; carried is preserved by
           sc_call_chk_pres (the dst lands in a SafeB object block, disjoint
           from bm); the act/chase/TAT temp invariants survive because the
           result temp is not in _m / wact / cact. *)
        apply andb_prop in Hsc as [How Hsc].
        assert (Hex : exec_stmt function_entry2 (lp_ge lp) e le m
                        (Scall optid (Evar cid fty) al)
                        t (set_opttemp optid vres le) m' Out_normal)
          by (econstructor; eauto).
        destruct (sc_call_chk_pres sc_pids cact optid cid fty al e le m t
                    (set_opttemp optid vres le) m' Out_normal
                    Hcp_sc Hsc_none Hch Hsc Hex
                    (conj HV (conj HS (conj HM HN))))
          as [(HV' & HS' & HM' & HN') _].
        refine (conj HV' (conj HS' (conj HM' (conj HN'
                 (conj _ (conj _ (conj _ (conj _ I)))))))).
      ...
      + (* a result-carrying call *)
        ...
        * (* plain censused call, or the smact-const leaf pattern --
          ...
          destruct Hbr as [Hbr | Hnp3c].
          2:{ (* the np3 leaf call: non-ptr 3rd arg from a censused
                 nids temp (set_mario_anim_with_accel) *)
            unfold np3_call_chk in Hnp3c.
            apply andb_prop in Hnp3c as [Hf3 Hnp3c].
            destruct tys as [| ty2 tys2]; try discriminate Hnp3c.
            ...
            { intros t0 Hmem b o Hg. cbn [set_opttemp] in Hg.
              rewrite PTree.gso in Hg
                by (intro EE; rewrite EE in Hmem; rewrite Hmem in Hnc;
                    discriminate Hnc).
              exact (Hch _ Hmem _ _ Hg). }
            { intros t0 Hmem v0 Hg. cbn [set_opttemp] in Hg.
              rewrite PTree.gso in Hg
                by (intro EE; rewrite EE in Hmem; rewrite Hmem in Hnn;
                    discriminate Hnn).
              exact (Hnp _ Hmem _ Hg). } }
      ...
      + (* a result-less call: plain censused, or smact-const *)
        apply orb_true_iff in Hbr.
        destruct Hbr as [Hbr | Hnp3c].
        2:{ (* the np3 leaf call, result-less *)
          unfold np3_call_chk in Hnp3c.
          apply andb_prop in Hnp3c as [Hf3 Hnp3c].
          destruct tys as [| ty2 tys2]; try discriminate Hnp3c.
          destruct tys2 as [| ty3 tys3]; try discriminate Hnp3c.
    ...
    - (* Sseq_1 *)
      cbn [wwalk_chk'] in Hchk.
      apply orb_true_iff in Hchk. destruct Hchk as [Hpair2 | Hchk].
      { (* one of the two fused pairs *)
        apply orb_true_iff in Hpair2. destruct Hpair2 as [Hpair | Hsbp].
        - (* the non-pointer-source pair *)
          match goal with
          | Hx1 : exec_stmt _ _ _ _ _ s1 _ _ _ Out_normal,
            Hx2 : exec_stmt _ _ _ _ _ s2 _ _ _ _ |- _ =>
              destruct (npsrc_pair_pres _ _ _ _ _ _ _ _ _ _ _ _ _ _ _
                          Hpair Hubgt Htat Hact Hch Hnp Hx1 Hx2 HM HV HS)
                as (HV' & HS' & HM' & Htat' & Hact' & Hch' & Hnp' & ->)
          end.
          exact (conj HV' (conj HS' (conj HM' (conj (HNoA_of_MWF _ HM')
                   (conj Htat' (conj Hact' (conj Hch' (conj Hnp' I)))))))).
        - (* the SafeB-to-SafeB pair *)
          match goal with
          | Hx1 : exec_stmt _ _ _ _ _ s1 _ _ _ Out_normal,
            Hx2 : exec_stmt _ _ _ _ _ s2 _ _ _ _ |- _ =>
              destruct (sbpair_pres _ _ _ _ _ _ _ _ _ _ _ _ _ _ _
                          Hsbp Htat Hact Hch Hnp Hx1 Hx2 HM HV HS)
                as (HV' & HS' & HM' & Htat' & Hact' & Hch' & Hnp' & ->)
          end.
          exact (conj HV' (conj HS' (conj HM' (conj (HNoA_of_MWF _ HM')
                   (conj Htat' (conj Hact' (conj Hch' (conj Hnp' I)))))))). }
      apply andb_prop in Hchk as [H1 H2].
      destruct (IHHexec1 Hlocal He Hubi Hubw Hubx Hubs Hubt Hoc_none Hwc_none Hsc_none Hnp3_none Hubgt H1 Htat Hact Hch Hnp HN HM HV HS)
        as (HV1 & HS1 & HM1 & HN1 & Htat1 & Hact1 & Hch1 & Hnp1 & _).
      exact (IHHexec2 Hlocal He Hubi Hubw Hubx Hubs Hubt Hoc_none Hwc_none Hsc_none Hnp3_none Hubgt H2 Htat1 Hact1 Hch1 Hnp1 HN1 HM1 HV1 HS1).
    - (* Sseq_2 *)
      cbn [wwalk_chk'] in Hchk.
      apply orb_true_iff in Hchk. destruct Hchk as [Hpair2 | Hchk].
      { (* either pair's Sset head cannot exit abnormally *)
        exfalso.
        apply orb_true_iff in Hpair2;
          destruct Hpair2 as [Hpair | Hpair];
          [ unfold npsrc_pair_chk in Hpair | unfold sbpair_chk in Hpair ];
      ...
      exact (IHHexec Hlocal He Hubi Hubw Hubx Hubs Hubt Hoc_none Hwc_none Hsc_none Hnp3_none Hubgt H1 Htat Hact Hch Hnp HN HM HV HS).
    - (* Sifthenelse *)
    ...
    - (* Sreturn (Some a): the censused return (only when rt = true) *)
      cbn [wwalk_chk'] in Hchk.
      refine (conj HV (conj HS (conj HM (conj HN
               (conj Htat (conj Hact (conj Hch (conj Hnp _)))))))).
    ...
    - (* Sloop stop1 *)
      ...
      destruct (IHHexec Hlocal He Hubi Hubw Hubx Hubs Hubt Hoc_none Hwc_none Hsc_none Hnp3_none Hubgt H1 Htat Hact Hch Hnp HN HM HV HS)
        as (HV1 & HS1 & HM1 & HN1 & Htat1 & Hact1 & Hch1 & Hnp1 & Hret1).
      refine (conj HV1 (conj HS1 (conj HM1 (conj HN1
               (conj Htat1 (conj Hact1 (conj Hch1 (conj Hnp1 _)))))))).
      match goal with
    ...
    - (* Sloop stop2 *)
      ...
      destruct (IHHexec1 Hlocal He Hubi Hubw Hubx Hubs Hubt Hoc_none Hwc_none Hsc_none Hnp3_none Hubgt H1 Htat Hact Hch Hnp HN HM HV HS)
        as (HV1 & HS1 & HM1 & HN1 & Htat1 & Hact1 & Hch1 & Hnp1 & _).
      destruct (IHHexec2 Hlocal He Hubi Hubw Hubx Hubs Hubt Hoc_none Hwc_none Hsc_none Hnp3_none Hubgt H2 Htat1 Hact1 Hch1 Hnp1 HN1 HM1 HV1 HS1)
        as (HV2 & HS2 & HM2 & HN2 & Htat2 & Hact2 & Hch2 & Hnp2 & Hret2).
      refine (conj HV2 (conj HS2 (conj HM2 (conj HN2
               (conj Htat2 (conj Hact2 (conj Hch2 (conj Hnp2 _)))))))).
      match goal with
      | Hbr : out_break_or_return _ _ |- _ => inv Hbr
      end.
      + exact I.
    ...
    - (* Sloop loop *)
      ...
      destruct (IHHexec1 Hlocal He Hubi Hubw Hubx Hubs Hubt Hoc_none Hwc_none Hsc_none Hnp3_none Hubgt H1 Htat Hact Hch Hnp HN HM HV HS)
        as (HV1 & HS1 & HM1 & HN1 & Htat1 & Hact1 & Hch1 & Hnp1 & _).
      destruct (IHHexec2 Hlocal He Hubi Hubw Hubx Hubs Hubt Hoc_none Hwc_none Hsc_none Hnp3_none Hubgt H2 Htat1 Hact1 Hch1 Hnp1 HN1 HM1 HV1 HS1)
        as (HV2 & HS2 & HM2 & HN2 & Htat2 & Hact2 & Hch2 & Hnp2 & _).
      apply IHHexec3; try assumption;
        cbn [wwalk_chk']; exact Hchk2.
    - (* Sswitch *)
      cbn [wwalk_chk'] in Hchk.
      destruct (IHHexec Hlocal He Hubi Hubw Hubx Hubs Hubt Hoc_none Hwc_none Hsc_none Hnp3_none Hubgt
                  (wwalk_chk_select' _ _ _ _ _ _ _ _ _ _ _ _ _ _ n _ Hchk)
  ...
  Proof.
    ...
    destruct (wwalk_pres rt wact ids wids cact xids sids tids
                nil nil nil nil nil nil
                Hcp Hcpa Hcpx Hcps Hcp3 hcp_oc_nil hcp_wc_nil hcp_sc_nil
                hcp_np3_nil s e le m0 tr le' m' out
                hls_nil (hlocal_nil e) Hexec
                He Hubi Hubw Hubx Hubs Hubt (hub_oc_nil e) (hub_oc_nil e)
                (hub_oc_nil e) (hub_oc_nil e) Hubgt
                Hchk Htat Hact Hch (nptr_inv_nil le) HN HM HV HS)
      as (HV' & HS' & HM' & HN' & Htat' & Hact' & Hch' & _ & Hret').
    exact (conj HV' (conj HS' (conj HM' (conj HN' (conj Htat'
             (conj Hact' (conj Hch' Hret'))))))).
  Qed.
...
(* the np3 (set_mario_anim_with_accel) parameter shape: Mario's pointer,
   the target animation id, and the RAW scalar accel (the np3 row's
   gated third argument). *)
Definition np3_params : list (ident * type) :=
  (mario_actions_airborne._m, tyMSp)
    :: (mario._targetAnimID, tint)
    :: (mario._accel, tint) :: nil.
...
Section ActWriterRows.
  ...
  (* ---- the LEAF analogue of call_pres_of_lwalk: a dispatched act handler
     (body_pres, not call_pres) whose only oddity is a stack-local array it
     NEVER stores through (dead padding, e.g. act_squished's `filler[4]`),
     so the unprimed walk (lids=nil) suffices but fn_vars is non-nil.  Same
     entry/exit frame as call_pres_of_lwalk (alloc_variables_carried over the
     fresh local blocks, free_list_carried_bm at exit), but takes `Internal f`
     directly (no resolve_pin_fd) and derives marg_ok from the funcall's
     marg_exempt-conditional premise -- exactly the body_pres_of_wwalk diff. *)
  Lemma body_pres_of_lwalk :
    forall (f : Clight.function) (ids wids xids sids : list ident),
      match fn_params f with
      | (i, ty) :: ps =>
          Pos.eqb i mario_actions_airborne._m
          && proj_sumbool (type_eq ty tyMSp)
  ...
  Proof.
    intros f ids wids xids sids Hps
           Hdg Hdi Hdw Hdx Hds Hdgt
           Hcp Hcpa Hcpx Hcps Hchk
           m0 vargs0 t0 mF vres0 Hmargf Hevf HN HM HV HS.
    inv Hevf.
    match goal with He : function_entry2 _ _ _ _ _ _ _ |- _ =>
      rename He into Hentry end.
    match goal with Hx : exec_stmt _ _ _ _ _ _ _ _ _ _ |- _ =>
  ...
  (* ---- body_pres_of_lwalk WITH a chase census (cact): a function with
     stack-local vars that are NOT stored into directly (lids = nil) -- the
     local is alloc'd and freed but written only via an external out-param
     (e.g. jumbo_star_cutscene_flying's targetPos, written by anim_spline_poll)
     -- AND whose body has chase stores through cact temps (marioObj /
     marioBodyState).  Identical entry/exit frame to body_pres_of_lwalk
     (alloc_variables_carried + free_list_carried_bm), but the body walk
     carries a real cact: the chase_inv at entry is vacuous because every cact
     temp is a NON-param (Hnpc) hence Vundef post-bind.  Zero new section
     state; consumed by flying / end_peach's end_obj_set_visual_pos. *)
  Lemma body_pres_of_lwalk_cact :
    forall (f : Clight.function) (ids wids cact xids sids : list ident),
      match fn_params f with
      | (i, ty) :: ps =>
          Pos.eqb i mario_actions_airborne._m
          && proj_sumbool (type_eq ty tyMSp)
      ...
      body_pres lp NoA MWF bm f.
  Proof.
    intros f ids wids cact xids sids Hps
           Hdg Hdi Hdw Hdx Hds Hdgt Hnpc
           Hcp Hcpa Hcpx Hcps Hchk
           m0 vargs0 t0 mF vres0 Hmargf Hevf HN HM HV HS.
    inv Hevf.
    match goal with He : function_entry2 _ _ _ _ _ _ _ |- _ =>
  ...
  Lemma call_pres_of_lwalk2 :
    forall (TU : Clight.program) (fid : ident) (f : Clight.function)
           (ids wids xids sids lids oc_pids wc_pids sc_pids : list ident),
      linkorder TU lp ->
      (prog_defmap TU) ! fid = Some (Gfun (Internal f)) ->
      match fn_params f with
      | (i, ty) :: ps =>
      ...
      (forall fid', mem_id fid' sc_pids = true ->
                    call_pres_ext_sc lp bm NoA MWF SafeB fid') ->
      wwalk_chk' lids oc_pids wc_pids sc_pids nil nil false nil ids wids nil xids sids nil (fn_body f) = true ->
      call_pres lp bm NoA MWF fid.
  Proof.
    intros TU fid f ids wids xids sids lids oc_pids wc_pids sc_pids LOtu Hdm Hps
    ...
    assert (Hub_s : forall g, mem_id g sids = true -> eloc ! g = None)
      by (intros g Hg;
          rewrite (alloc_variables_unbound (lp_ge lp) m0 (fn_vars f)
                     empty_env _ _ Halloc g (Hds g Hg)); apply PTree.gempty).
    assert (Hub_oc : forall g, mem_id g oc_pids = true -> eloc ! g = None)
      by (intros g Hg;
          rewrite (alloc_variables_unbound (lp_ge lp) m0 (fn_vars f)
    ...
    assert (Hub_sc : forall g, mem_id g sc_pids = true -> eloc ! g = None)
      by (intros g Hg;
          rewrite (alloc_variables_unbound (lp_ge lp) m0 (fn_vars f)
  ...
  (* ---- TIER-2 producer WITH a chase-temp census (cact != nil): a function
     that BOTH has stack-local out-params/arrays (lids) AND stores through
     chased pointers held in temps (cact), e.g. set_pole_position (the shared
     pole helper): _floor/_ceil out-param locals + m->marioObj->gfx.pos[i]
     chase stores through _t'NN temps.  Identical to call_pres_of_lwalk2 but
     the body walk carries a real `cact`; the chase_inv entry invariant is
     discharged exactly as in call_pres_of_wwalk_cact: every cact temp is a
     non-param (Hnpc), so at entry it is Vundef (create_undef_temps), never a
  ...
  (* ---- the call_pres_act producer EXTENDED to a FOURTH param (the
     hurt/damage writer class: hurt_and_set_mario_action(m, action,
     actionArg, hurtCounter)).  Same SECOND-position-action shape and i32
     return; the 4th param `harg` is bound but never seeded into wact/cact.
     Reusable for the damage handlers across all families. *)
  Lemma call_pres_act_of_wwalk4 :
    forall (TU : Clight.program) (fid : ident) (f : Clight.function)
           (wact ids wids cact xids sids : list ident)
           (harg : ident) (hty : type),
      linkorder TU lp ->
      ...
      call_pres_act lp bm NoA MWF fid.
  Proof.
    intros TU fid f wact ids wids cact xids sids harg hty LOtu Hdm Hvars Hparams
           Hret Hharg_m Hwa Hwm Hwarg Hwharg Hcm Hca Hcarg Hcharg
           Hcp Hcpa Hcpx Hcps Hchk
           fd m0 v0 aval rest t0 m1 vres0 Hevf Hres Hmarg Hu HN HM HV HS.
    pose proof (resolve_pin_fd lp _ _ _ _ LOtu Hdm Hres) as ->.
    inv Hevf.
  ...
  (* ---- the call_pres_act producer for a TWO-param writer: Mario's
     pointer FIRST, the untainted action SECOND (and nothing after),
     i32 return.  The action param ident is generic (`aid` -- e.g.
     check_fall_damage's `hardFallAction`), so it works for any
     2-arg action-passing writer whose action seeds the wact census.
     Same SECOND-position-action shape as call_pres_act_of_wwalk; the
     only difference is exactly two bound params (rest must be nil). *)
  Lemma call_pres_act_of_wwalk2 :
    forall (TU : Clight.program) (fid : ident) (f : Clight.function)
           (aid : ident) (aty : type)
           (wact ids wids cact xids sids : list ident),
      linkorder TU lp ->
      (prog_defmap TU) ! fid = Some (Gfun (Internal f)) ->
      fn_vars f = nil ->
      fn_params f = (mario_actions_airborne._m, tyMSp) :: (aid, aty) :: nil ->
      i32_ty (fn_return f) = true ->
      aid <> mario_actions_airborne._m ->
      mem_id aid wact = true ->
      mem_id mario_actions_airborne._m wact = false ->
      ...
      call_pres_act lp bm NoA MWF fid.
  Proof.
    intros TU fid f aid aty wact ids wids cact xids sids LOtu Hdm Hvars Hparams
           Hret Haid_m Hwa Hwm Hcm Hca Hcp Hcpa Hcpx Hcps Hchk
           fd m0 v0 aval rest t0 m1 vres0 Hevf Hres Hmarg Hu HN HM HV HS.
    pose proof (resolve_pin_fd lp _ _ _ _ LOtu Hdm Hres) as ->.
    inv Hevf.
    match goal with
    ...
    assert (Hact0 : act_inv wact
               (PTree.set aid aval
                  (PTree.set mario_actions_airborne._m v0 base))).
    { intros t' Hmem' x Hg'.
      destruct (Pos.eq_dec t' aid) as [-> | Hne1].
      { rewrite PTree.gss in Hg'. injection Hg' as <-. exact Hu. }
      rewrite PTree.gso in Hg' by exact Hne1.
      destruct (Pos.eq_dec t' mario_actions_airborne._m) as [-> | Hne3].
      { rewrite Hmem' in Hwm. discriminate Hwm. }
  ...
  (* ---- the funcall->body entry for the THIRD-position NON-POINTER
     ...
     seeding the nids census. *)
  Lemma call_pres_np3_of_wwalk :
    forall (TU : Clight.program) (fid : ident) (f : Clight.function)
           (ids wids cact xids sids : list ident),
      linkorder TU lp ->
      (prog_defmap TU) ! fid = Some (Gfun (Internal f)) ->
      fn_vars f = nil ->
      fn_params f = np3_params ->
      mem_id mario_actions_airborne._m cact = false ->
      mem_id mario._targetAnimID cact = false ->
      mem_id mario._accel cact = false ->
      (forall fid', mem_id fid' ids = true ->
                    call_pres lp bm NoA MWF fid') ->
      (forall fid', mem_id fid' wids = true ->
                    call_pres_act lp bm NoA MWF fid') ->
      ...
      call_pres_np3 lp bm NoA MWF fid.
  Proof.
    intros TU fid f ids wids cact xids sids LOtu Hdm Hvars Hparams
           Hcm Hcanim Hcacc Hcp Hcpa Hcpx Hcps Hchk
    ...
    rewrite Hparams in Hbind. unfold np3_params in Hbind.
    cbn [bind_parameter_temps] in Hbind.
    destruct rest as [| vx restx ];
      cbn [bind_parameter_temps] in Hbind; [ | discriminate Hbind ].
    injection Hbind as <-.
    ...
    (* the entry env facts *)
    assert (Htat0 : forall b o,
               (PTree.set mario._accel v2
                  (PTree.set mario._targetAnimID v1
                     (PTree.set mario_actions_airborne._m v0 base)))
                 ! mario_actions_airborne._m = Some (Vptr b o) ->
               b = bm /\ o = Ptrofs.zero).
    { intros b o Hg.
    ...
    assert (Hch0 : chase_inv SafeB cact
               (PTree.set mario._accel v2
                  (PTree.set mario._targetAnimID v1
                     (PTree.set mario_actions_airborne._m v0 base)))).
    { intros t' Hmem' b o Hg'.
      destruct (Pos.eq_dec t' mario._accel) as [-> | Hne1].
      { rewrite Hmem' in Hcacc. discriminate Hcacc. }
      rewrite PTree.gso in Hg' by exact Hne1.
      destruct (Pos.eq_dec t' mario._targetAnimID) as [-> | Hne2].
      { rewrite Hmem' in Hcanim. discriminate Hcanim. }
      rewrite PTree.gso in Hg' by exact Hne2.
      destruct (Pos.eq_dec t' mario_actions_airborne._m) as [-> | Hne3].
      { rewrite Hmem' in Hcm. discriminate Hcm. }
      ...
      pose proof (create_undef_temps_val _ _ _ Hg') as EE.
      discriminate EE. }
    assert (Hnp0 : nptr_inv (mario._accel :: nil)
               (PTree.set mario._accel v2
                  (PTree.set mario._targetAnimID v1
                     (PTree.set mario_actions_airborne._m v0 base)))).
    { intros t' Hmem' v' Hg'.
  ...
  (* ---- the body entry for a Mario-head leaf that THREADS an nids
     census (the act_climbing_pole shape): the nids temps are NON-PARAMS
     (Vundef at entry -> the invariant holds trivially), re-established
     only from float-to-int casts; np3_ids names the gated callees. ---- *)
  Lemma body_pres_of_wwalk_nids :
    forall (f : Clight.function)
           (ids wids cact xids sids tids nids np3_ids : list ident),
      fn_vars f = nil ->
      match fn_params f with
      ...
      body_pres lp NoA MWF bm f.
  Proof.
    intros f ids wids cact xids sids tids nids np3_ids Hvars Hps Hnpc Hnpn
           Hcp Hcpa Hcpx Hcps Hcp3t Hcpn3 Hchk
           m0 vargs0 t0 m1 vres0 Hmargf Hevf HN HM HV HS.
    inv Hevf.
    match goal with
