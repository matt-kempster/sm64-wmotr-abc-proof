...
Definition mov_walked_ids : list ident :=
  mario_actions_moving._check_common_moving_cancels
    ...
    (* act_dive_slide: csa-hybrid recognizer-walk (Scall-None common_slide_action
       const-action tail; object externals via mguo_row/mcog_row) *)
    :: mario_actions_moving._act_dive_slide
    (* act_riding_shell_ground: gwspf-hybrid recognizer-walk (update_shell_speed
       water-pseudo-floor patch) + tilt_body_ground_shell chase helper *)
    :: mario_actions_moving._act_riding_shell_ground
    (* act_walking: THE LAST moving leaf -- A-gate kill (set_jump_from_landing)
       + vec3f_copy(_startPos, m->pos) wol special + 12-ids generic census *)
    :: mario_actions_moving._act_walking :: nil.
...
Proof. vm_compute. reflexivity. Qed.
  ...
  (* ================================================================== *)
  (* act_dive_slide: the csa-HYBRID recognizer-walk (Scall-None special   *)
  (* site = common_slide_action with CONST args).  Engine census:         *)
  (*   dive_wact=[t'1] (the set_mario_action arg temp, set from an         *)
  (*     untainted const via Ecast)                                        *)
  (*   dive_ids=[update_sliding, is_anim_at_end, mario_set_forward_vel,    *)
  (*     mario_check_object_grab, mario_grab_used_object] (all call_pres:   *)
  (*     the latter two are EF_external in mario_actions_moving.prog but    *)
  (*     Internal in interaction.prog, walked in ObjectLeafSurface)        *)
  (*   dive_xids=[play_mario_landing_sound_once] (pure-audio external)     *)
  (*   dive_sids=[set_mario_action] (Hsmact; smact_call_chk covers BOTH    *)
  (*     the temp-arg `set_mario_action(m, t'1, 0)` and the const-arg      *)
  (*     `set_mario_action(m, 902, 0)` sites)                              *)
  (*   dive_cact=[t'8] (the m->marioBodyState->grabPos = 1 chase store)    *)
  (* The trailing common_slide_action(m, 902, 16779404, 136) has CONST     *)
  (* action args (both untainted) -> the special arm + csa_const_site_pres.*)
  (* ================================================================== *)

  (* the two object-grab callees: Internal in interaction.prog, resolved
     in lp; their walks live in ObjectLeafSurface. *)
  ...
  (* ---- the dive recognizer (engine || csa-const special arm) ---- *)
  ...
  Definition csa_const_site_chk (fid : ident) (fty : type) (al : list expr) : bool :=
    Pos.eqb fid M._common_slide_action
    && proj_sumbool
         (type_eq fty (Tfunction (tyMSp :: tuint :: tuint :: tint :: nil) tvoid cc_default))
    && match al with
       | Etempvar p pty :: Econst_int ea ety :: Econst_int aa aty
         :: Econst_int ana anty :: nil =>
           Pos.eqb p M._m && proj_sumbool (type_eq pty tyMSp)
           && proj_sumbool (type_eq ety tint) && proj_sumbool (type_eq aty tint)
           && proj_sumbool (type_eq anty tint)
           && wact_const ea && wact_const aa
       | _ => false
       end.

  Fixpoint dive_chk (s : statement) : bool :=
    wwalk_chk false dive_wact dive_ids nil dive_cact dive_xids dive_sids nil s
    || match s with
       | Ssequence s1 s2 => dive_chk s1 && dive_chk s2
       | Sifthenelse _ s1 s2 => dive_chk s1 && dive_chk s2
       | Sswitch _ sl => dive_chk_ls sl
       | Scall None (Evar fid fty) al => csa_const_site_chk fid fty al
       | _ => false
       end
  with dive_chk_ls (sl : labeled_statements) : bool :=
    match sl with
    | LSnil => true
    | LScons _ s sl' => dive_chk s && dive_chk_ls sl'
    end.

  Lemma dive_chk_ls_seq : forall sl,
      dive_chk_ls sl = true -> dive_chk (seq_of_labeled_statement sl) = true.
  Proof.
    induction sl as [| o s sl0 IH]; intros H.
    - reflexivity.
    - cbn [seq_of_labeled_statement]. cbn [dive_chk_ls] in H.
      apply andb_prop in H as [H1 H2]. cbn [dive_chk].
      apply orb_true_iff. right. rewrite H1, (IH H2). reflexivity.
  Qed.
  Lemma dive_chk_ls_case : forall n sl sl',
      dive_chk_ls sl = true -> select_switch_case n sl = Some sl' ->
      dive_chk_ls sl' = true.
  Proof.
    intros n sl; induction sl as [| o s sl0 IH]; intros sl' H Hsel.
    - discriminate Hsel.
    - cbn [dive_chk_ls] in H. apply andb_prop in H as [H1 H2].
      destruct o as [c|]; cbn [select_switch_case] in Hsel.
      + destruct (zeq c n).
        * injection Hsel as <-. cbn [dive_chk_ls]. rewrite H1, H2. reflexivity.
        * exact (IH sl' H2 Hsel).
      + exact (IH sl' H2 Hsel).
  Qed.
  Lemma dive_chk_ls_default : forall sl,
      dive_chk_ls sl = true -> dive_chk_ls (select_switch_default sl) = true.
  Proof.
    induction sl as [| o s sl0 IH]; intros H.
    - exact H.
    - cbn [dive_chk_ls] in H. apply andb_prop in H as [H1 H2].
      destruct o as [c|]; cbn [select_switch_default].
      + exact (IH H2).
      + cbn [dive_chk_ls]. rewrite H1, H2. reflexivity.
  Qed.
  Lemma dive_chk_select : forall n sl,
      dive_chk_ls sl = true ->
      dive_chk (seq_of_labeled_statement (select_switch n sl)) = true.
  Proof.
    intros n sl H. apply dive_chk_ls_seq. unfold select_switch.
    destruct (select_switch_case n sl) eqn:E.
    - exact (dive_chk_ls_case _ _ _ H E).
    - exact (dive_chk_ls_default _ H).
  Qed.
  Lemma dive_chk_scall_inv : forall optid a al,
      dive_chk (Scall optid a al) = true ->
      wwalk_chk false dive_wact dive_ids nil dive_cact dive_xids dive_sids nil
        (Scall optid a al) = true
      \/ (optid = None /\ exists fid fty,
            a = Evar fid fty /\ csa_const_site_chk fid fty al = true).
  Proof.
    intros optid a al H. cbn [dive_chk] in H.
    apply orb_true_iff in H as [Hg | Hsp]; [ left; exact Hg | right ].
    destruct optid as [t'|]; [ discriminate Hsp | ].
    destruct a as [ i0 t0 | f0 t0 | f0 t0 | i0 t0 | fid fty | id0 t0
                  | a0 t0 | a0 t0 | op a0 t0 | op a1 a2 t0 | a0 t0
                  | a0 f0 t0 | t1 t0 | t1 t0 ]; try discriminate Hsp.
    split; [ reflexivity | ]. exists fid, fty.
    split; [ reflexivity | exact Hsp ].
  Qed.
  Lemma csa_const_site_chk_shape : forall fid fty al,
      csa_const_site_chk fid fty al = true ->
      fid = M._common_slide_action
      /\ fty = Tfunction (tyMSp :: tuint :: tuint :: tint :: nil) tvoid cc_default
      /\ exists ea aa ana,
           al = (Etempvar M._m tyMSp :: Econst_int ea tint :: Econst_int aa tint
                 :: Econst_int ana tint :: nil)
           /\ wact_const ea = true /\ wact_const aa = true.
  Proof.
    intros fid fty al H. unfold csa_const_site_chk in H.
    apply andb_prop in H as [H Hal].
    apply andb_prop in H as [Hfid Hty].
    apply Pos.eqb_eq in Hfid.
    destruct (type_eq fty (Tfunction (tyMSp :: tuint :: tuint :: tint :: nil) tvoid cc_default))
      as [Efty | Nfty]; [ subst fty | discriminate Hty ]. clear Hty.
    destruct al as [| e0 al0]; [ discriminate Hal | ].
    destruct e0 as [ i t0 | f t0 | f t0 | i t0 | id t0 | p pty
                   | a0 t0 | a0 t0 | o a0 t0 | o a1 a2 t0 | a0 t0
                   | a0 f0 t0 | ty t0 | ty t0 ]; try discriminate Hal.
    destruct al0 as [| e1 al1]; [ discriminate Hal | ].
    destruct e1 as [ ea ety | f t0 | f t0 | i t0 | id t0 | p0 pt0
                   | a0 t0 | a0 t0 | o a0 t0 | o a1 a2 t0 | a0 t0
                   | a0 f0 t0 | ty t0 | ty t0 ]; try discriminate Hal.
    destruct al1 as [| e2 al2]; [ discriminate Hal | ].
    destruct e2 as [ aa aty | f t0 | f t0 | i t0 | id t0 | p0 pt0
                   | a0 t0 | a0 t0 | o a0 t0 | o a1 a2 t0 | a0 t0
                   | a0 f0 t0 | ty t0 | ty t0 ]; try discriminate Hal.
    destruct al2 as [| e3 al3]; [ discriminate Hal | ].
    destruct e3 as [ ana anty | f t0 | f t0 | i t0 | id t0 | p0 pt0
                   | a0 t0 | a0 t0 | o a0 t0 | o a1 a2 t0 | a0 t0
                   | a0 f0 t0 | ty t0 | ty t0 ]; try discriminate Hal.
    destruct al3 as [|]; [ | discriminate Hal ].
    apply andb_prop in Hal as [Hal Ha].
    apply andb_prop in Hal as [Hal He].
    apply andb_prop in Hal as [Hal Hanty].
    apply andb_prop in Hal as [Hal Haty].
    apply andb_prop in Hal as [Hal Hety].
    apply andb_prop in Hal as [Hpm Hpty].
    apply Pos.eqb_eq in Hpm; subst p.
    destruct (type_eq pty tyMSp) as [-> | ?]; [ | discriminate Hpty ].
    destruct (type_eq ety tint) as [-> | ?]; [ | discriminate Hety ].
    destruct (type_eq aty tint) as [-> | ?]; [ | discriminate Haty ].
    destruct (type_eq anty tint) as [-> | ?]; [ | discriminate Hanty ].
    split; [ exact Hfid | ]. split; [ reflexivity | ].
    exists ea, aa, ana. split; [ reflexivity | ].
    split; [ exact He | exact Ha ].
  Qed.

  (* ---- the generic-env const common_slide_action capture ---- *)
  Lemma csa_const_site_pres :
    forall e ea aa ana le m tr le' m' out,
      e ! M._common_slide_action = None ->
      wact_const ea = true -> wact_const aa = true ->
      (forall b o, le ! M._m = Some (Vptr b o) -> b = bm /\ o = Ptrofs.zero) ->
      exec_stmt function_entry2 (lp_ge lp) e le m
        (Scall None
           (Evar M._common_slide_action
              (Tfunction (tyMSp :: tuint :: tuint :: tint :: nil) tvoid cc_default))
           (Etempvar M._m tyMSp
            :: Econst_int ea tint :: Econst_int aa tint :: Econst_int ana tint :: nil))
        tr le' m' out ->
      NoA m -> MWF m -> Mem.valid_block m bm -> action_sat not_tainted m bm ->
      Mem.valid_block m' bm /\ action_sat not_tainted m' bm /\ MWF m' /\ NoA m'
      /\ out = Out_normal
      /\ (forall b o, le' ! M._m = Some (Vptr b o) -> b = bm /\ o = Ptrofs.zero).
  Proof.
    intros e ea aa ana le m tr le' m' out He Hea Haa Htat Hexec HN HM HV HS.
    inv Hexec.
    match goal with Hcf : classify_fun _ = _ |- _ => cbn in Hcf; inv Hcf end.
    match goal with Hv : eval_expr _ _ _ _ (Evar _ _) _ |- _ =>
      destruct (eval_Evar_funct lp _ _ _ _ _ _ _ _ He Hv) as (fb & Hsym & ->) end.
    ...
    match goal with Hv : eval_expr _ _ _ _ (Econst_int _ _) _ |- _ =>
      inv Hv; try (match goal with Hlv : eval_lvalue _ _ _ _ (Econst_int _ _) _ _ _ |- _ =>
                     inv Hlv end) end.
    match goal with Hc : sem_cast (Vint ea) _ _ _ = Some _ |- _ =>
      cbn in Hc; injection Hc as <- end.
    match goal with Hel : eval_exprlist _ _ _ _ (_ :: _ :: nil) _ _ |- _ => inv Hel end.
    match goal with Hv : eval_expr _ _ _ _ (Econst_int _ _) _ |- _ =>
      inv Hv; try (match goal with Hlv : eval_lvalue _ _ _ _ (Econst_int _ _) _ _ _ |- _ =>
                     inv Hlv end) end.
    match goal with Hc : sem_cast (Vint aa) _ _ _ = Some _ |- _ =>
      cbn in Hc; injection Hc as <- end.
    match goal with Hel : eval_exprlist _ _ _ _ (_ :: nil) _ _ |- _ => inv Hel end.
    match goal with Hel : eval_exprlist _ _ _ _ nil _ _ |- _ => inv Hel end.
    match goal with Hv1 : le ! M._m = Some ?v1 |- _ =>
    ...
    | Hevf : eval_funcall _ _ _ _ _ _ _ _ |- _ =>
        destruct (csa_funcall_pres _ _ _ _ _ _ _ _ _ Hres Hevf Htat1
                    (wact_const_sound _ Hea) (wact_const_sound _ Haa) HN HM HV HS)
          as (HV' & HS' & HM' & HN')
    end.
    refine (conj HV' (conj HS' (conj HM' (conj HN' (conj eq_refl _))))).
    intros b o Hg. cbn [set_opttemp] in Hg. exact (Htat b o Hg).
  ...
  (* ---- dive census rows ---- *)
  Lemma dive_ids_rows : forall fid, mem_id fid dive_ids = true ->
      call_pres lp bm NoA MWF fid.
  Proof.
    intros fid H. unfold dive_ids in H. cbn [mem_id existsb] in H.
    apply orb_true_iff in H as [Hm | H];
      [ apply Pos.eqb_eq in Hm; subst fid; exact mov_usl_row | ].
    apply orb_true_iff in H as [Hm | H];
      [ apply Pos.eqb_eq in Hm; subst fid; exact mov_iae_row | ].
  ...
  Lemma dive_sids_rows : forall fid, mem_id fid dive_sids = true ->
      call_pres_act lp bm NoA MWF fid.
  Proof.
    intros fid H. unfold dive_sids in H. cbn [mem_id existsb] in H.
    apply orb_true_iff in H as [Hm | H];
      [ apply Pos.eqb_eq in Hm; subst fid; exact Hsmact | discriminate H ].
  Qed.

  (* ---- the engine fallback over a dive_chk-recognized statement ---- *)
  Lemma dive_generic :
    forall s e le m0 tr le' m' out,
      (forall g, mem_id g stored_globals = true -> e ! g = None) ->
      (forall g, mem_id g dive_ids = true -> e ! g = None) ->
      (forall g, mem_id g dive_xids = true -> e ! g = None) ->
      (forall g, mem_id g dive_sids = true -> e ! g = None) ->
      e ! interaction._gGlobalTimer = None ->
      wwalk_chk false dive_wact dive_ids nil dive_cact dive_xids dive_sids nil s = true ->
      (forall b o, le ! mario_actions_airborne._m = Some (Vptr b o) ->
                   b = bm /\ o = Ptrofs.zero) ->
      act_inv dive_wact le ->
      chase_inv SafeB dive_cact le ->
      NoA m0 -> MWF m0 -> Mem.valid_block m0 bm -> action_sat not_tainted m0 bm ->
      exec_stmt function_entry2 (lp_ge lp) e le m0 s tr le' m' out ->
      Mem.valid_block m' bm /\ action_sat not_tainted m' bm /\ MWF m' /\ NoA m'
      /\ (forall b o, le' ! mario_actions_airborne._m = Some (Vptr b o) ->
                      b = bm /\ o = Ptrofs.zero)
      /\ act_inv dive_wact le' /\ chase_inv SafeB dive_cact le'.
  Proof.
    intros s e le m0 tr le' m' out Hub_g Hub_i Hub_x Hub_s Hubgt Hchk
           Htat Hact Hch HN HM HV HS Hexec.
    destruct (wwalk_pres0 lp LO_mario bm NoA MWF HNoA_of_MWF HMWF_window
  ...
  Proof.
    intros s e le m0 tr le' m' out Hexec.
    induction Hexec;
      intros Hub_g Hub_i Hub_x Hub_s Hcsn Hubgt Hchk Htat Hact Hch HN HM HV HS.
    - exact (conj HV (conj HS (conj HM (conj HN
               (conj Htat (conj Hact Hch)))))).
    - cbn [dive_chk] in Hchk.
      apply orb_true_iff in Hchk as [Hg | Hsp]; [ | discriminate Hsp ].
      eapply (dive_generic _ _ _ _ _ _ _ _ Hub_g Hub_i Hub_x Hub_s Hubgt
                Hg Htat Hact Hch HN HM HV HS);
        eapply exec_Sassign; eauto.
    - cbn [dive_chk] in Hchk.
      apply orb_true_iff in Hchk as [Hg | Hsp]; [ | discriminate Hsp ].
      eapply (dive_generic _ _ _ _ _ _ _ _ Hub_g Hub_i Hub_x Hub_s Hubgt
                Hg Htat Hact Hch HN HM HV HS);
        eapply exec_Sset; eauto.
    - destruct (dive_chk_scall_inv _ _ _ Hchk)
        as [Hg | (-> & fid & fty & -> & Hsp)].
      { eapply (dive_generic _ _ _ _ _ _ _ _ Hub_g Hub_i Hub_x Hub_s Hubgt
                  Hg Htat Hact Hch HN HM HV HS);
          eapply exec_Scall; eauto. }
      destruct (csa_const_site_chk_shape _ _ _ Hsp)
        as (-> & -> & ea & aa & ana & -> & Hea & Haa).
      assert (Hex : exec_stmt function_entry2 (lp_ge lp) e le m
                      (Scall None
                         (Evar M._common_slide_action
                            (Tfunction (tyMSp :: tuint :: tuint :: tint :: nil)
                               tvoid cc_default))
                         (Etempvar M._m tyMSp :: Econst_int ea tint :: Econst_int aa tint
                          :: Econst_int ana tint :: nil))
                      t (set_opttemp None vres le) m' Out_normal)
        by (eapply exec_Scall; eauto).
      destruct (csa_const_site_pres e ea aa ana le m _ _ _ _
                  Hcsn Hea Haa Htat Hex HN HM HV HS)
        as (HV' & HS' & HM' & HN' & _ & _).
      cbn [set_opttemp] in *.
      exact (conj HV' (conj HS' (conj HM' (conj HN'
               (conj Htat (conj Hact Hch)))))).
    ...
    - cbn [dive_chk] in Hchk.
      ...
      { eapply (dive_generic _ _ _ _ _ _ _ _ Hub_g Hub_i Hub_x Hub_s Hubgt
          ...
          eapply exec_Sseq_1; eauto. }
      apply andb_prop in Hsp as [H1 H2].
      destruct (IHHexec1 Hub_g Hub_i Hub_x Hub_s Hcsn Hubgt H1 Htat Hact Hch
                  HN HM HV HS)
        as (HV1 & HS1 & HM1 & HN1 & Htat1 & Hact1 & Hch1).
      exact (IHHexec2 Hub_g Hub_i Hub_x Hub_s Hcsn Hubgt H2 Htat1 Hact1 Hch1
               HN1 HM1 HV1 HS1).
    - cbn [dive_chk] in Hchk.
      apply orb_true_iff in Hchk as [Hg | Hsp].
      { eapply (dive_generic _ _ _ _ _ _ _ _ Hub_g Hub_i Hub_x Hub_s Hubgt
          ...
          eapply exec_Sseq_2; eauto. }
      apply andb_prop in Hsp as [H1 _].
      exact (IHHexec Hub_g Hub_i Hub_x Hub_s Hcsn Hubgt H1 Htat Hact Hch
               HN HM HV HS).
    - cbn [dive_chk] in Hchk.
      apply orb_true_iff in Hchk as [Hg | Hsp].
      { eapply (dive_generic _ _ _ _ _ _ _ _ Hub_g Hub_i Hub_x Hub_s Hubgt
    ...
    - cbn [dive_chk] in Hchk.
      ...
      { eapply (dive_generic _ _ _ _ _ _ _ _ Hub_g Hub_i Hub_x Hub_s Hubgt
          ...
          eapply exec_Sswitch; eauto. }
      apply IHHexec; try assumption.
      apply dive_chk_select. exact Hsp.
  Qed.

  Example ads_pin :
    (prog_defmap mario_actions_moving.prog) ! M._act_dive_slide
    = Some (Gfun (Internal M.f_act_dive_slide)).
  Proof. vm_compute. reflexivity. Qed.

  Lemma mov_ads_pres : body_pres lp NoA MWF bm M.f_act_dive_slide.
  Proof.
    intros m vargs t mEnd vres Hmarg Hevf HN HM HV HS.
    assert (Hmarg' : marg_ok bm vargs) by (apply Hmarg; vm_compute; reflexivity).
    inv Hevf.
    ...
    inv Hentry.
    match goal with Ha : alloc_variables _ _ _ _ _ _ |- _ =>
      change (fn_vars M.f_act_dive_slide) with (@nil (ident * type)) in Ha; inv Ha end.
    match goal with Hb : bind_parameter_temps _ _ _ = Some _ |- _ => rename Hb into Hbind end.
    change (fn_params M.f_act_dive_slide) with ((M._m, tyMSp) :: nil) in Hbind.
    cbn [bind_parameter_temps] in Hbind.
    destruct vargs as [| vhead vrest]; [ discriminate Hbind | ].
    destruct vrest as [|]; [ | discriminate Hbind ].
    injection Hbind as <-.
    change (blocks_of_env (lp_ge lp) empty_env) with (@nil (block * Z * Z)) in Hfree.
    cbn [Mem.free_list] in Hfree. injection Hfree as <-.
    set (base := create_undef_temps (fn_temps M.f_act_dive_slide)) in *.
    assert (Htat : forall b o,
       (PTree.set M._m vhead base) ! M._m = Some (Vptr b o) -> b = bm /\ o = Ptrofs.zero).
    { intros b o Hg. rewrite PTree.gss in Hg. injection Hg as Hvh.
      rewrite Hvh in Hmarg'. cbn in Hmarg'. exact Hmarg'. }
    assert (Hact0 : act_inv dive_wact (PTree.set M._m vhead base)).
    { intros t' Hmem' x Hg'.
      unfold dive_wact in Hmem'. cbn [mem_id existsb] in Hmem'.
      repeat (apply orb_true_iff in Hmem' as [Ht | Hmem'];
              [ apply Pos.eqb_eq in Ht; subst t';
                rewrite PTree.gso in Hg' by (vm_compute; congruence);
                left; exact (create_undef_temps_val _ _ _ Hg') | ]).
      discriminate Hmem'. }
    assert (Hch0 : chase_inv SafeB dive_cact (PTree.set M._m vhead base)).
    { intros t' Hmem' b o Hg'.
      unfold dive_cact in Hmem'. cbn [mem_id existsb] in Hmem'.
      apply orb_true_iff in Hmem' as [Ht | Hf]; [ | discriminate Hf ].
      apply Pos.eqb_eq in Ht; subst t'.
      rewrite PTree.gso in Hg' by (vm_compute; congruence).
      pose proof (create_undef_temps_val _ _ _ Hg') as EE; discriminate EE. }
    unfold M.f_act_dive_slide in Hbody; cbn [fn_body] in Hbody.
    destruct (dive_pres _ _ _ _ _ _ _ _ Hbody
                (empty_env_unbound _) (empty_env_unbound _) (empty_env_unbound _)
                (empty_env_unbound _) (PTree.gempty _ _) (PTree.gempty _ _)
                ltac:(vm_compute; reflexivity) Htat Hact0 Hch0 HN HM HV HS)
      as (HV' & HS' & HM' & _).
  ...
  (* ==================================================================== *)
  (* act_riding_shell_ground: the gwspf-HYBRID walk.                       *)
  (*   tilt_body_ground_shell = clean chase-store helper (val0C/marioObj). *)
  (*   update_shell_speed = a CALL-FREE body EXCEPT approach_s32 +         *)
  (*     apply_slope_accel, but with the water-pseudo-floor patch:         *)
  (*       m->floor = &gWaterSurfacePseudoFloor;  (root-store of a global  *)
  (*       t'21 = m->floor; t'22 = m->waterLevel; t'21->originOffset=t'22; *)
  (*     a root-store-of-global + load-after-store + through-store, which  *)
  (*     the engine rejects -> a SPECIAL SITE walked by uss_gwspf_block_   *)
  (*     pres (HMWF_window value-agnostic for the floor field store +      *)
  (*     HMWF_glob for the gwspf-block originOffset store).                *)
  (*   the leaf itself = a clean engine walk (no chase stores).            *)
  (* ==================================================================== *)

  Local Notation tMSM := (Tstruct M._MarioState noattr).
  Local Notation tSurfM := (Tstruct M._Surface noattr).
  Local Notation tSurfMp := (tptr (Tstruct M._Surface noattr)).

  (* Val.load_result is the identity on pointers at Mptr (ptr32). *)
  Lemma load_result_mptr_ptr_mov :
  ...
  (* ---- tilt_body_ground_shell: chase-store helper row ---- *)
  Definition tbgs_cact : list ident := M._val0C :: M._marioObj :: nil.
  Definition tbgs_xids : list ident := M._approach_s32 :: nil.

  Example tbgs_pin :
    (prog_defmap mario_actions_moving.prog) ! M._tilt_body_ground_shell
    = Some (Gfun (Internal M.f_tilt_body_ground_shell)).
  Proof. vm_compute. reflexivity. Qed.
  Example tbgs_vars : fn_vars M.f_tilt_body_ground_shell = nil.
  Proof. vm_compute. reflexivity. Qed.
  Example tbgs_pok :
    match fn_params M.f_tilt_body_ground_shell with
    | (i, ty) :: ps =>
        Pos.eqb i mario_actions_airborne._m
        && proj_sumbool (type_eq ty tyMSp)
        && negb (mem_id mario_actions_airborne._m (map fst ps))
  ...
  Proof. vm_compute. reflexivity. Qed.
  Example tbgs_nonparam :
    forallb (fun t' => negb (mem_id t' (map fst (fn_params M.f_tilt_body_ground_shell))))
      tbgs_cact = true.
  Proof. vm_compute. reflexivity. Qed.
  Example tbgs_walk :
    wwalk_chk false nil nil nil tbgs_cact tbgs_xids nil nil
      (fn_body M.f_tilt_body_ground_shell) = true.
  Proof. vm_compute. reflexivity. Qed.

  Lemma tbgs_xids_rows : forall fid, mem_id fid tbgs_xids = true ->
      call_pres_ext lp bm NoA MWF fid.
  Proof.
    intros fid H. unfold tbgs_xids in H. cbn [mem_id existsb] in H.
    apply orb_true_iff in H as [Hm | H];
      [ apply Pos.eqb_eq in Hm; subst fid;
        exact (Hpres_obj_ext mario_actions_object._approach_s32 eq_refl)
      | discriminate H ].
  ...
  Lemma mov_tbgs_row : call_pres lp bm NoA MWF M._tilt_body_ground_shell.
  Proof.
    apply (call_pres_of_wwalk_cact lp LO_mario bm NoA MWF HNoA_of_MWF
             HMWF_window HMWF_glob HMWF_act SafeB HSafeNotBm HchaseRoot
             HMWF_chase HMWF_root HMWF_sglob HchaseStep HMWF_chase_safe
             mario_actions_moving.prog M._tilt_body_ground_shell
             M.f_tilt_body_ground_shell
             nil nil tbgs_cact tbgs_xids nil
             LO_mov tbgs_pin tbgs_vars tbgs_pok tbgs_nonparam).
    - intros fid' H. discriminate H.
    - intros fid' H. discriminate H.
    - exact tbgs_xids_rows.
    - intros fid' H. discriminate H.
    - exact tbgs_walk.
  Qed.

  (* ---- update_shell_speed: the gwspf-hybrid recognizer-walk ---- *)
  Definition uss_ids : list ident := M._apply_slope_accel :: nil.
  Definition uss_xids : list ident := M._approach_s32 :: nil.

  Definition uss_gwspf_block_chk (s : statement) : bool :=
    match s with
    | Ssequence
        (Sassign (Efield (Ederef (Etempvar m1 tm1) ts1) fl1 tps1)
           (Eaddrof (Evar gw tgw) tps2))
        (Ssequence
           (Sset t21a (Efield (Ederef (Etempvar m2 tm2) ts2) fl2 tps3))
           (Ssequence
              (Sset t22a (Efield (Ederef (Etempvar m3 tm3) ts3) wl twl))
              (Sassign
                 (Efield (Ederef (Etempvar t21b tps4) ts4) oo too)
                 (Etempvar t22b ttv)))) =>
        Pos.eqb m1 M._m && Pos.eqb m2 M._m && Pos.eqb m3 M._m
        && Pos.eqb fl1 M._floor && Pos.eqb fl2 M._floor
        && Pos.eqb gw M._gWaterSurfacePseudoFloor
        && Pos.eqb t21a M._t'21 && Pos.eqb t21b M._t'21
        && Pos.eqb t22a M._t'22 && Pos.eqb t22b M._t'22
        && Pos.eqb wl M._waterLevel && Pos.eqb oo M._originOffset
        && proj_sumbool (type_eq tm1 tyMSp) && proj_sumbool (type_eq tm2 tyMSp)
        && proj_sumbool (type_eq tm3 tyMSp)
        && proj_sumbool (type_eq ts1 tMSM) && proj_sumbool (type_eq ts2 tMSM)
        && proj_sumbool (type_eq ts3 tMSM) && proj_sumbool (type_eq ts4 tSurfM)
        && proj_sumbool (type_eq tgw tSurfM)
        && proj_sumbool (type_eq tps1 tSurfMp) && proj_sumbool (type_eq tps2 tSurfMp)
        && proj_sumbool (type_eq tps3 tSurfMp) && proj_sumbool (type_eq tps4 tSurfMp)
        && proj_sumbool (type_eq twl tshort) && proj_sumbool (type_eq too tfloat)
        && proj_sumbool (type_eq ttv tshort)
    | _ => false
    end.

  Fixpoint uss_chk (s : statement) : bool :=
    wwalk_chk false nil uss_ids nil nil uss_xids nil nil s
    || match s with
       | Ssequence s1 s2 =>
           uss_gwspf_block_chk (Ssequence s1 s2) || (uss_chk s1 && uss_chk s2)
       | Sifthenelse _ s1 s2 => uss_chk s1 && uss_chk s2
       | _ => false
       end.

  Lemma uss_gwspf_block_decode :
    forall s, uss_gwspf_block_chk s = true ->
      s = Ssequence
            (Sassign (Efield (Ederef (Etempvar M._m tyMSp) tMSM) M._floor tSurfMp)
               (Eaddrof (Evar M._gWaterSurfacePseudoFloor tSurfM) tSurfMp))
            (Ssequence
               (Sset M._t'21
                  (Efield (Ederef (Etempvar M._m tyMSp) tMSM) M._floor tSurfMp))
               (Ssequence
                  (Sset M._t'22
                     (Efield (Ederef (Etempvar M._m tyMSp) tMSM) M._waterLevel tshort))
                  (Sassign
                     (Efield (Ederef (Etempvar M._t'21 tSurfMp) tSurfM)
                        M._originOffset tfloat)
                     (Etempvar M._t'22 tshort)))).
  Proof.
    intros s H.
    destruct s as [ | | | | | s1 s2 | | | | | | | | ]; try discriminate H.
    destruct s1 as [ | a1 a2 | | | | | | | | | | | | ]; try discriminate H.
    destruct a1 as [ | | | | | | | | | | | ef1 fl1 tps1 | | ]; try discriminate H.
    destruct ef1 as [ | | | | | | eb1 ts1 | | | | | | | ]; try discriminate H.
    destruct eb1 as [ | | | | | m1 tm1 | | | | | | | | ]; try discriminate H.
    destruct a2 as [ | | | | | | | inner tps2 | | | | | | ]; try discriminate H.
    destruct inner as [ | | | | gw tgw | | | | | | | | | ]; try discriminate H.
    destruct s2 as [ | | | | | s2a s2b | | | | | | | | ]; try discriminate H.
    destruct s2a as [ | | t21a ae | | | | | | | | | | | ]; try discriminate H.
    destruct ae as [ | | | | | | | | | | | ef2 fl2 tps3 | | ]; try discriminate H.
    destruct ef2 as [ | | | | | | eb2 ts2 | | | | | | | ]; try discriminate H.
    destruct eb2 as [ | | | | | m2 tm2 | | | | | | | | ]; try discriminate H.
    destruct s2b as [ | | | | | s2ba s2bb | | | | | | | | ]; try discriminate H.
    destruct s2ba as [ | | t22a ee | | | | | | | | | | | ]; try discriminate H.
    destruct ee as [ | | | | | | | | | | | ef3 wl twl | | ]; try discriminate H.
    destruct ef3 as [ | | | | | | eb3 ts3 | | | | | | | ]; try discriminate H.
    destruct eb3 as [ | | | | | m3 tm3 | | | | | | | | ]; try discriminate H.
    destruct s2bb as [ | b1 b2 | | | | | | | | | | | | ]; try discriminate H.
    destruct b1 as [ | | | | | | | | | | | ef4 oo too | | ]; try discriminate H.
    destruct ef4 as [ | | | | | | eb4 ts4 | | | | | | | ]; try discriminate H.
    destruct eb4 as [ | | | | | t21b tps4 | | | | | | | | ]; try discriminate H.
    destruct b2 as [ | | | | | t22b ttv | | | | | | | | ]; try discriminate H.
    cbn [uss_gwspf_block_chk] in H.
    apply andb_true_iff in H as [H Httv].
    apply andb_true_iff in H as [H Htoo].
    apply andb_true_iff in H as [H Htwl].
    ...
    apply andb_true_iff in H as [H Htgw].
    apply andb_true_iff in H as [H Hts4].
    apply andb_true_iff in H as [H Hts3].
    apply andb_true_iff in H as [H Hts2].
    apply andb_true_iff in H as [H Hts1].
    apply andb_true_iff in H as [H Htm3].
    apply andb_true_iff in H as [H Htm2].
    apply andb_true_iff in H as [H Htm1].
    apply andb_true_iff in H as [H Hoo].
    apply andb_true_iff in H as [H Hwl].
    apply andb_true_iff in H as [H Ht22b].
    apply andb_true_iff in H as [H Ht22a].
    apply andb_true_iff in H as [H Ht21b].
    apply andb_true_iff in H as [H Ht21a].
    apply andb_true_iff in H as [H Hgw].
    apply andb_true_iff in H as [H Hfl2].
    apply andb_true_iff in H as [H Hfl1].
    apply andb_true_iff in H as [H Hm3].
    apply andb_true_iff in H as [Hm1 Hm2].
    apply Pos.eqb_eq in Hm1; apply Pos.eqb_eq in Hm2; apply Pos.eqb_eq in Hm3.
    apply Pos.eqb_eq in Hfl1; apply Pos.eqb_eq in Hfl2; apply Pos.eqb_eq in Hgw.
    apply Pos.eqb_eq in Ht21a; apply Pos.eqb_eq in Ht21b.
    apply Pos.eqb_eq in Ht22a; apply Pos.eqb_eq in Ht22b.
    apply Pos.eqb_eq in Hwl; apply Pos.eqb_eq in Hoo.
    subst.
    destruct (type_eq tm1 tyMSp) as [->|]; [|discriminate Htm1].
    destruct (type_eq tm2 tyMSp) as [->|]; [|discriminate Htm2].
    destruct (type_eq tm3 tyMSp) as [->|]; [|discriminate Htm3].
    destruct (type_eq ts1 tMSM) as [->|]; [|discriminate Hts1].
    destruct (type_eq ts2 tMSM) as [->|]; [|discriminate Hts2].
    destruct (type_eq ts3 tMSM) as [->|]; [|discriminate Hts3].
    destruct (type_eq ts4 tSurfM) as [->|]; [|discriminate Hts4].
    destruct (type_eq tgw tSurfM) as [->|]; [|discriminate Htgw].
    destruct (type_eq tps1 tSurfMp) as [->|]; [|discriminate Htps1].
    destruct (type_eq tps2 tSurfMp) as [->|]; [|discriminate Htps2].
    destruct (type_eq tps3 tSurfMp) as [->|]; [|discriminate Htps3].
    destruct (type_eq tps4 tSurfMp) as [->|]; [|discriminate Htps4].
    destruct (type_eq twl tshort) as [->|]; [|discriminate Htwl].
    destruct (type_eq too tfloat) as [->|]; [|discriminate Htoo].
    destruct (type_eq ttv tshort) as [->|]; [|discriminate Httv].
    reflexivity.
  Qed.

  (* the gwspf-block special-site brick: C (m->floor=&gwspf) via HMWF_window
     (value-agnostic), D reload (load-after-store), E scalar load, F store
     through t'21 into the gwspf block via HMWF_glob (gwspf in
     stored_globals). *)
  Lemma uss_gwspf_block_pres :
    forall e le m0 tr le' m' out,
      e ! M._gWaterSurfacePseudoFloor = None ->
      (forall b o, le ! M._m = Some (Vptr b o) -> b = bm /\ o = Ptrofs.zero) ->
      exec_stmt function_entry2 (lp_ge lp) e le m0
        (Ssequence
           (Sassign (Efield (Ederef (Etempvar M._m tyMSp) tMSM) M._floor tSurfMp)
      ...
      /\ (forall b o, le' ! M._m = Some (Vptr b o) -> b = bm /\ o = Ptrofs.zero).
  Proof.
    intros e le m0 tr le' m' out Hgw Htat Hexec HN HM HV HS.
    assert (Hfo : field_offset (prog_comp_env mario.prog) M._floor mario_state_members
                  = OK (104%Z, Full)) by (vm_compute; reflexivity).
    assert (Hwin : store_window_ok 104 (size_chunk Mptr) = true)
      by (vm_compute; reflexivity).
    assert (Hu104 : Ptrofs.unsigned (Ptrofs.repr 104) = 104)
      by (vm_compute; reflexivity).
    inv Hexec.
    2:{ match goal with
        | Hs : exec_stmt _ _ _ _ _ (Sassign _ _) _ _ _ _ |- _ => inv Hs end.
        match goal with
        | Hne : Out_normal <> Out_normal |- _ =>
            contradiction Hne; reflexivity end. }
    match goal with
    | Hs : exec_stmt _ _ _ _ _ (Sassign _ _) _ _ _ _ |- _ => inv Hs end.
    match goal with
    ...
    match goal with
    | Hev : eval_expr _ _ _ _ (Eaddrof _ _) _ |- _ => inv Hev end.
    2:{ match goal with
        | Hlv : eval_lvalue _ _ _ _ (Eaddrof _ _) _ _ _ |- _ => inv Hlv end. }
    match goal with
    | Hlv : eval_lvalue _ _ _ _
              (Evar M._gWaterSurfacePseudoFloor _) _ _ _ |- _ => inv Hlv end.
    { match goal with
      | Hb : e ! M._gWaterSurfacePseudoFloor = Some _ |- _ =>
          rewrite Hgw in Hb; discriminate Hb end. }
    match goal with
    | Hsym0 : Genv.find_symbol _ M._gWaterSurfacePseudoFloor = Some ?g |- _ =>
        rename Hsym0 into Hsym; rename g into gb end.
    assert (Hgwmem : mem_id M._gWaterSurfacePseudoFloor stored_globals = true)
      by (vm_compute; reflexivity).
    destruct (HMWF_glob _ Hgwmem gb Hsym) as (Hgb_bm & Hgb_store).
    match goal with
    | Hca : sem_cast _ _ _ _ = Some _ |- _ =>
        apply AirborneSurface.sem_cast_ptr_ptr_id in Hca; subst end.
    match goal with
    | Has : assign_loc _ (typeof _) _ _ _ _ _ _ |- _ =>
        rewrite Ptrofs.add_zero_l in Has; cbn [typeof] in Has; inv Has end.
    2:{ match goal with
        | Hac : access_mode _ = By_copy |- _ => cbn in Hac; discriminate Hac end. }
    match goal with
    | Hac : access_mode _ = By_value _ |- _ => cbn in Hac; injection Hac as <- end.
    match goal with
    | Hsv : Mem.storev _ _ _ _ = Some _ |- _ =>
        unfold Mem.storev in Hsv;
        rewrite Hu104 in Hsv;
        rename Hsv into Hst1 end.
    assert (HM1 : MWF m1) by (exact (HMWF_window _ _ _ _ _ HM Hwin Hst1)).
    assert (HV1 : Mem.valid_block m1 bm)
      by (eapply Mem.store_valid_block_1; eauto).
    assert (HS1 : action_sat not_tainted m1 bm).
    { intros av Hload.
      rewrite (Mem.load_store_other _ _ _ _ _ _ Hst1) in Hload;
        [ exact (HS av Hload) | right; cbn; lia ]. }
    pose proof (Mem.load_store_same _ _ _ _ _ _ Hst1) as Hload1.
    rewrite load_result_mptr_ptr_mov in Hload1.
    match goal with
    | Hs : exec_stmt _ _ _ _ _ (Ssequence _ _) _ _ _ _ |- _ => inv Hs end.
    2:{ match goal with
        | Hs : exec_stmt _ _ _ _ _ (Sset _ _) _ _ _ _ |- _ => inv Hs end.
        match goal with
        | Hne : Out_normal <> Out_normal |- _ =>
            contradiction Hne; reflexivity end. }
    match goal with
    | Hs : exec_stmt _ _ _ _ _ (Sset M._t'21 _) _ _ _ _ |- _ => inv Hs end.
    match goal with
    | Hev : eval_expr _ _ _ _ (Efield _ _ _) _ |- _ => inv Hev end.
    match goal with
    | Hlv2 : eval_lvalue _ _ _ _ (Efield _ _ _) ?loc ?ofs ?bf |- _ =>
        destruct (mfield_lvalue_geom_lp lp LO_mario _ _ _ _ _ _
                    loc ofs bf _ _ _ Hlvb Hfo Hlv2) as (D3 & D4 & D5);
        subst loc ofs bf end.
    match goal with
    | Hd : deref_loc (typeof _) _ _ _ _ _ |- _ => cbn [typeof] in Hd end.
    match goal with
    | Hd : deref_loc _ _ bm _ _ _ |- _ =>
        rewrite Ptrofs.add_zero_l in Hd; inv Hd end.
    2:{ match goal with
        | Hac : access_mode _ = By_reference |- _ => cbn in Hac; discriminate Hac end. }
    2:{ match goal with
        | Hac : access_mode _ = By_copy |- _ => cbn in Hac; discriminate Hac end. }
    match goal with
    | Hac : access_mode _ = By_value _ |- _ => cbn in Hac; injection Hac as <- end.
    ...
    | Hldv : Mem.loadv _ _ _ = Some _ |- _ =>
        unfold Mem.loadv in Hldv;
        rewrite Hu104 in Hldv;
        rewrite Hload1 in Hldv; injection Hldv as <- end.
    match goal with
    | Hs : exec_stmt _ _ _ _ _ (Ssequence (Sset M._t'22 _) _) _ _ _ _ |- _ =>
        inv Hs end.
    2:{ match goal with
        | Hs : exec_stmt _ _ _ _ _ (Sset M._t'22 _) _ _ _ _ |- _ => inv Hs end.
        match goal with
        | Hne : Out_normal <> Out_normal |- _ =>
            contradiction Hne; reflexivity end. }
    match goal with
    ...
    | Hlv : eval_lvalue _ _ _ _ (Efield _ _ _) _ _ _ |- _ =>
        ...
        apply eval_lvalue_Efield_base in Hpin;
        destruct Hpin as (oF & HbaseF) end.
    destruct (eval_expr_Ederef_load _ _ _ _ _ _ _ HbaseF)
      as (lbF & obF & bfbF & HlvbF & HderefbF).
    pose proof (deref_loc_aggregate_block
                  tSurfM _ _ _ _ _ _ (or_intror eq_refl) HderefbF) as HblkF.
    apply eval_lvalue_Ederef_base in HlvbF.
    apply eval_expr_Etempvar_val in HlvbF.
    rewrite PTree.gso in HlvbF by (vm_compute; congruence).
    rewrite PTree.gss in HlvbF. injection HlvbF as <- <-.
    match goal with
    | Has : assign_loc _ (typeof _) _ _ _ _ _ _ |- _ =>
        cbn [typeof] in Has; rewrite HblkF in Has; inv Has end.
    2:{ match goal with
        | Hac : access_mode _ = By_copy |- _ => cbn in Hac; discriminate Hac end. }
    2:{ match goal with
        | Hsb : store_bitfield _ _ _ _ _ _ _ _ _ _ |- _ => inv Hsb end. }
    match goal with
    | Hac : access_mode _ = By_value _ |- _ => cbn in Hac; injection Hac as <- end.
    match goal with
    | Hsv : Mem.storev _ _ (Vptr gb _) _ = Some _ |- _ =>
        unfold Mem.storev in Hsv; rename Hsv into Hst3 end.
    assert (HM' : MWF m') by (eapply Hgb_store; [ exact HM1 | exact Hst3 ]).
    split; [ eapply Mem.store_valid_block_1; eauto | ].
    split; [ eapply (LocalVarsSurface.store_action_sat bm);
  ...
  Lemma uss_ids_rows : forall fid, mem_id fid uss_ids = true ->
      call_pres lp bm NoA MWF fid.
  Proof.
    intros fid H. unfold uss_ids in H. cbn [mem_id existsb] in H.
    apply orb_true_iff in H as [Hm | H];
      [ apply Pos.eqb_eq in Hm; subst fid; exact mov_asa_row | discriminate H ].
  Qed.
  Lemma uss_xids_rows : forall fid, mem_id fid uss_xids = true ->
      call_pres_ext lp bm NoA MWF fid.
  Proof.
    intros fid H. unfold uss_xids in H. cbn [mem_id existsb] in H.
    apply orb_true_iff in H as [Hm | H];
      [ apply Pos.eqb_eq in Hm; subst fid;
        exact (Hpres_obj_ext mario_actions_object._approach_s32 eq_refl)
      | discriminate H ].
  ...
  (* the engine fallback over a wwalk_chk-recognized statement *)
  Lemma uss_generic :
    forall s e le m0 tr le' m' out,
      (forall g, mem_id g stored_globals = true -> e ! g = None) ->
      (forall g, mem_id g uss_ids = true -> e ! g = None) ->
      (forall g, mem_id g uss_xids = true -> e ! g = None) ->
      (forall g, mem_id g (@nil ident) = true -> e ! g = None) ->
      e ! interaction._gGlobalTimer = None ->
      wwalk_chk false nil uss_ids nil nil uss_xids nil nil s = true ->
      (forall b o, le ! mario_actions_airborne._m = Some (Vptr b o) ->
                   b = bm /\ o = Ptrofs.zero) ->
      act_inv nil le ->
      chase_inv SafeB nil le ->
  ...
  Proof.
    ...
    destruct (wwalk_pres0 lp LO_mario bm NoA MWF HNoA_of_MWF HMWF_window
                HMWF_glob HMWF_act SafeB HSafeNotBm HchaseRoot HMWF_chase
                HMWF_root HMWF_sglob HchaseStep HMWF_chase_safe
                false nil uss_ids nil nil uss_xids nil nil
                uss_ids_rows ltac:(intros fid HH; discriminate HH)
                uss_xids_rows ltac:(intros fid HH; discriminate HH)
                ltac:(intros fid HH; discriminate HH)
                _ _ _ _ _ _ _ _ Hexec
                Hub_g Hub_i ltac:(intros g HH; discriminate HH) Hub_x Hub_s
                ltac:(intros g HH; discriminate HH) Hubgt
  ...
  Proof.
    intros s e le m0 tr le' m' out Hexec.
    induction Hexec;
      intros Hub_g Hub_i Hub_x Hub_s Hgw_e Hubgt Hchk Htat Hact Hch HN HM HV HS.
    - exact (conj HV (conj HS (conj HM (conj HN
               (conj Htat (conj Hact Hch)))))).
    - cbn [uss_chk] in Hchk.
      apply orb_true_iff in Hchk as [Hg | Hsp]; [ | discriminate Hsp ].
      eapply (uss_generic _ _ _ _ _ _ _ _ Hub_g Hub_i Hub_x Hub_s Hubgt
                Hg Htat Hact Hch HN HM HV HS);
        eapply exec_Sassign; eauto.
    - cbn [uss_chk] in Hchk.
      apply orb_true_iff in Hchk as [Hg | Hsp]; [ | discriminate Hsp ].
    ...
    - cbn [uss_chk] in Hchk.
      ...
      { eapply (uss_generic _ _ _ _ _ _ _ _ Hub_g Hub_i Hub_x Hub_s Hubgt
                  Hg Htat Hact Hch HN HM HV HS);
          eapply exec_Sseq_1; eauto. }
      apply orb_true_iff in Hsp as [Hsite | Hrec].
      { apply uss_gwspf_block_decode in Hsite. injection Hsite as -> ->.
        match goal with
        ...
        destruct (uss_gwspf_block_pres _ _ _ _ _ _ _ Hgw_e Htat Hexs HN HM HV HS)
          as (HV' & HS' & HM' & HN' & _ & Htat').
        refine (conj HV' (conj HS' (conj HM' (conj HN'
                  (conj Htat' (conj _ _))))));
          [ intros t' HH; discriminate HH | intros t' HH; discriminate HH ]. }
      apply andb_prop in Hrec as [H1 H2].
      destruct (IHHexec1 Hub_g Hub_i Hub_x Hub_s Hgw_e Hubgt H1 Htat Hact Hch
                  HN HM HV HS)
        as (HV1 & HS1 & HM1 & HN1 & Htat1 & Hact1 & Hch1).
      exact (IHHexec2 Hub_g Hub_i Hub_x Hub_s Hgw_e Hubgt H2 Htat1 Hact1 Hch1
               HN1 HM1 HV1 HS1).
    - cbn [uss_chk] in Hchk.
      apply orb_true_iff in Hchk as [Hg | Hsp].
      { eapply (uss_generic _ _ _ _ _ _ _ _ Hub_g Hub_i Hub_x Hub_s Hubgt
      ...
      exact (IHHexec Hub_g Hub_i Hub_x Hub_s Hgw_e Hubgt H1 Htat Hact Hch
               HN HM HV HS).
    - cbn [uss_chk] in Hchk.
      apply orb_true_iff in Hchk as [Hg | Hsp].
      { eapply (uss_generic _ _ _ _ _ _ _ _ Hub_g Hub_i Hub_x Hub_s Hubgt
  ...
  Example uss_pin :
    (prog_defmap mario_actions_moving.prog) ! M._update_shell_speed
    = Some (Gfun (Internal M.f_update_shell_speed)).
  Proof. vm_compute. reflexivity. Qed.

  Lemma mov_uss_pres : body_pres lp NoA MWF bm M.f_update_shell_speed.
  Proof.
    intros m vargs t mEnd vres Hmarg Hevf HN HM HV HS.
    assert (Hmarg' : marg_ok bm vargs) by (apply Hmarg; vm_compute; reflexivity).
    inv Hevf.
    ...
    inv Hentry.
    match goal with Ha : alloc_variables _ _ _ _ _ _ |- _ =>
      change (fn_vars M.f_update_shell_speed) with (@nil (ident * type)) in Ha; inv Ha end.
    match goal with Hb : bind_parameter_temps _ _ _ = Some _ |- _ => rename Hb into Hbind end.
    change (fn_params M.f_update_shell_speed) with ((M._m, tyMSp) :: nil) in Hbind.
    cbn [bind_parameter_temps] in Hbind.
    destruct vargs as [| vhead vrest]; [ discriminate Hbind | ].
    destruct vrest as [|]; [ | discriminate Hbind ].
    injection Hbind as <-.
    change (blocks_of_env (lp_ge lp) empty_env) with (@nil (block * Z * Z)) in Hfree.
    cbn [Mem.free_list] in Hfree. injection Hfree as <-.
    set (base := create_undef_temps (fn_temps M.f_update_shell_speed)) in *.
    assert (Htat : forall b o,
       (PTree.set M._m vhead base) ! M._m = Some (Vptr b o) -> b = bm /\ o = Ptrofs.zero).
    { intros b o Hg. rewrite PTree.gss in Hg. injection Hg as Hvh.
      rewrite Hvh in Hmarg'. cbn in Hmarg'. exact Hmarg'. }
    unfold M.f_update_shell_speed in Hbody; cbn [fn_body] in Hbody.
    destruct (uss_pres _ _ _ _ _ _ _ _ Hbody
                (empty_env_unbound _) (empty_env_unbound _) (empty_env_unbound _)
                (empty_env_unbound _) (PTree.gempty _ _) (PTree.gempty _ _)
                ltac:(vm_compute; reflexivity) Htat
                ltac:(intros t' HH; discriminate HH)
                ltac:(intros t' HH; discriminate HH)
                HN HM HV HS)
      as (HV' & HS' & HM' & _).
    exact (conj HV' (conj HS' HM')).
  ...
  Lemma mov_uss_row : call_pres lp bm NoA MWF M._update_shell_speed.
  Proof.
    exact (call_pres_of_body lp bm NoA MWF HNoA_of_MWF
             mario_actions_moving.prog M._update_shell_speed
             M.f_update_shell_speed LO_mov uss_pin mov_uss_pres).
  Qed.

  (* ---- the leaf: act_riding_shell_ground ---- *)
  Let Hmsro : call_pres lp bm NoA MWF interaction._mario_stop_riding_object :=
    ObjectLeafSurface.msro_row lp LO_mario LO_int bm NoA MWF HNoA_of_MWF
      HMWF_window HMWF_glob HMWF_act SafeB HSafeNotBm HchaseRoot HMWF_chase
      HMWF_root HMWF_sglob HchaseStep HMWF_chase_safe
      (Hpres_obj_ext interaction._stop_shell_music eq_refl).

  Definition rsg_ids : list ident :=
    M._update_shell_speed :: M._tilt_body_ground_shell
    :: M._mario_stop_riding_object :: M._set_mario_animation
    :: M._perform_ground_step :: M._mario_set_forward_vel
    :: M._adjust_sound_for_speed :: nil.
  Definition rsg_xids : list ident := M._play_sound :: nil.
  Definition rsg_sids : list ident := M._set_mario_action :: nil.

  Lemma rsg_ids_rows : forall fid, mem_id fid rsg_ids = true ->
      call_pres lp bm NoA MWF fid.
  Proof.
    intros fid H. unfold rsg_ids in H. cbn [mem_id existsb] in H.
    apply orb_true_iff in H as [Hm | H];
      [ apply Pos.eqb_eq in Hm; subst fid; exact mov_uss_row | ].
    apply orb_true_iff in H as [Hm | H];
      [ apply Pos.eqb_eq in Hm; subst fid; exact mov_tbgs_row | ].
    apply orb_true_iff in H as [Hm | H];
      [ apply Pos.eqb_eq in Hm; subst fid; exact Hmsro | ].
    apply orb_true_iff in H as [Hm | H];
      [ apply Pos.eqb_eq in Hm; subst fid; exact mov_sma_row | ].
    apply orb_true_iff in H as [Hm | H];
      [ apply Pos.eqb_eq in Hm; subst fid; exact Hcp_pgs | ].
    ...
    discriminate H.
  Qed.
  Lemma rsg_xids_rows : forall fid, mem_id fid rsg_xids = true ->
      call_pres_ext lp bm NoA MWF fid.
  Proof.
    intros fid H. unfold rsg_xids in H. cbn [mem_id existsb] in H.
    apply orb_true_iff in H as [Hm | H];
      [ apply Pos.eqb_eq in Hm; subst fid; exact Hcpx_psound | discriminate H ].
  Qed.
  Lemma rsg_sids_rows : forall fid, mem_id fid rsg_sids = true ->
      call_pres_act lp bm NoA MWF fid.
  Proof.
    intros fid H. unfold rsg_sids in H. cbn [mem_id existsb] in H.
    apply orb_true_iff in H as [Hm | H];
      [ apply Pos.eqb_eq in Hm; subst fid; exact Hsmact | discriminate H ].
  Qed.

  Example arsg_pin :
    (prog_defmap mario_actions_moving.prog) ! M._act_riding_shell_ground
    = Some (Gfun (Internal M.f_act_riding_shell_ground)).
  Proof. vm_compute. reflexivity. Qed.
  Example arsg_vars : fn_vars M.f_act_riding_shell_ground = nil.
  Proof. vm_compute. reflexivity. Qed.
  Example arsg_pok :
    match fn_params M.f_act_riding_shell_ground with
    | (i, ty) :: ps =>
        Pos.eqb i mario_actions_airborne._m
        && proj_sumbool (type_eq ty tyMSp)
        && negb (mem_id mario_actions_airborne._m (map fst ps))
  ...
  Proof. vm_compute. reflexivity. Qed.
  Example arsg_nonparam :
    forallb (fun t' => negb (mem_id t' (map fst (fn_params M.f_act_riding_shell_ground))))
      (@nil ident) = true.
  Proof. vm_compute. reflexivity. Qed.
  Example arsg_walk :
    wwalk_chk false nil rsg_ids nil nil rsg_xids rsg_sids nil
      (fn_body M.f_act_riding_shell_ground) = true.
  Proof. vm_compute. reflexivity. Qed.

  Lemma mov_arsg_pres : body_pres lp NoA MWF bm M.f_act_riding_shell_ground.
  Proof.
    apply (body_pres_of_wwalk_cact lp LO_mario bm NoA MWF HNoA_of_MWF
             HMWF_window HMWF_glob HMWF_act SafeB HSafeNotBm HchaseRoot
             HMWF_chase HMWF_root HMWF_sglob HchaseStep HMWF_chase_safe
             M.f_act_riding_shell_ground
             rsg_ids nil nil rsg_xids rsg_sids nil
             arsg_vars arsg_pok arsg_nonparam).
    - exact rsg_ids_rows.
    - intros fid' H. discriminate H.
    - exact rsg_xids_rows.
    - exact rsg_sids_rows.
  ...
  (* find_wall_collisions: EF_external in EVERY TU (the honest terminal-ext
     boundary), ol-gated on its sole pointer arg = &(stack-local wallCols).
     Discharged at the capstone by the existing Holcp_fwc_real (already a
     residual there for the resolve walk -- ONE fwc row total). *)
  Hypothesis Holcp_fwc :
    call_pres_ext_ol lp bm NoA MWF SafeB mario._find_wall_collisions.
  (* vec3f_copy(_startPos, m->pos) in act_walking: dst = local _startPos array,
     src = m->pos window.  vec3f_copy writes ONLY through its dst (arg0), so
     under args_window_or_local (both args window-or-local) it preserves
     carried.  EF_external terminal boundary; the honest UNION gate (wol).
     Discharged at the capstone alongside the other vec3f_copy rows. *)
  Hypothesis Hwolcp_v3f :
    call_pres_ext_wol lp bm NoA MWF SafeB mario_actions_automatic._vec3f_copy.
  ...
  (* ==================================================================== *)
  (* check_ledge_climb_down: HYBRID walk (the LAST act_walking helper).     *)
  (*   fn_vars = [_wallCols (Tstruct WallCollisionData); _floor (Surface ptr)]*)
  (*   SPECIAL sites -- write LOCAL blocks ONLY:                            *)
  (*     - 5x  wallCols.{x,y,z,radius,offsetY} = ..  (local-struct field    *)
  (*           store via local_field_assign_pres)                           *)
  (*     - find_wall_collisions(&wallCols)  (ol terminal-ext; args_all_local *)
  (*           gate; ONE find_wall_collisions row, shared with the resolve  *)
  (*           walk's Holcp_fwc_real at the capstone)                       *)
  (*   GENERIC (wwalk census):  find_floor(x,y,z,&floor) [oc],              *)
  (*     m->pos[i] + m->faceAngle[i] window stores, set_mario_action(m,1358,*)
  (*     0) [sids, ACT_LEDGE_CLIMB_DOWN = 0x54E = MOVING, untainted],       *)
  (*     set_mario_animation(m,28) [ids], atan2s [xids].                    *)
  (* ==================================================================== *)
  Definition clcd_lids : list ident := M._wallCols :: M._floor :: nil.
  Definition clcd_oc : list ident := mario._find_floor :: nil.
  Definition clcd_ids : list ident := mario._set_mario_animation :: nil.
  Definition clcd_xids : list ident := mario._atan2s :: nil.
  Definition clcd_sids : list ident := mario._set_mario_action :: nil.

  Definition clcd_gen (s : statement) : bool :=
    wwalk_chk' clcd_lids clcd_oc nil nil nil nil false
      nil clcd_ids nil nil clcd_xids clcd_sids nil s.

  (* the two SPECIAL shapes (everything else generic). *)
  Definition clcd_sp_chk (s : statement) : bool :=
    match s with
    | Sassign (Efield (Evar lid (Tstruct sid _)) _ fty) _ =>
        Pos.eqb lid M._wallCols
        && Pos.eqb sid M._WallCollisionData
        && proj_sumbool (type_eq fty tfloat)
    | Scall optid (Evar fid (Tfunction tyl rty cc)) al =>
        match optid with
        | Some id => negb (Pos.eqb id mario_actions_airborne._m)
        | None => true
        end
        && Pos.eqb fid M._find_wall_collisions
        && proj_sumbool
             (type_eq (Tfunction tyl rty cc)
                (Tfunction (tptr (Tstruct M._WallCollisionData noattr) :: nil)
                   tint cc_default))
        && match al with
           | Eaddrof (Evar cd cdt) tya :: nil =>
               Pos.eqb cd M._wallCols
               && proj_sumbool
                    (type_eq cdt (Tstruct M._WallCollisionData noattr))
               && proj_sumbool
                    (type_eq tya (tptr (Tstruct M._WallCollisionData noattr)))
           | _ => false
           end
    | _ => false
    end.

  Fixpoint clcd_chk (s : statement) : bool :=
    clcd_gen s
    || match s with
       | Ssequence s1 s2 => clcd_chk s1 && clcd_chk s2
       | Sifthenelse _ s1 s2 => clcd_chk s1 && clcd_chk s2
       | _ => clcd_sp_chk s
       end.

  Example clcd_walk :
    clcd_chk (fn_body M.f_check_ledge_climb_down) = true.
  Proof. vm_compute. reflexivity. Qed.

  (* ---- the generic-subtree discharger: ONE wwalk_pres call (clcd census) ---- *)
  Lemma clcd_ids_rows : forall fid, mem_id fid clcd_ids = true ->
      call_pres lp bm NoA MWF fid.
  Proof.
    intros fid H. unfold clcd_ids in H. cbn [mem_id existsb] in H.
    apply orb_true_iff in H as [Hm | H];
      [ apply Pos.eqb_eq in Hm; subst fid; exact mov_sma_row | discriminate H ].
  Qed.
  Lemma clcd_xids_rows : forall fid, mem_id fid clcd_xids = true ->
      call_pres_ext lp bm NoA MWF fid.
  Proof.
    intros fid H. unfold clcd_xids in H. cbn [mem_id existsb] in H.
    apply orb_true_iff in H as [Hm | H];
      [ apply Pos.eqb_eq in Hm; subst fid;
        exact (Hpres_obj_ext mario._atan2s eq_refl) | discriminate H ].
  Qed.
  Lemma clcd_sids_rows : forall fid, mem_id fid clcd_sids = true ->
      call_pres_act lp bm NoA MWF fid.
  Proof.
    intros fid H. unfold clcd_sids in H. cbn [mem_id existsb] in H.
    apply orb_true_iff in H as [Hm | H];
      [ apply Pos.eqb_eq in Hm; subst fid; exact Hsmact | discriminate H ].
  Qed.
  Lemma clcd_oc_rows : forall fid, mem_id fid clcd_oc = true ->
      call_pres_ext_oc lp bm NoA MWF SafeB fid.
  Proof.
    intros fid H. unfold clcd_oc in H. cbn [mem_id existsb] in H.
    apply orb_true_iff in H as [Hm | H];
      [ apply Pos.eqb_eq in Hm; subst fid; exact Hocp_find_floor | discriminate H ].
  Qed.

  Lemma clcd_generic :
    forall s e le m0 tr le' m' out,
      (forall lid, mem_id lid clcd_lids = true ->
         exists lblk tyenv, e ! lid = Some (lblk, tyenv) /\
                            local_blk lp bm SafeB lblk) ->
      (forall g, mem_id g stored_globals = true -> e ! g = None) ->
      (forall g, mem_id g clcd_ids = true -> e ! g = None) ->
      (forall g, mem_id g clcd_xids = true -> e ! g = None) ->
      (forall g, mem_id g clcd_sids = true -> e ! g = None) ->
      (forall g, mem_id g clcd_oc = true -> e ! g = None) ->
      e ! interaction._gGlobalTimer = None ->
      clcd_gen s = true ->
      (forall b o, le ! mario_actions_airborne._m = Some (Vptr b o) ->
                   b = bm /\ o = Ptrofs.zero) ->
  ...
  Proof.
    intros s e le m0 tr le' m' out Hlocal Hub_g Hub_i Hub_x Hub_s Hub_oc
           Hubgt Hchk Htat Hact Hch HN HM HV HS Hexec.
    destruct (wwalk_pres lp LO_mario bm NoA MWF HNoA_of_MWF HMWF_window
                HMWF_glob HMWF_act SafeB HSafeNotBm HchaseRoot HMWF_chase
                HMWF_root HMWF_sglob HchaseStep HMWF_chase_safe
                false nil clcd_ids nil nil clcd_xids clcd_sids nil
  ...
  (* ---- decode of the special Sassign (wallCols.fld store) ---- *)
  Lemma clcd_sp_assign_shape :
    forall a1 a2, clcd_sp_chk (Sassign a1 a2) = true ->
      exists sattr fld,
        a1 = Efield (Evar M._wallCols (Tstruct M._WallCollisionData sattr))
               fld tfloat.
  Proof.
    intros a1 a2 H. cbn [clcd_sp_chk] in H.
    destruct a1 as [ | | | | | | | | | | | base fld fty | | ];
      try discriminate H.
    destruct base as [ | | | | lid lty | | | | | | | | | ]; try discriminate H.
    destruct lty as [ | | | | | | | sid sattr | ]; try discriminate H.
    apply andb_true_iff in H as [H Hfty].
    apply andb_true_iff in H as [Hlid Hsid].
    apply Pos.eqb_eq in Hlid; subst lid.
    apply Pos.eqb_eq in Hsid; subst sid.
    destruct (type_eq fty tfloat) as [-> | ]; [ | discriminate Hfty ].
    exists sattr, fld. reflexivity.
  Qed.

  (* ---- decode of the special Scall (find_wall_collisions(&wallCols)) ---- *)
  Lemma clcd_sp_call_shape :
    forall optid a al, clcd_sp_chk (Scall optid a al) = true ->
      (forall id, optid = Some id -> id <> mario_actions_airborne._m)
      /\ a = Evar M._find_wall_collisions
            (Tfunction (tptr (Tstruct M._WallCollisionData noattr) :: nil)
               tint cc_default)
      /\ al = Eaddrof (Evar M._wallCols (Tstruct M._WallCollisionData noattr))
                 (tptr (Tstruct M._WallCollisionData noattr)) :: nil.
  Proof.
    intros optid a al H. cbn [clcd_sp_chk] in H.
    destruct a as [ | | | | fid fty | | | | | | | | | ]; try discriminate H.
    destruct fty as [ | | | | | | tyl rty cc | | ]; try discriminate H.
    apply andb_true_iff in H as [H Hal].
    apply andb_true_iff in H as [H Hfty].
    apply andb_true_iff in H as [Hopt Hfid].
    apply Pos.eqb_eq in Hfid; subst fid.
    assert (Hoptne : forall id, optid = Some id ->
                                id <> mario_actions_airborne._m).
    { destruct optid as [oid | ]; [ | intros id Hd; discriminate Hd ].
      intros id Hd; injection Hd as ->.
      apply negb_true_iff in Hopt.
      intro EE; subst id; rewrite Pos.eqb_refl in Hopt; discriminate Hopt. }
    destruct (type_eq (Tfunction tyl rty cc)
                (Tfunction (tptr (Tstruct M._WallCollisionData noattr) :: nil)
                   tint cc_default)) as [E | ]; [ | discriminate Hfty ].
    injection E as <- <- <-.
    destruct al as [ | x al0 ]; try discriminate Hal.
    destruct x as [ | | | | | | | base tya | | | | | | ]; try discriminate Hal.
    destruct base as [ | | | | cd cdt | | | | | | | | | ]; try discriminate Hal.
    destruct al0 as [ | x1 al1 ]; [ | discriminate Hal ].
    apply andb_true_iff in Hal as [Hal Htya].
    apply andb_true_iff in Hal as [Hcd Hcdt].
    apply Pos.eqb_eq in Hcd; subst cd.
    destruct (type_eq cdt (Tstruct M._WallCollisionData noattr)) as [-> | ];
      [ | discriminate Hcdt ].
    destruct (type_eq tya (tptr (Tstruct M._WallCollisionData noattr)))
      as [-> | ]; [ | discriminate Htya ].
    split; [ exact Hoptne | split; reflexivity ].
  Qed.
  ...
  Lemma clcd_pres :
    forall s e le m0 tr le' m' out,
      ...
      (forall g, mem_id g clcd_sids = true -> e ! g = None) ->
      (forall g, mem_id g clcd_oc = true -> e ! g = None) ->
      e ! M._find_wall_collisions = None ->
      e ! interaction._gGlobalTimer = None ->
      clcd_chk s = true ->
      (forall b o, le ! mario_actions_airborne._m = Some (Vptr b o) ->
                   b = bm /\ o = Ptrofs.zero) ->
  ...
  Proof.
    ...
    induction Hexec;
      intros Hub_g Hub_i Hub_x Hub_s Hub_oc Hfwc Hubgt
             Hchk Htat Hact Hch HN HM HV HS.
    - (* Sskip *)
      exact (conj HV (conj HS (conj HM (conj HN
               (conj Htat (conj Hact Hch)))))).
    - (* Sassign: generic OR the wallCols.fld special store *)
      cbn [clcd_chk] in Hchk.
      apply orb_true_iff in Hchk as [Hg | Hsp].
      { eapply (clcd_generic _ _ _ _ _ _ _ _ Hlocal Hub_g Hub_i Hub_x Hub_s
                  Hub_oc Hubgt Hg Htat Hact Hch HN HM HV HS).
        eapply exec_Sassign; eauto. }
      destruct (clcd_sp_assign_shape _ _ Hsp) as (sattr & fld & ->).
      assert (Hex : exec_stmt function_entry2 (lp_ge lp) e le m
                      (Sassign (Efield (Evar M._wallCols
                                  (Tstruct M._WallCollisionData sattr)) fld tfloat)
                         a2) E0 le m' Out_normal)
        by (econstructor; eauto).
      destruct (Hlocal M._wallCols eq_refl) as (lblk & tyenv & Hbind & Hlb).
      assert (Hc0 : carried bm NoA MWF m)
        by (split; [ exact HV | split; [ exact HS
                   | split; [ exact HM | exact HN ] ] ]).
      destruct (local_field_assign_pres lp bm NoA MWF SafeB Hls_real
                  HNoA_of_MWF e M._wallCols M._WallCollisionData sattr tyenv
                  fld tfloat a2 le m _ _ m' _ lblk Mfloat32
                  Hbind Hlb eq_refl Hex Hc0) as (Hc' & _ & _).
    ...
    - (* Sset: generic only *)
      cbn [clcd_chk] in Hchk.
      apply orb_true_iff in Hchk as [Hg | Hsp];
        [ | cbn [clcd_sp_chk] in Hsp; discriminate Hsp ].
      eapply (clcd_generic _ _ _ _ _ _ _ _ Hlocal Hub_g Hub_i Hub_x Hub_s
                Hub_oc Hubgt Hg Htat Hact Hch HN HM HV HS).
      eapply exec_Sset; eauto.
    - (* Scall: generic census OR the find_wall_collisions special site *)
      cbn [clcd_chk] in Hchk.
      apply orb_true_iff in Hchk as [Hg | Hsp].
      { eapply (clcd_generic _ _ _ _ _ _ _ _ Hlocal Hub_g Hub_i Hub_x Hub_s
                  Hub_oc Hubgt Hg Htat Hact Hch HN HM HV HS).
        eapply exec_Scall; eauto. }
      destruct (clcd_sp_call_shape _ _ _ Hsp) as (Hoptne & -> & ->).
      assert (Hex : exec_stmt function_entry2 (lp_ge lp) e le m
                      (Scall optid
                         (Evar M._find_wall_collisions
                            (Tfunction (tptr (Tstruct M._WallCollisionData noattr)
                                        :: nil) tint cc_default))
                         (Eaddrof (Evar M._wallCols
                                     (Tstruct M._WallCollisionData noattr))
                            (tptr (Tstruct M._WallCollisionData noattr)) :: nil))
      ...
      assert (Hgate : forall vargs,
          eval_exprlist (lp_ge lp) e le m
            (Eaddrof (Evar M._wallCols (Tstruct M._WallCollisionData noattr))
               (tptr (Tstruct M._WallCollisionData noattr)) :: nil)
            (tptr (Tstruct M._WallCollisionData noattr) :: nil) vargs ->
      ...
      { intros vargs0 Hvl.
        ...
        [ match goal with
          | Hb : e ! M._wallCols = Some _ |- _ =>
              rewrite Hbind in Hb; injection Hb as <- _; exact Hlb
          end
        | match goal with
          | Hn : e ! M._wallCols = None |- _ =>
              rewrite Hbind in Hn; discriminate Hn
          end ]. }
      assert (HtatL : forall b o,
                 (set_opttemp optid vres le) ! mario_actions_airborne._m
                   = Some (Vptr b o) -> b = bm /\ o = Ptrofs.zero).
      { destruct optid as [oid | ]; cbn [set_opttemp]; [ | exact Htat ].
        assert (Hne : oid <> mario_actions_airborne._m)
          by (apply Hoptne; reflexivity).
        intros b o Hg.
        rewrite PTree.gso in Hg by exact (not_eq_sym Hne).
        exact (Htat b o Hg). }
      assert (Hc0 : carried bm NoA MWF m)
        by (split; [ exact HV | split; [ exact HS
                   | split; [ exact HM | exact HN ] ] ]).
      destruct (ol_scall_pres lp bm NoA MWF SafeB optid
                  M._find_wall_collisions
                  (tptr (Tstruct M._WallCollisionData noattr) :: nil)
                  tint cc_default
                  (Eaddrof (Evar M._wallCols (Tstruct M._WallCollisionData noattr))
                     (tptr (Tstruct M._WallCollisionData noattr)) :: nil)
                  e le m _ _ m' _ Hfwc Holcp_fwc Hgate Hex Hc0) as (Hc' & _).
      destruct Hc' as (HV' & HS' & HM' & HN').
      assert (HactL : act_inv nil (set_opttemp optid vres le))
        by (intros t' Hmem' x Hg'; discriminate Hmem').
      assert (HchL : chase_inv SafeB nil (set_opttemp optid vres le))
        by (intros t' Hmem' b o Hg'; discriminate Hmem').
      exact (conj HV' (conj HS' (conj HM' (conj HN'
               (conj HtatL (conj HactL HchL)))))).
    - (* Sbuiltin: rejected by BOTH arms *)
      cbn [clcd_chk] in Hchk.
      apply orb_true_iff in Hchk as [Hg | Hsp];
        [ cbn [clcd_gen wwalk_chk'] in Hg; discriminate Hg
        | cbn [clcd_sp_chk] in Hsp; discriminate Hsp ].
    - (* Sseq_1 *)
      cbn [clcd_chk] in Hchk.
      apply orb_true_iff in Hchk as [Hg | Hsp].
      { eapply (clcd_generic _ _ _ _ _ _ _ _ Hlocal Hub_g Hub_i Hub_x Hub_s
        ...
        eapply exec_Sseq_1; eauto. }
      apply andb_prop in Hsp as [H1 H2].
      destruct (IHHexec1 Hlocal Hub_g Hub_i Hub_x Hub_s Hub_oc Hfwc Hubgt
                  H1 Htat Hact Hch HN HM HV HS)
        as (HV1 & HS1 & HM1 & HN1 & Htat1 & Hact1 & Hch1).
      exact (IHHexec2 Hlocal Hub_g Hub_i Hub_x Hub_s Hub_oc Hfwc Hubgt
    ...
    - (* Sseq_2 *)
      ...
      { eapply (clcd_generic _ _ _ _ _ _ _ _ Hlocal Hub_g Hub_i Hub_x Hub_s
        ...
        eapply exec_Sseq_2; eauto. }
      apply andb_prop in Hsp as [H1 _].
      exact (IHHexec Hlocal Hub_g Hub_i Hub_x Hub_s Hub_oc Hfwc Hubgt
               H1 Htat Hact Hch HN HM HV HS).
    - (* Sifthenelse *)
      cbn [clcd_chk] in Hchk.
      apply orb_true_iff in Hchk as [Hg | Hsp].
  ...
  Lemma clcd_pin :
    (prog_defmap mario_actions_moving.prog) ! M._check_ledge_climb_down
    = Some (Gfun (Internal M.f_check_ledge_climb_down)).
  Proof. vm_compute. reflexivity. Qed.

  (* THE LEAF entry: mirrors call_pres_of_lwalk2 (alloc _wallCols/_floor,
     bind _m, the Hlocal constructor, free at exit); body goes to clcd_pres. *)
  Lemma mov_clcd_pres : body_pres lp NoA MWF bm M.f_check_ledge_climb_down.
  Proof.
    intros m0 vargs0 t0 mF vres0 Hmargf Hevf HN HM HV HS.
    inv Hevf.
    match goal with He : function_entry2 _ _ _ _ _ _ _ |- _ =>
    ...
    pose proof (alloc_variables_carried bm NoA MWF HMWF_alloc HNoA_of_MWF
                  _ _ _ _ _ _ Halloc Hc0) as Hca.
    destruct Hca as (HVa & HSa & HMa & HNa).
    pose proof (alloc_variables_hlocal lp bm SafeB m0 _ eloc _ clcd_lids
                  Halloc HV (HSafeValid m0 HM) (HGlobValid m0 HM)
                  ltac:(intros lid Hl; unfold clcd_lids, mem_id in Hl;
                        cbn [existsb] in Hl;
                        ...
                        | discriminate Hl ]))
      as Hlocal.
    assert (Hps : match fn_params M.f_check_ledge_climb_down with
                  | (i, ty) :: ps =>
                      (Pos.eqb i mario_actions_airborne._m
                       && proj_sumbool (type_eq ty tyMSp)
    ...
    assert (Hmarg : marg_ok bm vargs0)
      by (apply Hmargf; vm_compute; reflexivity).
    destruct (fn_params M.f_check_ledge_climb_down)
      as [| [i ty] ps ] eqn:Eps; [ discriminate Hps | ].
    apply andb_prop in Hps as [Hps Hnm].
    apply andb_prop in Hps as [Hi Hty].
    apply Pos.eqb_eq in Hi. subst i.
    ...
    { intros g Hg.
      ...
      apply PTree.gempty. }
    assert (Hub_i : forall g, mem_id g clcd_ids = true -> eloc ! g = None).
    { intros g Hg.
      rewrite (alloc_variables_unbound (lp_ge lp) m0 _ empty_env _ _ Halloc g)
        by (intro Hin; vm_compute in Hin;
            destruct Hin as [E | [E | []]]; subst g;
            vm_compute in Hg; discriminate Hg).
      apply PTree.gempty. }
    assert (Hub_x : forall g, mem_id g clcd_xids = true -> eloc ! g = None).
    { intros g Hg.
      rewrite (alloc_variables_unbound (lp_ge lp) m0 _ empty_env _ _ Halloc g)
        by (intro Hin; vm_compute in Hin;
            destruct Hin as [E | [E | []]]; subst g;
            vm_compute in Hg; discriminate Hg).
      apply PTree.gempty. }
    assert (Hub_s : forall g, mem_id g clcd_sids = true -> eloc ! g = None).
    { intros g Hg.
      rewrite (alloc_variables_unbound (lp_ge lp) m0 _ empty_env _ _ Halloc g)
        by (intro Hin; vm_compute in Hin;
            destruct Hin as [E | [E | []]]; subst g;
            vm_compute in Hg; discriminate Hg).
      apply PTree.gempty. }
    assert (Hub_oc : forall g, mem_id g clcd_oc = true -> eloc ! g = None).
    { intros g Hg.
      rewrite (alloc_variables_unbound (lp_ge lp) m0 _ empty_env _ _ Halloc g)
        by (intro Hin; vm_compute in Hin;
            destruct Hin as [E | [E | []]]; subst g;
            vm_compute in Hg; discriminate Hg).
      apply PTree.gempty. }
    assert (Hfwcn : eloc ! M._find_wall_collisions = None).
    { rewrite (alloc_variables_unbound (lp_ge lp) m0 _ empty_env _ _ Halloc
                 M._find_wall_collisions)
        by (intro Hin; vm_compute in Hin;
    ...
    { rewrite (alloc_variables_unbound (lp_ge lp) m0 _ empty_env _ _ Halloc
        ...
        by (intro Hin; vm_compute in Hin;
            destruct Hin as [E | [E | []]]; discriminate E).
      apply PTree.gempty. }
    destruct (clcd_pres _ _ _ _ _ _ _ _ Hlocal Hbody
                Hub_g Hub_i Hub_x Hub_s Hub_oc Hfwcn Hub_gt
                clcd_walk Htat0 Hact0 Hch0 HNa HMa HVa HSa)
      as (HVb & HSb & HMb & HNb & _ & _ & _).
    pose proof (blocks_of_env_bm lp bm m0 _ eloc _ Halloc HV) as Hforall.
    pose proof (free_list_carried_bm bm NoA MWF HMWF_free HNoA_of_MWF
                  (blocks_of_env (lp_ge lp) eloc) _ mF Hforall Hfree
  ...
  Lemma mov_clcd_row : call_pres lp bm NoA MWF M._check_ledge_climb_down.
  Proof.
    exact (call_pres_of_body lp bm NoA MWF HNoA_of_MWF
             mario_actions_moving.prog M._check_ledge_climb_down
             M.f_check_ledge_climb_down LO_mov clcd_pin mov_clcd_pres).
  Qed.

  (* ==================================================================== *)
  (* act_walking: THE LAST moving leaf.  HYBRID: A-gate kill (the same      *)
  (* set_jump_from_landing input&2 gate as act_decelerating) + the          *)
  (* vec3f_copy(_startPos, m->pos) wol special (dst = local _startPos       *)
  (* array, src = m->pos window) + generic census (12 ids, set_mario_action *)
  (* sids, safe-field window stores actionState/particleFlags/actionTimer,  *)
  (* local _startPos, the perform_ground_step switch).                      *)
  (* ==================================================================== *)
  Definition aw_lids : list ident := M._startPos :: nil.
  Definition aw_ids : list ident :=
    M._mario_drop_held_object :: M._should_begin_sliding
    :: M._begin_braking_action :: M._check_ground_dive_or_punch
    :: M._analog_stick_held_back :: M._update_walking_speed
    :: M._perform_ground_step :: M._set_mario_animation
    :: M._anim_and_audio_for_walk :: M._push_or_sidle_wall
    :: M._check_ledge_climb_down :: M._tilt_body_walking :: nil.
  Definition aw_sids : list ident := M._set_mario_action :: nil.

  Definition aw_gen (s : statement) : bool :=
    wwalk_chk' aw_lids nil nil nil nil nil false
      nil aw_ids nil nil nil aw_sids nil s.

  (* vec3f_copy(_startPos, m->pos): dst = local array, src = pos window *)
  Definition aw_v3f_src (a : expr) : bool :=
    match a with
    | Efield (Ederef (Etempvar p pty) sty) fld faty =>
        Pos.eqb p mario_actions_airborne._m
  ...
  Definition aw_v3f_chk (s : statement) : bool :=
    match s with
    | Scall None (Evar fid (Tfunction tyl rty cc)) al =>
        Pos.eqb fid M._vec3f_copy
        && proj_sumbool
             (type_eq (Tfunction tyl rty cc)
                (Tfunction (tptr tfloat :: tptr tfloat :: nil)
                   (tptr tvoid) cc_default))
        && match al with
           | Evar sp spty :: src :: nil =>
               Pos.eqb sp M._startPos
               && proj_sumbool (type_eq spty (tarray tfloat 3))
               && aw_v3f_src src
           | _ => false
           end
    | _ => false
  ...
  (* the A-gate (canonical set_jump_from_landing input&2 shape; ELSE generic) *)
  Definition aw_gate_chk (s : statement) : bool :=
    match s with
    | Ssequence (Sset t6 src) (Sifthenelse g _ sELSE) =>
        dec_input_src src
        && dec_a_guard t6 g
  ...
  Fixpoint aw_chk (s : statement) : bool :=
    aw_gen s
    || match s with
       | Ssequence s1 s2 => aw_gate_chk s || (aw_chk s1 && aw_chk s2)
       | Sifthenelse _ s1 s2 => aw_chk s1 && aw_chk s2
       | Sswitch _ ls => aw_chk_ls ls
       | Scall None _ _ => aw_v3f_chk s
       | _ => false
       end
  with aw_chk_ls (ls : labeled_statements) : bool :=
    match ls with
    | LSnil => true
    | LScons _ s rest => aw_chk s && aw_chk_ls rest
    end.

  Example aw_walk : aw_chk (fn_body M.f_act_walking) = true.
  Proof. vm_compute. reflexivity. Qed.

  (* ---- src window: m->pos as a 4-byte safe window (twin of pos_window_val,
     local copy at SIZE 4 in the moving namespace; consumer = the wol gate) ---- *)
  Lemma aw_pos_window4_val :
    forall e le m v,
      (forall b o, le ! mario_actions_airborne._m = Some (Vptr b o) ->
                   b = bm /\ o = Ptrofs.zero) ->
      eval_expr (lp_ge lp) e le m
        (Efield
           (Ederef (Etempvar mario_actions_airborne._m
                      (tptr (Tstruct mario._MarioState noattr)))
              (Tstruct mario._MarioState noattr))
           mario._pos (tarray tfloat 3)) v ->
  ...
  Proof.
    ...
    assert (Hfo : field_offset (prog_comp_env mario.prog)
                    mario._pos mario_state_members = OK (60, Full))
      by (vm_compute; reflexivity).
    assert (Hwin : store_window_ok 60 4 = true) by (vm_compute; reflexivity).
    inv Hev.
    match goal with
  ...
  (* ---- dst block: an array-typed local Evar decays By_reference to its env
     block (twin of eval_expr_Evar_local_block, array instead of struct) ---- *)
  Lemma eval_Evar_arr_local :
    forall e le m lid ety n v b tyenv,
      e ! lid = Some (b, tyenv) ->
      eval_expr (lp_ge lp) e le m (Evar lid (tarray ety n)) v ->
      exists ofs, v = Vptr b ofs.
  Proof.
    intros e le m lid ety n v b tyenv He Hev.
    inv Hev.
    match goal with
    | Hlv : eval_lvalue _ _ _ _ (Evar _ _) _ _ _ |- _ => inv Hlv
    end.
    - match goal with
      | Hb : e ! lid = Some (?l0, _) |- _ =>
          assert (Hbl : l0 = b) by congruence; subst l0
      end.
      match goal with
      | Hd : deref_loc (typeof _) _ _ _ _ _ |- _ => cbn [typeof] in Hd
      end.
      match goal with
      | Hd : deref_loc (tarray _ _) _ _ _ _ _ |- _ =>
          inv Hd;
          try (match goal with Har : access_mode (tarray _ _) = _ |- _ =>
                 cbn in Har; discriminate Har end);
          try (match goal with Hlb : load_bitfield (tarray _ _) _ _ _ _ _ _ _ |- _ =>
                 inv Hlb end)
      end.
      eexists. reflexivity.
    - match goal with
  ...
  (* ---- the generic-subtree row providers (12 ids + 1 sids) ---- *)
  Lemma aw_ids_rows : forall fid, mem_id fid aw_ids = true ->
      call_pres lp bm NoA MWF fid.
  Proof.
    intros fid H. unfold aw_ids in H. cbn [mem_id existsb] in H.
    apply orb_true_iff in H as [Hm | H];
      [ apply Pos.eqb_eq in Hm; subst fid; exact Hmdho | ].
    apply orb_true_iff in H as [Hm | H];
      [ apply Pos.eqb_eq in Hm; subst fid; exact mov_sbs_row | ].
    apply orb_true_iff in H as [Hm | H];
      [ apply Pos.eqb_eq in Hm; subst fid; exact mov_bba_row | ].
    apply orb_true_iff in H as [Hm | H];
      [ apply Pos.eqb_eq in Hm; subst fid; exact mov_cgdop_row | ].
    apply orb_true_iff in H as [Hm | H];
    ...
    apply orb_true_iff in H as [Hm | H];
      [ apply Pos.eqb_eq in Hm; subst fid; exact mov_sma_row | ].
    apply orb_true_iff in H as [Hm | H];
      [ apply Pos.eqb_eq in Hm; subst fid; exact mov_aaw_row | ].
    apply orb_true_iff in H as [Hm | H];
      [ apply Pos.eqb_eq in Hm; subst fid; exact mov_psw_row | ].
    apply orb_true_iff in H as [Hm | H];
      [ apply Pos.eqb_eq in Hm; subst fid; exact mov_clcd_row | ].
    apply orb_true_iff in H as [Hm | H];
      [ apply Pos.eqb_eq in Hm; subst fid; exact mov_tbw_row | discriminate H ].
  Qed.

  Lemma aw_sids_rows : forall fid, mem_id fid aw_sids = true ->
      call_pres_act lp bm NoA MWF fid.
  Proof.
    intros fid H. unfold aw_sids in H. cbn [mem_id existsb] in H.
    apply orb_true_iff in H as [Hm | H];
      [ apply Pos.eqb_eq in Hm; subst fid; exact Hsmact | discriminate H ].
  Qed.

  (* ---- the generic-subtree discharger: ONE wwalk_pres call (aw census) ---- *)
  Lemma aw_generic :
    forall s e le m0 tr le' m' out,
      (forall lid, mem_id lid aw_lids = true ->
         exists lblk tyenv, e ! lid = Some (lblk, tyenv) /\
                            local_blk lp bm SafeB lblk) ->
      (forall g, mem_id g stored_globals = true -> e ! g = None) ->
      (forall g, mem_id g aw_ids = true -> e ! g = None) ->
      (forall g, mem_id g aw_sids = true -> e ! g = None) ->
      e ! interaction._gGlobalTimer = None ->
      aw_gen s = true ->
      (forall b o, le ! mario_actions_airborne._m = Some (Vptr b o) ->
                   b = bm /\ o = Ptrofs.zero) ->
      ...
      act_inv nil le' /\ chase_inv SafeB nil le'.
  Proof.
    intros s e le m0 tr le' m' out Hlocal Hub_g Hub_i Hub_s Hubgt
           Hchk Htat Hact Hch HN HM HV HS Hexec.
    destruct (wwalk_pres lp LO_mario bm NoA MWF HNoA_of_MWF HMWF_window
                HMWF_glob HMWF_act SafeB HSafeNotBm HchaseRoot HMWF_chase
                HMWF_root HMWF_sglob HchaseStep HMWF_chase_safe
                false nil aw_ids nil nil nil aw_sids nil
                aw_lids nil nil nil nil nil
                aw_ids_rows
                (fun fid HH => match Bool.diff_false_true HH with end)
                (fun fid HH => match Bool.diff_false_true HH with end)
                aw_sids_rows
  ...
  (* ---- switch-selection transfer (mirror of the dec switch lemmas) ---- *)
  Lemma aw_chk_ls_seq : forall sl,
      aw_chk_ls sl = true ->
      aw_chk (seq_of_labeled_statement sl) = true.
  Proof.
    induction sl as [| o s sl0 IH]; intros H.
    - reflexivity.
    - cbn in H. apply andb_prop in H as [H1 H2].
      cbn [seq_of_labeled_statement aw_chk].
      apply orb_true_iff. right.
      apply orb_true_iff. right.
      rewrite H1, (IH H2). reflexivity.
  Qed.
  Lemma aw_chk_ls_case : forall n sl sl',
      aw_chk_ls sl = true ->
      select_switch_case n sl = Some sl' ->
      aw_chk_ls sl' = true.
  Proof.
    ...
    - cbn in H. apply andb_prop in H as [H1 H2].
      ...
      + exact (IH sl' H2 Hsel).
  Qed.
  Lemma aw_chk_ls_default : forall sl,
      aw_chk_ls sl = true ->
      aw_chk_ls (select_switch_default sl) = true.
  Proof.
    induction sl as [| o s sl0 IH]; intros H.
    - exact H.
    - cbn in H. apply andb_prop in H as [H1 H2].
      ...
      + rewrite H1, H2. reflexivity.
  Qed.
  Lemma aw_chk_select : forall n sl,
      aw_chk_ls sl = true ->
      aw_chk (seq_of_labeled_statement (select_switch n sl)) = true.
  Proof.
    intros n sl H. apply aw_chk_ls_seq.
    unfold select_switch.
    destruct (select_switch_case n sl) eqn:E.
    - exact (aw_chk_ls_case _ _ _ H E).
    - exact (aw_chk_ls_default _ H).
  Qed.

  (* ---- the A-gate decode (mirror dec_gate_shape, no cact conjunct) ---- *)
  Lemma aw_gate_shape :
    forall s1 s2,
      aw_gate_chk (Ssequence s1 s2) = true ->
      exists t6 sTHEN sELSE,
        s1 = Sset t6 (Efield (Ederef (Etempvar mario_actions_airborne._m
                                (tptr (Tstruct mario._MarioState noattr)))
                        (Tstruct mario._MarioState noattr))
        ...
        aw_gen sELSE = true.
  Proof.
    intros s1 s2 H. unfold aw_gate_chk in H.
    destruct s1 as [ | e1 e2 | t6 src | oi ef alq | oi2 ef2 tyl alq2
                   | sa sb | ga sa sb | sla slb | | | oret | swa swl
                   | lid ls | gid ];
      try discriminate H.
    ...
    split; [ reflexivity | ].
    split; [ reflexivity | ].
    split; [ exact Hneq | exact HgenE ].
  Qed.

  (* ---- the KILL CONSUMPTION (mirror dec_gate_pres; chase_inv nil, no nptr,
     generic ELSE via aw_generic) ---- *)
  Lemma aw_gate_pres :
    forall t6 sTHEN sELSE e le m0 tr le' m' out,
      Pos.eqb t6 mario_actions_airborne._m = false ->
      aw_gen sELSE = true ->
      ...
      act_inv nil le' /\ chase_inv SafeB nil le'.
  Proof.
    intros t6 sTHEN sELSE e le m0 tr le' m' out Hneq HgenE Hexec
           Hlocal Hub_g Hub_i Hub_s Hubgt Htat Hact Hch HN HM HV HS.
    assert (Hle_m : le ! mario_actions_airborne._m
                    = Some (Vptr bm Ptrofs.zero)).
    { inv Hexec.
      - match goal with
    ...
    assert (Hact' : act_inv nil (PTree.set t6 (Vint vi) le))
      by (intros t' Hmem' x Hg'; discriminate Hmem').
    assert (Hch' : chase_inv SafeB nil (PTree.set t6 (Vint vi) le))
      by (intros t' Hmem' b o Hg'; discriminate Hmem').
    exact (aw_generic _ _ _ _ _ _ _ _ Hlocal Hub_g Hub_i Hub_s Hubgt
             HgenE Htat' Hact' Hch' HN HM HV HS Helse).
  Qed.

  (* ---- decode of the special Scall (vec3f_copy(_startPos, m->pos)) ---- *)
  Lemma aw_v3f_shape :
    forall a al, aw_v3f_chk (Scall None a al) = true ->
      a = Evar M._vec3f_copy
            (Tfunction (tptr tfloat :: tptr tfloat :: nil)
               (tptr tvoid) cc_default)
      /\ al = Evar M._startPos (tarray tfloat 3)
              :: Efield (Ederef (Etempvar mario_actions_airborne._m
                            (tptr (Tstruct mario._MarioState noattr)))
                    (Tstruct mario._MarioState noattr))
                   mario._pos (tarray tfloat 3) :: nil.
  Proof.
    intros a al H. cbn [aw_v3f_chk] in H.
    destruct a as [ | | | | fid fty | | | | | | | | | ]; try discriminate H.
    destruct fty as [ | | | | | | tyl rty cc | | ]; try discriminate H.
    apply andb_true_iff in H as [H Hal].
    ...
    destruct (type_eq (Tfunction tyl rty cc)
                (Tfunction (tptr tfloat :: tptr tfloat :: nil)
                   (tptr tvoid) cc_default)) as [E | ]; [ | discriminate Hfty ].
    injection E as <- <- <-.
    destruct al as [ | x al0 ]; try discriminate Hal.
    destruct x as [ | | | | sp spty | | | | | | | | | ]; try discriminate Hal.
    destruct al0 as [ | src al1 ]; try discriminate Hal.
    destruct al1 as [ | y al2 ]; [ | discriminate Hal ].
    apply andb_true_iff in Hal as [Hal Hsrc].
    apply andb_true_iff in Hal as [Hsp Hspty].
    apply Pos.eqb_eq in Hsp; subst sp.
    destruct (type_eq spty (tarray tfloat 3)) as [-> | ]; [ | discriminate Hspty ].
    unfold aw_v3f_src in Hsrc.
    destruct src as [ | | | | | | | | | | | base fld faty | | ];
      try discriminate Hsrc.
    destruct base as [ | | | | | | pbase sty | | | | | | | ];
      try discriminate Hsrc.
    destruct pbase as [ | | | | | p pty | | | | | | | | ];
      try discriminate Hsrc.
    apply andb_true_iff in Hsrc as [Hsrc Hfaty].
    apply andb_true_iff in Hsrc as [Hsrc Hfld].
    apply andb_true_iff in Hsrc as [Hsrc Hsty].
    apply andb_true_iff in Hsrc as [Hp Hpty].
    apply Pos.eqb_eq in Hp; subst p.
    apply Pos.eqb_eq in Hfld; subst fld.
    destruct (type_eq pty (tptr (Tstruct mario._MarioState noattr)))
      as [-> | ]; [ | discriminate Hpty ].
    destruct (type_eq sty (Tstruct mario._MarioState noattr))
      as [-> | ]; [ | discriminate Hsty ].
    destruct (type_eq faty (tarray tfloat 3)) as [-> | ]; [ | discriminate Hfaty ].
    split; reflexivity.
  Qed.
  ...
  Proof.
    intros s e le m0 tr le' m' out Hlocal Hexec.
    induction Hexec;
      intros Hub_g Hub_i Hub_s Hv3f Hubgt
             Hchk Htat Hact Hch HN HM HV HS.
    - (* Sskip *)
      exact (conj HV (conj HS (conj HM (conj HN
               (conj Htat (conj Hact Hch)))))).
    - (* Sassign: generic only *)
      cbn [aw_chk] in Hchk.
      apply orb_true_iff in Hchk as [Hg | Hsp]; [ | discriminate Hsp ].
      eapply (aw_generic _ _ _ _ _ _ _ _ Hlocal Hub_g Hub_i Hub_s Hubgt
                Hg Htat Hact Hch HN HM HV HS);
        eapply exec_Sassign; eauto.
    - (* Sset: generic only *)
      cbn [aw_chk] in Hchk.
      ...
      eapply (aw_generic _ _ _ _ _ _ _ _ Hlocal Hub_g Hub_i Hub_s Hubgt
                Hg Htat Hact Hch HN HM HV HS);
        eapply exec_Sset; eauto.
    - (* Scall: generic census OR the vec3f_copy(_startPos, m->pos) special *)
      cbn [aw_chk] in Hchk.
      apply orb_true_iff in Hchk as [Hg | Hsp].
      { eapply (aw_generic _ _ _ _ _ _ _ _ Hlocal Hub_g Hub_i Hub_s Hubgt
                  Hg Htat Hact Hch HN HM HV HS);
          eapply exec_Scall; eauto. }
      destruct optid as [oid | ]; [ discriminate Hsp | ].
      destruct (aw_v3f_shape _ _ Hsp) as (-> & ->).
      assert (Hex : exec_stmt function_entry2 (lp_ge lp) e le m
                      (Scall None
                         (Evar M._vec3f_copy
                            (Tfunction (tptr tfloat :: tptr tfloat :: nil)
                               (tptr tvoid) cc_default))
                         (Evar M._startPos (tarray tfloat 3)
                          :: Efield (Ederef (Etempvar mario_actions_airborne._m
                                       (tptr (Tstruct mario._MarioState noattr)))
                                 (Tstruct mario._MarioState noattr))
                               mario._pos (tarray tfloat 3) :: nil))
                      t (set_opttemp None vres le) m' Out_normal)
        by (eapply exec_Scall; eauto).
      destruct (Hlocal M._startPos eq_refl) as (lblk & tyenv & Hbind & Hlb).
      assert (Hgate : forall vargs,
          eval_exprlist (lp_ge lp) e le m
            (Evar M._startPos (tarray tfloat 3)
             :: Efield (Ederef (Etempvar mario_actions_airborne._m
      ...
      { intros vargs0 Hvl.
        ...
        destruct (aw_pos_window4_val _ _ _ _ Htat Hev_b) as (o2 & Ev2 & Hwin2).
        subst v1b.
        destruct (eval_Evar_arr_local _ _ _ _ _ _ _ _ _ Hbind Hev_a)
          as (o1 & Ev1). subst v1a.
        intros bb oo Hin; cbn in Hin.
      ...
      assert (Hc0 : carried bm NoA MWF m)
        by (split; [ exact HV | split; [ exact HS
                   | split; [ exact HM | exact HN ] ] ]).
      destruct (wol_scall_pres lp bm NoA MWF SafeB None
                  M._vec3f_copy
                  ...
                  (Evar M._startPos (tarray tfloat 3)
                   :: Efield (Ederef (Etempvar mario_actions_airborne._m
                          ...
                          (Tstruct mario._MarioState noattr))
                        mario._pos (tarray tfloat 3) :: nil)
                  e le m _ _ m' _ Hv3f Hwolcp_v3f Hgate Hex Hc0) as (Hc' & _).
      destruct Hc' as (HV' & HS' & HM' & HN').
      exact (conj HV' (conj HS' (conj HM' (conj HN'
               (conj Htat (conj Hact Hch)))))).
    - (* Sbuiltin: rejected by both arms *)
      cbn [aw_chk] in Hchk.
      apply orb_true_iff in Hchk as [Hg | Hsp];
        [ unfold aw_gen in Hg; cbn [wwalk_chk'] in Hg; discriminate Hg
        | discriminate Hsp ].
    - (* Sseq_1: generic, THE A-GATE, or recurse *)
      cbn [aw_chk] in Hchk.
      apply orb_true_iff in Hchk as [Hg | Hrest].
      { eapply (aw_generic _ _ _ _ _ _ _ _ Hlocal Hub_g Hub_i Hub_s Hubgt
          ...
          eapply exec_Sseq_1; eauto. }
      apply orb_true_iff in Hrest as [Hgate | Hand].
      + destruct (aw_gate_shape _ _ Hgate)
          as (t6 & sTHEN & sELSE & -> & -> & Hneq & HgenE).
        eapply aw_gate_pres; try eassumption.
        eapply exec_Sseq_1; eauto.
      + apply andb_prop in Hand as [H1 H2].
        destruct (IHHexec1 Hlocal Hub_g Hub_i Hub_s Hv3f Hubgt H1
                    Htat Hact Hch HN HM HV HS)
          as (HV1 & HS1 & HM1 & HN1 & Htat1 & Hact1 & Hch1).
        exact (IHHexec2 Hlocal Hub_g Hub_i Hub_s Hv3f Hubgt H2
    ...
    - (* Sseq_2 *)
      ...
      + apply andb_prop in Hand as [H1 _].
        exact (IHHexec Hlocal Hub_g Hub_i Hub_s Hv3f Hubgt H1
                 Htat Hact Hch HN HM HV HS).
    - (* Sifthenelse *)
      cbn [aw_chk] in Hchk.
      apply orb_true_iff in Hchk as [Hg | Hsp].
    ...
    - (* Sswitch: generic OR the case-selection descent *)
      ...
      { eapply (aw_generic _ _ _ _ _ _ _ _ Hlocal Hub_g Hub_i Hub_s Hubgt
                  Hg Htat Hact Hch HN HM HV HS);
          eapply exec_Sswitch; eauto. }
      exact (IHHexec Hlocal Hub_g Hub_i Hub_s Hv3f Hubgt
               (aw_chk_select _ _ Hls) Htat Hact Hch HN HM HV HS).
  Qed.

  Lemma aw_pin :
    (prog_defmap mario_actions_moving.prog) ! M._act_walking
    = Some (Gfun (Internal M.f_act_walking)).
  Proof. vm_compute. reflexivity. Qed.

  (* THE LEAF entry: mirrors mov_clcd_pres (alloc _startPos, bind _m, the
     Hlocal constructor, free at exit); body goes to aw_pres. *)
  Lemma mov_aw_pres : body_pres lp NoA MWF bm M.f_act_walking.
  Proof.
    intros m0 vargs0 t0 mF vres0 Hmargf Hevf HN HM HV HS.
    inv Hevf.
    match goal with He : function_entry2 _ _ _ _ _ _ _ |- _ =>
    ...
    pose proof (alloc_variables_carried bm NoA MWF HMWF_alloc HNoA_of_MWF
                  _ _ _ _ _ _ Halloc Hc0) as Hca.
    destruct Hca as (HVa & HSa & HMa & HNa).
    pose proof (alloc_variables_hlocal lp bm SafeB m0 _ eloc _ aw_lids
                  Halloc HV (HSafeValid m0 HM) (HGlobValid m0 HM)
                  ltac:(intros lid Hl; unfold aw_lids, mem_id in Hl;
                        cbn [existsb] in Hl;
                        apply Bool.orb_true_iff in Hl;
                        destruct Hl as [Hm1 | Hl];
                        [ apply Pos.eqb_eq in Hm1; subst lid;
                        ...
                        | discriminate Hl ]))
      as Hlocal.
    assert (Hps : match fn_params M.f_act_walking with
                  | (i, ty) :: ps =>
                      (Pos.eqb i mario_actions_airborne._m
                       && proj_sumbool (type_eq ty tyMSp)
                       && negb (mem_id mario_actions_airborne._m
    ...
    assert (Hmarg : marg_ok bm vargs0)
      by (apply Hmargf; vm_compute; reflexivity).
    destruct (fn_params M.f_act_walking)
      as [| [i ty] ps ] eqn:Eps; [ discriminate Hps | ].
    apply andb_prop in Hps as [Hps Hnm].
    apply andb_prop in Hps as [Hi Hty].
    apply Pos.eqb_eq in Hi. subst i.
    ...
    { intros g Hg.
      rewrite (alloc_variables_unbound (lp_ge lp) m0 _ empty_env _ _ Halloc g)
        by (intro Hin; vm_compute in Hin;
            destruct Hin as [E | []]; subst g;
            vm_compute in Hg; discriminate Hg).
      apply PTree.gempty. }
    assert (Hub_i : forall g, mem_id g aw_ids = true -> eloc ! g = None).
    { intros g Hg.
      rewrite (alloc_variables_unbound (lp_ge lp) m0 _ empty_env _ _ Halloc g)
        by (intro Hin; vm_compute in Hin;
            destruct Hin as [E | []]; subst g;
            vm_compute in Hg; discriminate Hg).
      apply PTree.gempty. }
    assert (Hub_s : forall g, mem_id g aw_sids = true -> eloc ! g = None).
    { intros g Hg.
      rewrite (alloc_variables_unbound (lp_ge lp) m0 _ empty_env _ _ Halloc g)
        by (intro Hin; vm_compute in Hin;
            destruct Hin as [E | []]; subst g;
            vm_compute in Hg; discriminate Hg).
      apply PTree.gempty. }
    assert (Hv3fn : eloc ! M._vec3f_copy = None).
    { rewrite (alloc_variables_unbound (lp_ge lp) m0 _ empty_env _ _ Halloc
                 M._vec3f_copy)
        by (intro Hin; vm_compute in Hin;
    ...
    { rewrite (alloc_variables_unbound (lp_ge lp) m0 _ empty_env _ _ Halloc
        ...
        by (intro Hin; vm_compute in Hin;
            destruct Hin as [E | []]; discriminate E).
      apply PTree.gempty. }
    destruct (aw_pres _ _ _ _ _ _ _ _ Hlocal Hbody
                Hub_g Hub_i Hub_s Hv3fn Hub_gt
                aw_walk Htat0 Hact0 Hch0 HNa HMa HVa HSa)
      as (HVb & HSb & HMb & HNb & _ & _ & _).
    pose proof (blocks_of_env_bm lp bm m0 _ eloc _ Halloc HV) as Hforall.
    pose proof (free_list_carried_bm bm NoA MWF HMWF_free HNoA_of_MWF
                  (blocks_of_env (lp_ge lp) eloc) _ mF Hforall Hfree
  ...
  Lemma mov_aw_row : call_pres lp bm NoA MWF M._act_walking.
  Proof.
    exact (call_pres_of_body lp bm NoA MWF HNoA_of_MWF
             mario_actions_moving.prog M._act_walking
             M.f_act_walking LO_mov aw_pin mov_aw_pres).
  Qed.
  ...
  Proof.
    ...
    (* 2: act_walking -- WALKED (A-gate kill + vec3f_copy wol special) *)
    apply orb_true_iff in H as [Hm | H].
    { apply Pos.eqb_eq in Hm; subst fid.
      rewrite aw_pin in Hdm. injection Hdm as <-. exact mov_aw_pres. }
    ...
    (* 8: act_riding_shell_ground -- WALKED (gwspf-hybrid uss + tbgs chase) *)
    apply orb_true_iff in H as [Hm | H].
    { apply Pos.eqb_eq in Hm; subst fid.
      rewrite arsg_pin in Hdm. injection Hdm as <-. exact mov_arsg_pres. }
    ...
    (* 17: act_dive_slide -- WALKED (csa-hybrid recognizer-walk) *)
    apply orb_true_iff in H as [Hm | H].
    { apply Pos.eqb_eq in Hm; subst fid.
      rewrite ads_pin in Hdm. injection Hdm as <-. exact mov_ads_pres. }
