...
Definition area3_upward_floor_y_max : Z := 384.
Definition hand_collision_top_max : Z := 507.
Definition upward_travel_max : Z := 288.

Definition support_ceiling (rank : hand_rank) : Z :=
  match rank with
  | FirstHand => area3_upward_floor_y_max
  | SecondHand => 1179
  end.
...
Definition global_height_ceiling : Z := 1467.
...
Lemma ceiling_values :
  support_ceiling FirstHand = 384 /\
  height_ceiling FirstHand = 672 /\
  support_ceiling SecondHand = 1179 /\
...
Proof. destruct rank; cbv [height_ceiling support_ceiling global_height_ceiling
  upward_travel_max area3_upward_floor_y_max]; lia. Qed.
