...
rom="${1:?usage: run_jp_platform_probe.sh /path/to/authentic/baserom.jp.z64 [output-directory] [both|natural|right-injected|left-injected]}"
...
run_case() {
    ...
    gcc -shared -fPIC -std=c99 -Wall -Wextra -Werror -O2 \
        ...
        -DHAND_SIDE="$side" \
        -DLOG_ALLOCATION_CENSUS="${LOG_ALLOCATION_CENSUS:-0}" \
        -DSKIP_AREA2_MACROS="${SKIP_AREA2_MACROS:-0}" \
...
case "$case_selector" in
    both)
        run_case natural_null 0 1
        run_case injected_right_hand_slot 1 1
        run_case injected_left_hand_slot 1 -1
        ;;
    natural)
        run_case natural_null 0 1
        ;;
...
if [[ -f "$output_dir/natural_null/raw.log" \
   && -f "$output_dir/injected_right_hand_slot/raw.log" \
   && -f "$output_dir/injected_left_hand_slot/raw.log" ]]; then
    python3 "$script_dir/analyze_jp_platform_probe.py" \
        "$output_dir/results" \
        "$output_dir/natural_null/raw.log" \
        "$output_dir/injected_right_hand_slot/raw.log" \
        "$output_dir/injected_left_hand_slot/raw.log"
fi
