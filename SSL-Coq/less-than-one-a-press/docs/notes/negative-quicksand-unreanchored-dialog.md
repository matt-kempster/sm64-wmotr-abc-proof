...
| A forged action, timer, descriptor, callback, or input | No concrete clean SSL writer has been found.  The initialized stock interaction callback is closed, and the strengthened writable-table audit accepts only four terminal reads per version, with no store/address handoff or initializer alias.  The private relation is now constructed at selected-program initialization and carried through every classified finite Clight run, including outside calls; only the reached-step classifier remains for this seed.  OOB/ACE producers are outside the current model. |
...
- `NegativeDepthInteractionClosure.v` follows all 29 distinct initialized
  ...
  private-block injection is now constructed at initialization and carried by
  a finite-run theorem; the remaining step is classifying each reached linked
  execution step.
