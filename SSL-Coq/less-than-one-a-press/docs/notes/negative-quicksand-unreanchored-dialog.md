...
| A forged action, timer, descriptor, callback, or input | No concrete clean SSL writer has been found.  The initialized stock interaction callback is closed, and the writable-table audit now proves that named controller-driven source operations only read the three action tables and have no named mutation producer.  A table seed must exhibit one exact valid alias or reached outside-call write; OOB/ACE producers are outside the current model. |
...
The bounded forgery and writable-table audits make that residual more precise.  All nine landing descriptors, `sInteractionHandlers`, and the two knockback-action tables are writable globals, but the bilateral generated programs contain no direct assignment to them.  Each landing-descriptor address appears only in its matching wrapper; the handler table is used only by its stock dispatcher; and each knockback table is used only by `determine_knockback_action`.  The three action tables occupy exactly `320` writable bytes, yet named ordinary controller operations write zero bytes and merely select bounded reads.  This is not because a mutation would be harmless: one four-byte knockback entry can encode any 32-bit action, including `ACT_LONG_JUMP`, and one compatible handler-pointer word can redirect the coin or pole row to another stock automatic handler; a two-word handler-plus-knockback construction can conditionally request long jump.  No named source producer for either write exists.  Corrupting only the action timer is also insufficient for a four-frame landing because the preincrement cancel returns before timer 4 or 5 reaches the body, and the landing function-pointer call remains below `m->input & INPUT_A_PRESSED`.  The 29 initialized handlers, 23 direct action literals, four local selectors, Snufit's 18 bounded entries, and Bully's five outcomes are all non-target.  A surviving in-model table seed must now supply the exact valid alias into the private block or a reached outside-call effect; changing Mario's action separately still requires a same-block byte-overlapping store.  See the [writable action-table audit](writable-action-table-mutation.md).  N64 flat-address OOB behavior remains deferred to a machine model.
