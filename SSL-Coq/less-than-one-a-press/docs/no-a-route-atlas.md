...
**What is already known.** The supplied setup works, but it needs at least a `960`-unit gap between Mario's displayed and collision heights (`1010` at the warp center), and a second missed floor check is fatal; ordinary creation, Mario's normal behavior and callbacks, stock display offsets, and another object's slot do not make that gap, while a deliberately non-stock `+1160` offset confirms that the geometry works.  A hash-checked original-JP run follows one uninterrupted accepted level-select entry and records exactly 19 harmless writes, ending with Mario in slot 67, both game pointers selecting him, his normal behavior and player-list membership installed, the two dangerous fields safe, and the flag value `0x100`.  All 73 allocations succeed, so cleanup, unloading, and the source-sound call never happen; the ordinary area-loading sound call happens once, with a recorded stack safely outside the object pool.  A new all-path check of the real JP instructions covers that reached sound routine and every helper it can call, proves that its writes can touch only sound data, music data, or that safe stack, and also proves that the separate square-root routine cannot write memory at all; therefore none of the three formerly abstract pre-entry calls can alter Mario's slot or protected values, without needing an IDO-to-Clight bridge.  Later gameplay still needs per-step preservation through timer 131, and negative quicksand plus dialog remains a separate constrained alternative.

**What closes it.** Begin at the now-accepted safe machine endpoint and classify every reachable later game step, callback, and outside call through timer 131 as keeping Mario in the same slot and player list with the same normal behavior and safe protected values, or as making a specifically checked harmless write; the ordinary castle entry, an IDO-to-Clight entry bridge, and all three pre-entry outside-call effects are no longer required under this boundary.  Completing that later timeline would rule out every remaining successful in-bounds corruption-style version in the chosen model, while the first later write or identity change that fails the check would identify a real producer.  Negative quicksand plus dialog still needs both an unusual negative seed and an X/Z transport to the warp, while out-of-bounds writes, arbitrary code execution, and DMA require a separate retail-machine model; only a surviving producer warrants continuing to the failed lookup, top-owned retry, top activation or explosion, JP displacement, and the separate Act-3 and Act-6 continuations.
