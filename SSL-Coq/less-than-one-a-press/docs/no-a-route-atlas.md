...
| 9A | Direct Area-2 gates | [Use the 100-coin star to interrupt an action at the gate](#route-rank-9a) | Low; fixed coin layouts are too far from the shaft, and Goomba drops are the concrete mobile producer still needing clean placement; the airborne ledge setup and coin-star budget remain open |
...
**What is already known.** Collecting the star while attached to the pole selects the standing dance, which loses the handstand elevation; the ledge trick also rejects a rising Mario. An airborne pickup still has a promising local wall/ledge calculation, but no clean placement or complete crossing. The stock layouts of all 41 fixed coin actors, including formations and switched blue coins, are away from the shaft. Goomba drops are the concrete mobile producer: Coq checks their launch operation, but getting a drop high enough and close enough remains open. The pyramid's boxes contain 1-ups, and the helper that releases a coin at Mario belongs to Whomp, absent from the stock pyramid. The lower route spends the star earlier and the upper Act-3 plan needs it later. See the [gate proof](notes/rank9a-coin-star-gate.md) and [ordinary coin audit](notes/rank9a-ordinary-coin-producers.md).

**What closes it.** First follow a normal Goomba defeat and its released coin through the actual floors, walls, moving supports and timing to a useful star placement, or show that every reachable drop stays too low or too far away; any different coin source or changed fixed-coin position needs its own clean history. Then collect the star while airborne and reach a suitable ledge check while level or descending, following every collision, action change, landing and target-star continuation in one zero-A run. The earlier climb and later route must not spend the same star twice. An isolated coin toss, an artificially placed star, or the local ring calculation does not close the route.
