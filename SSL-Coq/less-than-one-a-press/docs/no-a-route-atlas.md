...
| 9 | Downstream collection | [Upper Act-3 100-coin/star-dance itinerary](#route-rank-9) | Low-medium conditional continuation; a concrete static ledge-and-landing window and nearby coin now fit, but live placement and timing remain unproved |
...
**What is already known.** Collecting the star while attached to the pole starts a standing dance that loses the handstand elevation; a rising Mario also cannot grab the proposed ledge. An airborne pickup has a useful local calculation but no clean installation. All 41 fixed coin actors start away from the shaft, the boxes contain 1-ups, and the coin-at-Mario helper belongs to Whomp, absent from the pyramid. Goomba drops remain possible producers, but the [new flight bound](notes/rank9a-goomba-coin-flight.md) shows that the finishing attack, coin toss, pauses and checked lower-floor bounces are insufficient even if the enemy reaches the conditional Spindel raising station. Further raising, higher selected supports and Mario moving before the star chooses its position remain legitimate gameplay-glitch possibilities, not disproved routes. The earlier climb and later star collection must also share the one-use 100-coin star. See the [gate proof](notes/rank9a-coin-star-gate.md) and [coin-source audit](notes/rank9a-ordinary-coin-producers.md).

**What closes it.** Construct the missing height gain through a higher Goomba attack pose, a higher floor or moving support, another defined movement glitch, or Mario's movement between coin contact and the star choosing its position, then follow that same coin and star through the required location, timing, airborne pickup, ledge catch and target-star collection without A. Alternatively, show that every reached update fits the checked flight cases and that no later movement rescues the placement. A different coin source needs its own clean history, and the earlier climb and later route cannot spend the same star twice. Neither an isolated toss nor an artificially placed enemy proves installation.
...
**What is already known.** The [new proof and timing check](notes/rank10a-ground-pound-moving-geometry.md) confirm a real but conditional height window: if Mario could start a normal ground pound at the elevator floor's height while it steadily descends, the fifteen-update pause could leave him 260 units above it, exceeding the checked wall-height cutoff of 231. The planned lifts total 110 units, every headroom outcome has a checked height bound, and startup stops sideways speed; B or Z cannot turn that pause into an escape. Wall and floor checks also continue before the action each frame. Ordinary freefall and ceiling-hanging actions can request ground pound, but the tested rollout, jump-kick, dive and initial upper-entry drop cannot do so directly. Following the elevator's ten-unit descent does not by itself create freefall. No clean eligible entry, useful departure or star collection has been established.
...
**What is already known.** This is the transcript's upper route, not the different lower route shown in the supplied video. The [new upper-platform investigation](notes/rank9-upper-star-dance.md) finds a concrete star-fall ledge catch behind the platform: an offline test of the static walls and floors succeeds for 50 nearby starting heights and lands Mario on the platform on the following update. Coq checks the source geometry, the operations that record the caught floor, and the caller's decision not to undo the ledge result; the actual live collision queries still need proof. A normal coin in the nearby vertical row also fits a proposed placement, but collecting it at the right time and delaying the star pickup until Mario reaches the catch window remain unproved. Standing on the flat platform still leaves the familiar 75-unit star gap, and no continuous upper-route replay or final pickup has been established.

**What closes it.** Reach the nearby coin with 99 coins and the 100-coin reward still available, place its star, and use stored rollout speed and ground-pound timing to collect that star inside the checked rear-wall catch window without an earlier pickup. Then follow the real walls, floors, moving objects and animation through the catch, landing, dance, final rollout and Act-3 pickup in one continuous continuation without another A press; the static test alone does not supply those live steps or the earlier elevator escape.
