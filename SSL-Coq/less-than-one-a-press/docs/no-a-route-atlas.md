...
**What is already known.** The supplied setup works, but it needs at least a `960`-unit gap between Mario's displayed and collision heights (`1010` at the warp center), and a second missed floor check is fatal; ordinary creation, Mario's normal behavior, normal callbacks, stock display offsets, and reuse of another object slot do not make that gap, while an intentionally non-stock `+1160` offset confirms that the geometry itself works.  A hash-checked original-JP run follows one uninterrupted accepted level-select entry—clear old objects, load Area 1, spawn its objects, separately spawn and initialize Mario, then perform Mario's first update—and records exactly 19 harmless writes: Mario is slot 67, both game pointers select him, his normal behavior and player-list membership are installed, allocation writes zero to both dangerous fields, and his first behavior writes the safe flag value `0x100`.  A second exact receipt resolves one misleading item from the conservative call list: all 73 allocations succeed, so the allocation-failure cleanup, object unloading, and its source-sound call are never reached and need no effect assumption; the normal area-loading sound call is reached once, while the surface square-root call was not tested by this receipt.  Coq checks these counts and the relevant machine-code branches and packages them with the safe endpoint, so no ordinary castle entry or IDO-to-Clight bridge is needed under the accepted boundary.  Later gameplay still needs per-step preservation through timer 131, and negative quicksand plus dialog remains a separate constrained alternative.
