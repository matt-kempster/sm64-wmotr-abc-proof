...
**What is already known.** The corrected timer-131 point works conditionally, but it needs at least a `960`-unit gap between Mario's displayed and collision heights (`1010` at warp center), and a second failed floor lookup is fatal.  Normal creation, Mario's script, ordinary callbacks, stock graphical offsets (at most `+240`), and reuse of a different object slot cannot make that gap; a deliberately non-stock `+1160` offset confirms that the geometry itself would work.  The official JP starting memory now proves that every object slot begins with both relevant values cleared, and the generated game data proves that Mario is the only normal behavior assigned to player list 0; the checked load order, allocator clearing, behavior forwarding, and list insertion also agree with that story.  The trace proof now permits ordinary list rewrites rather than pretending every link is frozen: once the live spawn produces Mario's active slot, behavior, and list-0 connection, every later step must either be one of the harmless writes or preserve those facts.  Built-in calls do so automatically, and each unresolved outside call must either preserve the exact cells at that call or disclose a separately checked writer effect.  These are initial-memory and source facts, not yet a completed execution of the spawn, so the route remains open; out-of-bounds writes, invalid pointers, ACE, and DMA remain outside the current execution model rather than disproved retail ideas, while negative quicksand plus dialog remains a separate constrained alternative.

**What closes it.** Execute the ordinary JP prefix from object-list clearing through Area-1 loading and Mario's spawn, showing that the allocated Mario slot still has the cleared values, receives Mario's normal behavior, and is actually linked into player list 0; then show that every reachable internal update is one of the checked harmless cases and give each reachable unresolved outside call either an exact protected-cell guarantee or a checked writer effect.  That would disprove every remaining corruption-style version that is expressible as a successful in-bounds game execution, while the first step that breaks the classification would identify a concrete producer worth testing.  The negative-quicksand/dialog mechanism still needs both a nonstandard negative seed and an X/Z transport to the warp.  Out-of-bounds, ACE, and DMA versions require separate retail machine semantics and should not consume ordinary Clight route-search effort; only a surviving in-scope producer justifies proceeding to the live failed lookup, top-owned retry, clean top activation/explosion, JP displacement, and the Act-3 and Act-6 continuations.
