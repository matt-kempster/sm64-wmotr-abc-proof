...
| 10 | Direct Area-2 gates | Held-A jump-kick or B rollout from the upper elevator shaft | Low; table-triggered action has no writer or proved consumer |
| 11 | Direct Area-2 gates | Lower-aperture impulse, clip, or support switch | Low-medium; pole-table fallback has no writer or timing witness |
| 12 | Ink installation | Negative quicksand depth plus stalled automatic dialog | Very low; table mutation now needs a concrete valid alias or outside write |
...
**Overall rank: 12. Family priority: 2. Likelihood: very low; no named
controller-driven table mutation producer exists.**
...
**What is already known.** Exact binary32 models can build more than the required height gap, but the checked clean zero-A rules cannot create the needed negative starting depth: the only normal late negative writer is long-jump landing, whose ordinary setup inherits an A press, and ordinary shorter landings end too early.  All stock interaction choices are also checked and none supplies long jump.  The three tempting action tables really are writable storage—`320` bytes total—but no named ordinary controller operation writes them: controller state only selects bounded entries to read, with no named table assignment or separate explicit address-taking site.  A hypothetical four-byte knockback edit could request any action, including long jump, and a compatible handler-pointer edit could redirect a coin or pole collision, but no such named writer exists; only a concrete valid alias or reached outside-call write remains in the current model.  Even granting the negative value and unlimited non-reanchoring stalls, Mario's raw X/Z stays near `(5760,4900)` instead of the upper warp, the next ordinary shallow-quicksand update resets the known negative values to positive `1.6`, ordinary dialogs reanchor, and the checked fresh-star timing misses vertically.

**What closes it.** Seed work still comes first, but table search is now sharply reduced: either show the exact valid in-bounds alias or reached outside routine that overwrites a handler or knockback cell, or prove that neither can touch the private table blocks during the clean linked run; the latter removes table mutation completely and helps disprove the negative seed without any X/Z search.  A different surviving seed must likewise identify its exact descriptor, action, timer, callback, alias, or outside effect.  Only after one seed survives should the same run be tested for raw X/Z transport to the upper warp before reset or reanchoring.  Out-of-bounds writes, ACE, and post-undefined-behavior continuations need a separate machine model.  See the [writable-table audit](notes/writable-action-table-mutation.md) and [negative-quicksand/dialog audit](notes/negative-quicksand-unreanchored-dialog.md).
