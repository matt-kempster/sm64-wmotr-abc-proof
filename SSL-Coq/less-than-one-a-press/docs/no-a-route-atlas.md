...
**What is already known.** The timer-131 geometry works: the corrected midpoint is on the modeled spinning top, the old home-position sample is not, and the midpoint needs at least a `960`-unit gap between Mario's displayed and collision heights (`1010` at warp center).  An injected setup with a fully activated top reaches the conditional JP displacement and Act-6 evidence, while a second failed floor lookup is fatal in the audited schedule.  For normal stock play, both game versions have now been checked from Mario's creation through both object-update paths and every ordinary helper called by Mario's three callbacks: none can switch on the dangerous graphics-copy flag or supply the large graphical offset, even when the code names the same storage differently.  Fresh allocation clears both values, Mario's script switches on a harmless different flag and has no offset command, every stock offset command elsewhere is at most `+240`, and the only large cross-object copy belongs to actors absent from the audited Area-1 setup.  A deliberately non-stock `+1160` offset still makes the retry succeed, so the geometry is possible but the normal stock script, callback-helper, and anchor explanations are disproved at the checked source boundary; forged behavior, corrupted or aliased storage, outside code, object-slot reuse, and failure to preserve Mario's live object identity remain open.

**What closes it.** First prove in live play that the object being updated is still the Mario object created at entry, its storage slot has not been reused, and its cleared flag and small offset survive every indirect call and outside effect; that would eliminate the remaining corruption-style versions of this idea, while one concrete contrary execution would establish the installer.  The separate negative-quicksand/dialog idea must also be proved or ruled out.  Only if some clean gap producer survives does it make sense to finish the first failed lookup, the top-owned retry, the top's activation and explosion, the JP displacement, and the separate Act-3 and Act-6 continuations.
