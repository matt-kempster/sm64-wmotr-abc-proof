...
| 15 | Eyerok | [Board and ride a raised hand into a lower Area-2 route](#route-rank-15) | Medium as a proved local ride, but very low as a full route; the promoted two-hand barrier absorbs all seven upward classes, leaving one live hand/floor projection escape or a separately raised predecessor |
...
**What is already known.** A hash-checked North-American retail run starts from game-confirmed ownership on the real hand and uses one B edge with A already held to perform idle to punching to jump-kick without a new A edge, catch the hand, and ride its six upward steps to Y `-943`; a separate suffix confirms several controller-selectable Eyerok movements but never boards a useful hand.  Jump-kick replaces vertical speed with `20`, favorable conservation still makes every seed through `31` fail, and a finite search proves that none of `181,944` generous boss/two-hand/effect cases can manufacture seed `32`.  The formerly listed seven dynamic-support classes have now been folded into the active two-hand barrier: both generated versions contain only the checked vertical writers, all positive episodes fit a `288`-unit rise budget, every upward static floor is at or below `384`, and the complete hand meshes have a `507`-unit scaled top; even allowing one hand to stand on the other and then granting Mario an excessive `630`-unit rise reaches only Y `1809`, below the Area-2 query threshold `1889`.  This is an arbitrary-length proof inside that generous relation, not yet a claim that every live game frame belongs to it.  See the [controller-authentic ride, VSC, schedule, and dynamic-support audit](notes/rank15-eyerok-controller-ride.md) and [controller-manipulation receipt](notes/eyerok-controller-manipulation.md).

**What closes it.** Reach the confirmed idle-on-hand boundary from ordinary controller play and build one memory-faithful live projection that keeps the two real hands in spawn/list order, identifies every selected static or hand-owned floor, and classifies each reached game step as a checked direct pose, landing, bounded rise, nonrise, wait, or deletion; success would apply the promoted barrier to all seven upward classes at once and close dynamic support, while the first unclassifiable pose, floor owner, list-order change, writer, or lifetime transition would be the exact counterexample candidate to test.  A positive route must instead exhibit that escape or authenticate a separate incoming seed of at least about `32`, then pass the wall, horizontal, hand-to-warp, and Act-3 continuation checks; Act 6 remains separate.
