...
- **Writable action-table mutation is no longer a free-form alias/external
  ...
  private table relation at successful initialization and carries it through
  every classified live step; only the reached-step classification remains.
...
| 10 | Direct Area-2 gates | Held-A jump-kick or B rollout from the upper elevator shaft | Low; table fallback awaits a failed reached-step check and a useful consumer |
| 11 | Direct Area-2 gates | Lower-aperture impulse, clip, or support switch | Low-medium; pole-table fallback awaits the same concrete failure and a timing witness |
| 12 | Ink installation | Negative quicksand depth plus stalled automatic dialog | Very low; no initialized table alias, with reached-step classification left |
...
**Overall rank: 12. Family priority: 2. Likelihood: very low; the action-table
seed has no defined first producer in the checked boundary.**
...
**What is already known.** Exact binary32 models can build more than the required height gap, but the checked clean zero-A rules cannot create the needed negative starting depth: the only normal late negative writer is long-jump landing, whose ordinary setup inherits an A press, and ordinary shorter landings end too early.  All stock interaction choices are checked and none supplies long jump.  The three tempting action tables are `320` bytes of writable engine storage, so an edit made after boot would survive an ordinary level change into SSL, but all 38 modeled US/JP units have no saved initializer address or public table name, the only four uses per version are final reads, both linked programs contain the expected valid private blocks, and the pinned 431-file source tree names the tables only in `interaction.c`.  The proof now builds the private table relation directly from successful initialization and carries it across harmless stores, copying, allocation, freeing, and outside calls, and across any finite live execution whose reached steps pass that exact check; an outside routine cannot be the first writer or return a table address.  Even granting the negative value and unlimited non-reanchoring stalls, Mario's raw X/Z stays near `(5760,4900)` instead of the upper warp, the next ordinary shallow-quicksand update resets the known negative values to positive `1.6`, ordinary dialogs reanchor, and the checked fresh-star timing misses vertically.

**What closes it.** Check every reached live step against the now-constructed private-table relation; a complete check removes table mutation from successful in-bounds play, while a failure identifies the exact store, copied value, allocation, free, or outside-call argument that reopens it.  Then either rule out every other defined negative-depth seed or exhibit one exact descriptor, action, timer, callback, or specified outside effect.  Only after a seed survives should the same run be tested for raw X/Z transport to the upper warp before reset or reanchoring.  Out-of-bounds writes, ACE, and post-undefined-behavior continuations need a separate machine model.  See the [writable-table audit](notes/writable-action-table-mutation.md) and [negative-quicksand/dialog audit](notes/negative-quicksand-unreanchored-dialog.md).
...
**What closes it.** Connect the checked stock warp, reinitialization, and non-Wing values to the same live Mario receiver, then execute the initial descent and every floor query through the intended live-elevator landing and connect every checked quarter-step to its live wall, floor, ceiling, action transition, and collision result.  That would eliminate the ordinary held-A and B-rollout vertical versions and leave horizontal clips or another named writer class to test.  A Wing version can reopen only by identifying a nonstock course or warp, a different live receiver, or another in-scope writer that grants Wing after reinitialization; merely arriving at the Area-1 entrance with Wing no longer suffices.  A table-triggered version must first fail the reached-step private-table check at one exact aliasing step, then replay the matching upper interaction and resulting action through the cut.  Either a live trajectory crosses the cut or this finite action/collision split becomes exhaustive.
...
**What closes it.** Instantiate and complete the current conditional Float32 collision-phase theorem with linked execution and an exhaustive pole-action exit split, then classify every impulse, clip, support, and external writer at the first target-side crossing.  For the table fallback, the same run must first expose the exact step that breaks the private-table invariant, preserve normal pole handling until the useful contact, trigger the substituted handler or knockback entry, and replay the resulting action across the aperture.
