...
**What is already known.** The corrected timer-131 setup works when supplied, but it needs at least a `960`-unit gap between Mario's displayed and collision heights (`1010` at warp center), and a second failed floor lookup is fatal.  Normal creation, Mario's script, ordinary callbacks, stock graphical offsets (at most `+240`), and reuse of another object slot cannot make that gap; a deliberately non-stock `+1160` offset confirms that the geometry would work.  A hash-checked original-JP run records the accepted level-select sequence as `clear objects → load Area 1 → spawn Area-1 objects → spawn Mario → initialize Mario`, then observes Mario in pool slot 67 with the expected behavior, matching game-state pointer, safe flag, zero graphical offset, and one-object player-list ring.  The proof now starts at that level-select clear, distinguishes the two spawn callers, checks the recorded order and slot arithmetic, derives all of those endpoint facts from exact game-memory reads, and finds no direct dangerous flag or graphical-offset writer in either the 85-function entry family or the broader 150-function family that also allows a first object update.  The narrower entry family leaves only three possible outside sites—an object-unload sound stop, the Mario-area sound-bank stop, and square root during surface loading—while the broader family has five names at eight sites.  The continuous step-by-step execution is still unbuilt: indirect dispatch, valid aliases, branch reachability, and the exact effects of each outside call actually reached must still be tied to the same run, while out-of-bounds writes, invalid pointers, ACE, and DMA remain outside the current model and negative quicksand plus dialog remains a separate constrained alternative.

**What closes it.** Build the one continuous proof execution from the accepted level-select clear through the two distinct spawn calls and initialization, show that its final reads are the recorded slot-67 behavior/list/safe-tail reads, and classify every intervening internal step, indirect dispatch, and outside call as preserving the two protected values or making a checked safe write; no ordinary castle-entry connection is required for this route.  The new proof theorem then combines that entry certificate with later per-step coverage to disprove every remaining corruption-style version expressible as a successful in-bounds game execution, while the first unclassifiable step identifies a concrete producer worth testing.  The separate negative-quicksand/dialog mechanism still needs both a nonstandard negative seed and an X/Z transport to the warp, and out-of-bounds, ACE, and DMA versions require a different retail machine model; only a surviving producer justifies proceeding to the live failed lookup, top-owned retry, top activation/explosion, JP displacement, and the Act-3 and Act-6 continuations.
