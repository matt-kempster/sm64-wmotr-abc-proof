...
**What is already known.** Exact binary32 models can build more than the required height gap, but the checked clean zero-A rules cannot create the needed negative starting depth: the only normal late negative writer is long-jump landing, whose ordinary setup inherits an A press, and ordinary shorter landings end too early.  All stock interaction choices are checked and none supplies long jump.  The three tempting action tables are `320` bytes of writable engine storage, so a hypothetical edit made after boot would survive an ordinary level change into SSL, but all 38 modeled US/JP units have no saved initializer address or public table name, the only four uses per version are final reads, both linked programs contain the expected valid private blocks, and the pinned 431-file source tree names the tables only in `interaction.c`.  The completed execution proof starts from the selected initialized task state, classifies every reached assignment, copy, allocation, free, call, return, continuation, indirect dispatch, and legitimate terminal table read, and carries one private-block injection and byte frame through every finite successful in-bounds US/JP Clight run; consequently no such run can first write a table byte or obtain a table pointer through an outside call.  Even granting the negative value and unlimited non-reanchoring stalls, Mario's raw X/Z stays near `(5760,4900)` instead of the upper warp, the next ordinary shallow-quicksand update resets the known negative values to positive `1.6`, ordinary dialogs reanchor, and the checked fresh-star timing misses vertically.

**What closes it.** Writable-table mutation is closed for successful in-bounds executions of the selected CompCert programs, so the remaining in-model task is either to rule out every other defined negative-depth seed or exhibit one exact descriptor, action, timer, callback, or specified outside effect; only after a seed survives should the same run be tested for raw X/Z transport to the upper warp before reset or reanchoring.  Reopening the table branch requires a counterexample to the proved start or step relation, while an out-of-bounds write, ACE, DMA, or post-undefined-behavior continuation requires a separate retail-machine model rather than another Clight producer search.  See the [writable-table audit](notes/writable-action-table-mutation.md) and [negative-quicksand/dialog audit](notes/negative-quicksand-unreanchored-dialog.md).
