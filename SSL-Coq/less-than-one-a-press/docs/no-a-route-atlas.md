...
| 12 | Ink installation | Negative quicksand depth plus stalled automatic dialog | Very low; named internal and alias seeds are closed, leaving live-step and outside-call framing |
...
**Overall rank: 12. Family priority: 2. Likelihood: very low; no clean internal
negative-depth producer remains in the checked source boundary.**
...
**What is already known.** A negative depth would create more than enough displayed-height gap, but the clean zero-A game code has no known way to create that starting value: the audit found every direct depth change—18 assignments in eight routines per game version—and every one is nonnegative or preserves nonnegativity except the late long-jump landing subtraction, while the normal route to that landing requires A and shorter landings finish before the dangerous timer.  Stock interactions do not select long jump, the writable action tables provably retain their original contents in every successful in-bounds selected CompCert execution, and a whole-game alias audit found only Mario's intended base pointer, with no code-created interior or untyped pointer, whole-state or whole-descriptor copy, saved pointer, returned pointer, or saved descriptor address that could secretly reach the relevant values.  No exact descriptor, action, timer, callback, or typed internal writer has survived these checks; what remains is to connect every live execution step to the audited cases and constrain CompCert's genuinely abstract outside calls, whose effects cannot be inferred from their C declarations.  Even if a negative value is granted, the checked dialog loop leaves Mario near `(5760,4900)` rather than the upper warp, ordinary shallow quicksand resets the tested values to positive `1.6`, ordinary dialogs reanchor, and the fresh-star timing misses vertically.

**What closes it.** Follow one accepted live Clight execution from its clean zero-depth start and show that every reached internal step is one of the audited harmless depth/action cases while Mario keeps the intended state identity, then give every reached unresolved outside call an exact promise that it preserves the input, action, timer, depth, Mario-state pointer, and landing-descriptor bytes; that would disprove the remaining successful in-bounds CompCert seed branch before any X/Z transport search is needed, while the first step or outside effect that fails the check would be the concrete producer to test.  CompCert deliberately leaves ordinary external-call effects abstract, so this final result needs an implementation specification or an accepted external-frame policy rather than a guess from the function prototype.  Out-of-bounds writes, ACE, DMA, and execution after undefined behavior remain separate retail-machine extensions.  See the [writable-table audit](notes/writable-action-table-mutation.md), [defined-producer proof](../proofs/NegativeDepthDefinedProducerClosure.v), and [negative-quicksand/dialog audit](notes/negative-quicksand-unreanchored-dialog.md).
