...
- **Rank 15's local hand ride is real, but ordinary VSC does not finish it:**
  ...
  search is the live-effect bridge plus exact dynamic-support geometry for
  seven upward hand-action classes.
...
| 15 | Eyerok | [Board and ride a raised hand into a lower Area-2 route](#route-rank-15) | Medium as a proved local ride, but very low as a full route; the finite quotient cannot create seed 32, leaving its live-effect bridge, seven exact dynamic-support geometry classes, or a separately raised predecessor |
...
**What is already known.** A hash-checked North-American retail run starts from game-confirmed floor and platform ownership on the real hand and, on the release poll, writes nothing to Mario while supplying one B edge with A already held; the game performs idle to punching to jump-kick in one frame with no new A edge, catches the ordinary 49-unit first gap, and rides every `+85,+70,+55,+40,+25,+10` step to Y `-943`, while the generated US and JP source contains the same action-loop chain.  A separate paired US suffix wakes Eyerok, forces the target chase, and selects both sweep signs without A, but it never boards the hand and remains too low and badly aligned.  The old `243` figure cannot be stacked because jump-kick replaces vertical speed with `20`; favorable VSC still makes every integral seed through `31` fail, `32` is only the first arithmetic threshold, and all `176` static Area-3 triangles contain no upward bridge.  The new finite scheduler search counts `181,944` combinations in an intentionally generous boss/two-hand/effect quotient and proves for arbitrarily many represented cycles that its six candidate effects cannot turn an incoming seed at most `31` into `32`: they preserve or clear it, install jump-kick `20`, or install the ordinary hand bounce `30`; generated data authenticates the key hitbox and action facts, while a live-frame-to-effect bridge and exact dynamic-support geometry for seven upward hand-action classes remain open.  See the [controller-authentic ride, VSC, and schedule audit](notes/rank15-eyerok-controller-ride.md) and [controller-manipulation receipt](notes/eyerok-controller-manipulation.md).

**What closes it.** Reach the confirmed idle-on-hand boundary from ordinary controller play, connect every reached clean frame to one of the six checked seed-effect classes, then either authenticate a seed of at least about `32` that already exists before them or find a usable two-hand support in one of the seven remaining upward-motion classes—sleep, idle, target, begin-double, double-pound, attacked, or die—and carry its exact Float32 pose, update order, floor ownership, wall and X/Z checks through the hand-to-warp and Act-3 continuation.  To close the route negatively, prove the live-effect connection, attach exact geometry and ownership to the seven finite classes, and show that every return to an equivalent home/action/health state adds no new possibility; Act 6 remains a separate downstream task.
