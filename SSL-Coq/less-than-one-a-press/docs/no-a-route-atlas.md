...
**What is already known.** The supplied setup works, but it needs at least a `960`-unit gap between Mario's displayed and collision heights (`1010` at the warp center), and a second missed floor check is fatal; normal creation, Mario's usual behavior, ordinary callbacks, stock display offsets, and reuse of another object slot do not make that gap, while an intentionally non-stock `+1160` offset confirms that the geometry itself works.  A hash-checked original-JP run now follows one uninterrupted accepted level-select entry—clear old objects, load Area 1, spawn its objects, separately spawn and initialize Mario, then perform Mario's first update—and watches every write to the values that establish Mario's identity and the two dangerous fields.  It records exactly 19 harmless writes: Mario is derived as slot 67, both game pointers select that object, its normal behavior and one-object player list are installed, allocation first writes zero to both dangerous fields, and Mario's first behavior later writes the safe flag value `0x100`; no intervening instruction, indirect call, or outside machine-code routine writes an unsafe value there.  Coq replays those writes from arbitrary old watched values and derives the recorded endpoint, so the corruption-style entry is ruled out for this authenticated retail execution.  A fully native CompCert execution is still unavailable because the retail game was compiled by IDO rather than CompCert and three calls are left abstract in the current model; that is a proof-model gap, not a surviving producer.  Later gameplay still needs the existing per-step preservation proof, and negative quicksand plus dialog remains a separate constrained alternative.

**What closes it.** Treat the authenticated 19-write machine receipt as the accepted entry theorem, or add a genuine IDO-machine-to-Clight state-and-step connection (or independently execute the reconstructed Clight start state) and give the three abstract outside calls exact effects, then combine the now-derived safe endpoint with the later per-step preservation proof through timer 131; no ordinary castle-entry connection is required.  That would rule out every remaining successful in-bounds corruption-style version in the chosen model, while the first later write that fails preservation would identify a real producer.  Negative quicksand plus dialog still needs both a nonstandard negative seed and an X/Z transport to the warp, and out-of-bounds writes, arbitrary code execution, and DMA require a separate retail-machine model; only a surviving producer warrants continuing to the failed lookup, top-owned retry, top activation/explosion, JP displacement, and the separate Act-3 and Act-6 continuations.
