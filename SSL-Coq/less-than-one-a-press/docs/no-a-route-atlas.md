...
- **Writable action-table mutation is no longer a free-form alias/external
  ...
  each version has only four terminal reads, both linked programs contain the
  expected valid private blocks, and ordinary level changes do not reload the
  tables; constructing and carrying the private start-to-trace invariant is
...
| 10 | Direct Area-2 gates | Held-A jump-kick or B rollout from the upper elevator shaft | Low; table fallback awaits the private-invariant bridge and a useful consumer |
| 11 | Direct Area-2 gates | Lower-aperture impulse, clip, or support switch | Low-medium; pole-table fallback awaits the same bridge and a timing witness |
| 12 | Ink installation | Negative quicksand depth plus stalled automatic dialog | Very low; action-table seed has no defined first producer, with one live-invariant bridge left |
