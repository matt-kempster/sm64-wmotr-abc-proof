...
**What is already known.** The corrected timer-131 point works conditionally, but it needs at least a `960`-unit gap between Mario's displayed and collision heights (`1010` at warp center), and a second failed floor lookup is fatal.  Normal creation, Mario's own script, ordinary callbacks, stock graphical offsets (at most `+240`), and normal reuse of a different object slot cannot make the gap; a deliberately non-stock `+1160` offset confirms that the geometry itself would work.  The proof now also follows a real game-execution trace rather than only a list of hypothetical writes: if Mario begins with the two relevant values cleared, has the expected behavior, is connected to the list-0 head in the same active pool slot, and every reached update either performs one of the checked harmless writes or leaves the protected bytes alone, then Mario keeps that identity and the dangerous graphical update remains impossible for the whole trace.  Known built-in engine calls satisfy this automatically, while any unresolved outside call must be given a concrete protected-memory guarantee.  The remaining in-scope possibilities are therefore the missing live entry receipts or a first reachable write/call that violates this classification; out-of-bounds writes, invalid pointers, ACE, and DMA remain outside the current execution model rather than retail disproofs.  The negative-quicksand/dialog idea is a separate constrained alternative.

**What closes it.** Derive the three missing entry facts from the live allocator/spawn execution—the cleared words, Mario's expected behavior pointer, and the actual list-0 link chain—then classify every reachable internal write with the checked harmless cases and give every reachable unresolved outside call an exact protected-memory effect; the new trace theorem will then disprove all remaining in-scope corruption-style versions, while the first step that fails the classification will identify a concrete new producer.  The negative-quicksand/dialog mechanism still needs both a nonstandard negative seed and an X/Z transport to the warp.  Out-of-bounds, ACE, and DMA versions require separate retail machine semantics and should not consume ordinary Clight route-search effort; only a surviving in-scope producer justifies proceeding to the live failed lookup, top-owned retry, clean top activation/explosion, JP displacement, and the Act-3 and Act-6 continuations.
