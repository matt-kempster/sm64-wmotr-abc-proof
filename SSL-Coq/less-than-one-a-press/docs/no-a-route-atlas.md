...
| 15 | Eyerok | [Board and ride a raised hand into a lower Area-2 route](#route-rank-15) | Medium as a proved local ride; the deterministic chase/sweep suffix is now live but nonproductive, and a complete route remains very low |
...
**What is already known.** A hash-checked North-American retail run starts from game-confirmed floor and platform ownership on the real hand and, on the release poll, writes nothing to Mario while supplying one B edge with A already held; the game performs idle to punching to jump-kick in one frame with no new A edge, catches the ordinary 49-unit first gap, and rides every `+85,+70,+55,+40,+25,+10` step to Y `-943`, while the generated US and JP source contains the same action-loop chain.  A separate paired US suffix now wakes Eyerok, occupies the RNG-independent target strip, releases the chase, and selects both sweep signs with zero A polls and no post-entry write, but its closest grounded sample is Z=`-1889.765` at the arena edge, its safe mirror bends backward in Z after stored speed reaches zero, and neither run ever boards the hand; the source cap also leaves the tracking mesh behind the warp and its Y=`-928` top `288` below the tunnel-query threshold.  The old `243` figure cannot be stacked onto conserved speed because jump-kick replaces vertical speed with `20`; even under favorable VSC assumptions every integral seed through `31` fails and `32` is only the first arithmetic threshold, while all `176` static Area-3 triangles contain no upward bridge between arena and tunnel.  See the [controller-authentic ride and VSC audit](notes/rank15-eyerok-controller-ride.md) and [controller-manipulation receipt](notes/eyerok-controller-manipulation.md).

**What closes it.** Reach the confirmed idle-on-hand boundary from ordinary controller play, then exhibit a clean conserved seed of at least about `32`, a dynamic intermediate support, an already-raised Mario predecessor, or another departure that passes the exact wall, X/Z, action, and collision checks before executing the hand-to-warp and Act-3 continuation; repeating the now-checked deterministic chase/sweep without one of those additions is insufficient.  An impossibility result instead must prove that every clean boss/hand schedule supplies no seed above the checked bound and no suitable dynamic support, while Act 6 remains a separate downstream task.
