...
| 10 | Direct Area-2 gates | [Held-A jump-kick or B rollout from the upper elevator shaft](#route-rank-10) | Very low; one clean B-only rollout now hits the real elevator wall and returns, while universal and held-A live coverage remain |
...
**What is already known.** A held-A plus B jump kick is a genuine no-new-edge action, and B-only dive/rollout movement is real.  The proof checks the important rising samples at maxima `134` and `224.5`, then checks conservative full-return envelopes—64 jump-kick and 84 rollout quarter steps—which reveal the slightly later true maxima `135` and `227.5`; both remain below the strict `231` wall cutoff, though rollout has only 3.5 units to spare.  A new uninterrupted, read-only original-JP replay starts from the established zero-A four-pillar route, enters Area 2, finds Mario in slot 10 and the unique live elevator, matches all 17 descent samples, and sees that same elevator own every selected floor in all 671 observed frames.  Controller movement then makes a legal B speed-kick dive and lands it on the moving elevator; the second B starts a real forward rollout, which immediately meets the elevator's east inner wall at X `411`, loses all horizontal speed, peaks at relative endpoint Y `220`, remains inside the cage for all 20 observed rollout frames, and lands back on the elevator.  The run has no A input and no Wing frame, and the observed wall is owned by the same live elevator.  This disproves that concrete B-only trajectory, while the stock source also shows that the Area-1-to-Area-2 transition resets Wing; a hypothetical post-reset Wing state would have only two above-cutoff samples, `234` and `232`, and is not a stock entry.

**What closes it.** The tested B-only schedule is closed, so the remaining ordinary work is to replay the held-A jump kick and any genuinely different launch poses, and to connect every internal wall, floor, and ceiling query—not only the end-of-frame selected pointers—to the already checked full-return envelope in JP and US.  If that connection shows that every clean launch either selects the same intact cage wall or returns to the elevator below the cutoff, the ordinary family is disproved; the first different wall, skipped query, horizontal clip, support switch, action writer, or identity change instead becomes a concrete candidate.  A Wing version can reopen only through a real post-reset Wing grant or different live receiver, and the table-triggered version remains outside successful in-bounds selected CompCert runs unless the accepted memory invariant is refuted or a retail-machine corruption model is added.
