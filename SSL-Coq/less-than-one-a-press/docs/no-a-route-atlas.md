...
- **Writable action-table mutation is no longer a free-form alias/external
  lead:** each version has only four terminal reads, and CompCert externals
  preserve and cannot reveal the private blocks once the live invariant is
  installed; constructing that start-to-trace invariant is the remaining
  bridge.
