...
**What is already known.** The corrected timer-131 setup works when supplied, but it needs at least a `960`-unit gap between Mario's displayed and collision heights (`1010` at warp center), and a second failed floor lookup is fatal.  Normal creation, Mario's script, ordinary callbacks, stock graphical offsets (at most `+240`), and reuse of another object slot cannot make that gap; a deliberately non-stock `+1160` offset confirms that the geometry would work.  The proof shows that all object slots start with the two dangerous values cleared and that Mario is the only normal player-list behavior, while a hash-checked original-JP run now records the real `clear objects → load Area 1 → spawn Mario → initialize Mario` sequence and then observes Mario in pool slot 67 with matching game-state pointer, the expected safe flag and zero graphical offset, and a one-object player-list ring.  That run uses level select and is a read-only machine-code observation, not yet the step-by-step proof certificate: the code between checkpoints and outside calls is still unclassified, ordinary castle-entry equivalence is open, out-of-bounds writes, invalid pointers, ACE, and DMA are outside the current model, and negative quicksand plus dialog remains a separate constrained alternative.

**What closes it.** Translate the authenticated machine-code prefix into the proof's step-by-step execution model, derive the observed Mario slot, behavior, and player-list facts there, and classify every intervening game step and outside call as either preserving the two protected values or performing a checked safe write; also connect an ordinary castle entry if level select is not accepted as the start boundary.  Completing that bridge would disprove every remaining corruption-style version expressible as a successful in-bounds game execution, while the first step that fails would identify a concrete producer worth testing.  The separate negative-quicksand/dialog mechanism still needs both a nonstandard negative seed and an X/Z transport to the warp, and out-of-bounds, ACE, and DMA versions require a different retail machine model; only a surviving producer justifies proceeding to the live failed lookup, top-owned retry, top activation/explosion, JP displacement, and the Act-3 and Act-6 continuations.
...
**What is already known.** A held-A plus B jump kick is a genuine no-new-edge action, and B-only dive/rollout movement is real.  Exact binary32 replay checks every one of the 32 jump-kick and 40 rollout quarter-steps: all 72 collision queries stay below the strict `231` wall cutoff, with maxima of `134` and `224.5`, and the generated quarter-step function has only six possible result codes.  This corrects the old Wing estimate: a retained Wing Cap produces a transient `234` query even though its frame endpoints peak at `228`, so endpoint arithmetic cannot eliminate that case.  Mario's normal initializer writes only non-Wing flag values and resets the cap timer, but the live descent, elevator landing and ownership, wall/floor choices, action transitions, and preservation of that cap state are not yet executed in the proof.

**What closes it.** Execute the initial descent and every floor query through the intended live-elevator landing, then connect every checked binary32 quarter-step to its live wall, floor, ceiling, action-transition, and collision result while proving that the initialized non-Wing cap state persists.  That would eliminate the ordinary held-A and B-rollout vertical versions and leave horizontal clips or other named writer classes to test; if the cap proof fails, the retained-Wing `234` transient is the concrete branch to execute.  Either a live trajectory crosses the cut or this finite action/collision split becomes exhaustive.
