...
out_dir="$(mktemp -d \
    "$project_dir/build/instrumentation/jp-rank4-warp-top.XXXXXX")"
mkdir -p "$out_dir/config" "$out_dir/data" "$out_dir/shots"
plugin="$out_dir/jp-rank4-warp-top.so"
raw_log="$out_dir/jp-rank4-warp-top.raw.log"
receipt="$out_dir/warp-top-receipt.txt"
...
grep -a '^RANK4_' "$raw_log" >"$receipt"
if grep -q '^RANK4_FAILURE,' "$receipt" \
    || ! grep -q '^RANK4_WARP_TOP_RESULT,.*invariant=1$' "$receipt" \
    ...
    || ! grep -q '^RESULT,.*mode12PillarsComplete=1,.*warpDisappeared=1,warpUsedObj=1,platformTop=0,.*aPressedFrames=0,aDownFrames=0,controllerAFrames=0$' \
        "$raw_log"; then
    printf '%s\n' 'Rank-4 warp/top invariant or route receipt failed' >&2
    printf 'Raw log: %s\n' "$raw_log" >&2
    exit 3
fi

if [ -f "$script_dir/expected-warp-top-receipt.txt" ]; then
    if ! diff -u "$script_dir/expected-warp-top-receipt.txt" "$receipt"; then
        printf '%s\n' 'exact Rank-4 warp/top receipt failed' >&2
        exit 3
    fi
fi

printf '%s\n' 'Exact Rank-4 warp/top receipt passed.'
printf 'Receipt: %s\n' "$receipt"
