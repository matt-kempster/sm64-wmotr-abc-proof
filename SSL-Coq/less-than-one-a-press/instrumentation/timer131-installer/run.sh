...
out_dir="$project_dir/build/instrumentation/timer131-installer"
mkdir -p "$out_dir/config" "$out_dir/data" "$out_dir/shots"
plugin="$out_dir/timer131-installer.so"
raw_log="$out_dir/timer131-installer.raw.log"
trace="$out_dir/timer131-installer.trace.txt"

gcc -shared -fPIC -std=c99 -Wall -Wextra -Werror -O2 \
    "$script_dir/timer131_installer_probe.c" -ldl -lm -o "$plugin"
...
grep '^INSTALL' "$trace"
grep '^NEXT' "$trace"
grep -q '^RESULT,armed=1,installed=1,next=1,retryCopied=1,retryOwnerTop=1,warpAction=1,usedObjWarp=1,platformTop=1,aPressedFrames=0,aDownFrames=0,controllerAFrames=0$' "$trace"
printf 'Trace: %s\n' "$trace"
