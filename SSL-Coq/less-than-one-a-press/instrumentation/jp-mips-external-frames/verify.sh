...
classify_store() {
    case "$1" in
        8032078c|80320864) printf '%s' audio-bank ;;
        8031e150) printf '%s' music-mask ;;
        8032014c|8032022c|8031d7e4|8031d7f0|8031d824|8031d828|8031d82c|\
        8031d878|8031d8b0|8031d910|8031d914|8031d918)
            printf '%s' sequence-player ;;
        *) printf '%s' stack ;;
    esac
}
...
while IFS=, read -r name start end word_count expected_sha256; do
    ...
    dd if="$rom" of="$slice" bs=1 skip="$offset" count="$byte_count" \
        status=none
...
if ! diff -u "$script_dir/expected-store-call-manifest.csv" \
    "$actual_manifest"; then
    printf '%s\n' 'retail store/call manifest mismatch' >&2
    exit 3
fi

printf '%s\n' 'Authenticated 332 instructions, 42 stores, and 8 direct calls.'
