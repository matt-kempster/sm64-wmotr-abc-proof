(** One-lookup JP [gMarioState] source receipt and symbol transport. *)
...
Lemma jp_level_update_is_mario_state_source_unit :
  nIn jp_level_update.prog jp_units.
Proof. unfold jp_units. do 28 right. now left. Qed.

Theorem jp_level_update_gMarioState_defmap_checked :
  (AST.prog_defmap (Ctypes.program_of_program jp_level_update.prog)) !
      jp_level_update._gMarioState =
    Some (Gvar jp_level_update.v_gMarioState).
Proof. vm_compute. reflexivity. Qed.

Theorem jp_official_area1_state_pointer_symbol_exists :
  exists block,
    Genv.find_symbol (Clight.globalenv jp_official_cleaned_slice)
      jp_level_update._gMarioState = Some block.
Proof.
  eapply (jp_source_definition_has_official_symbol jp_level_update.prog).
  - exact jp_level_update_is_mario_state_source_unit.
  - pose proof jp_level_update_gMarioState_defmap_checked as H.
