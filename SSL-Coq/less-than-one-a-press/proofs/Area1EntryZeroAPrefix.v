...
From LessThanOneAPress.Generated Require Import
  jp_level_update jp_mario us_level_update us_mario.
From LessThanOneAPress.Proofs Require Import
  CleanedClightPrograms InputSemantics JPWarpLevelEntryResolution
  JPZeroAReachability OrdinaryArea1EntryMemory USViewportRepairedProgramSelection
  USWarpLevelEntryResolution USWholeASTTagRepair.
...
(** The repaired-US ordinary-entry postcondition supplies the same exact
    controller sample, so it starts the same reflexive zero-A suffix. *)
Theorem us_ordinary_area1_entry_memory_has_no_a_edge :
  forall memory addresses x y z previous_down current_down,
    USArea1EntryMemoryPostcondition memory addresses x y z
      (entry_sample_from_history previous_down current_down) ->
    frame_has_no_a_press
      {| frame_previous_down := previous_down;
         frame_current_down := current_down |} ->
    ControllerNoAEdgeMemory memory
      (area1_controller_storage_block addresses) 0.
Proof.
  intros memory addresses x y z previous_down current_down [Hentry _] Hno_a.
  pose proof (no_a_frame_yields_no_a_entry_sample
    previous_down current_down Hno_a) as Hsample_no_a.
  unfold entry_sample_has_no_a_edge in Hsample_no_a.
  econstructor.
...
Corollary us_ordinary_area1_return_starts_zero_a_reachability :
  forall program continuation memory addresses x y z previous_down current_down,
    USArea1EntryMemoryPostcondition memory addresses x y z
      (entry_sample_from_history previous_down current_down) ->
    frame_has_no_a_press
      {| frame_previous_down := previous_down;
    ...
    ZeroAEdgeClightReachable program
      (area1_controller_storage_block addresses) 0
      (Clight.Returnstate Vundef continuation memory)
      (Clight.Returnstate Vundef continuation memory).
Proof.
  intros program continuation memory addresses x y z previous_down current_down
    Hentry Hno_a. constructor.
  unfold ClightStateHasNoAEdge, clight_state_memory; cbn.
  eapply us_ordinary_area1_entry_memory_has_no_a_edge; eauto.
Qed.
...
(** Selected-US counterpart of the JP boundary composition.  Exact
    [_warp_level] resolution is discharged; route, entry execution, symbol
    bindings, input history, and live memory remain explicit premises. *)
Corollary us_viewport_repaired_ordinary_area1_prefix_fixes_zero_a_boundary :
  forall program_initial route_trace continuation memory_before entry_trace
      memory_after addresses x y z previous_down current_down,
    @Smallstep.star _ _ Clight.step2
      (Clight.globalenv us_viewport_repaired_program)
      program_initial route_trace
      (Clight.Callstate (Ctypes.Internal us_level_update.f_warp_level)
        [] continuation memory_before) ->
    USArea1EntrySymbolBindings
      (Clight.globalenv us_viewport_repaired_program) addresses ->
    OrdinaryArea1WarpRequestMemory memory_before addresses ->
    OrdinaryArea1ControllerHistoryMemory memory_before addresses
      previous_down current_down ->
    frame_has_no_a_press
    ...
    Genv.find_funct_ptr (Clight.globalenv us_viewport_repaired_program)
      warp_level_block = Some (Ctypes.Internal us_level_update.f_warp_level) /\
    @Smallstep.star _ _ Clight.step2
      (Clight.globalenv us_viewport_repaired_program)
      program_initial (route_trace ** entry_trace)
      (Clight.Returnstate Vundef continuation memory_after) /\
    Genv.find_symbol (Clight.globalenv us_viewport_repaired_program)
      us_mario._gControllers =
      Some (area1_controller_storage_block addresses) /\
    ClightStateHasNoAEdge
      (area1_controller_storage_block addresses) 0
      (Clight.Returnstate Vundef continuation memory_after) /\
    ZeroAEdgeClightReachable us_viewport_repaired_program
      (area1_controller_storage_block addresses) 0
      (Clight.Returnstate Vundef continuation memory_after)
      (Clight.Returnstate Vundef continuation memory_after).
Proof.
  intros program_initial route_trace continuation memory_before entry_trace
    memory_after addresses x y z previous_down current_down Hroute Hsymbols
    _ _ Hno_a Hentry_steps Hentry_memory.
  destruct us_warp_level_resolves_exact_body as
    [warp_level_block [Hwarp_symbol Hwarp_body]].
  exists warp_level_block. split; [exact Hwarp_symbol |].
  split; [exact Hwarp_body |].
  split.
  - eapply Smallstep.star_trans; eauto.
  - split.
    + exact (us_area1_controller_storage_symbol _ _ Hsymbols).
    + split.
      * unfold ClightStateHasNoAEdge, clight_state_memory; cbn.
        eapply us_ordinary_area1_entry_memory_has_no_a_edge; eauto.
      * eapply us_ordinary_area1_return_starts_zero_a_reachability; eauto.
Qed.
