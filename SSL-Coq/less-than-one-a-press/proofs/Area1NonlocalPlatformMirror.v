(** Exact platform-payload witness for the rank-3 nonlocal-State alias.
    ...
    file shows that [apply_platform_displacement]'s rotation arithmetic is
    numerically capable of producing that exact endpoint in one call: rotate
    the local point [(-1862, 768, -902)] by 180 degrees in pitch around the
    pivot [(-1862, 34041, -902)].  The midpoint identity
    ...
    pivot and angle velocity, or links the functional matrix mirror to a live
    Clight call.  Those are now the precise remaining rank-3 obligations. *)
