...
pitch values 15,664 through 16,031. The transformed floor is spinner 7 triangle
...
`TTCSpinnerSchedule.v` proves both the gain and the remaining limitation.  The
368-unit interval contains the selected first movement `15864 -> 15664` when
the direction is `-1`. Starting just after a direction change, timers 1 through
5 are stationary, while timers 6 and 7 move by a cumulative
`400 * direction`. The second movement exits the interval for either legal
direction. The theorem quantifies over every RNG observation, so dust-induced
seed advances cannot rescue this fixed interval beyond the first movement.
