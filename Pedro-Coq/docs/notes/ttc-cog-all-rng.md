...
two remaining helper-execution premises after the
[reflection and transition follow-up](ttc-cog-transition-execution.md).
...
1. Establish a precise legal in-spot state or state family and preserving
   successive action updates. For the slide kick, execute the two remaining
   helpers (sliding and the full ground step), its complete dispatcher/entry
   path and the following knockback update. Establish the live animation,
   terrain/floor, velocity and sound-queue images used by the executed helpers.
